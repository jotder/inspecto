...
| **AGT-3** | Air-gap guarantee: hosted SDKs physically absent from air-gapped builds | Must | ✅ Built and tested, and the invariant is asserted by a test | **All** — this one *is* edition-wide, but trivially: the guarantee is that the code is **absent**, and in a stock bundle the whole layer is absent (§3.10) |
