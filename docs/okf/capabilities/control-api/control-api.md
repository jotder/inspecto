...
| `API-2` | OpenAPI 3.1 contract (`docs/api/openapi-v1.json`) enforced by `ApiContractTest` | Must | ✅ SHIPPED (W2) — ⚠ **exemplar coverage**: 19 paths / 24 operations of ~332 registered routes; the test asserts doc → live, never live → doc; and the served document's own description claimed the retired legacy table until 2026-09-08 (§2 corrections) | All |
...
|---|---|---|
| **OpenAPI covers 19 of ~332 routes and the test cannot see the gap** | `openapi-v1.json`: 19 paths; `ControlApi.java:1086`: ~332 registrations; `ApiContractTest` asserts doc → live only | A gateway or an external consumer reading the contract sees 6 % of the surface; "OpenAPI-first" is true only of what was written after W2. A live → doc coverage assertion, or a documented decision to keep exemplar coverage, is owed |
