...
| 2026-09-22, 🔴 **REVERSED 2026-09-23** | ⛔ **A created Pipeline stays FLAT at the space config root; `config/<id>/` is REFUSED (D3).** It was signed on the premise that *every shipped Pipeline lives in a per-Pipeline directory* — **counted on disk, that is false**: `config/orders/` holds THREE Pipelines (`orders`, `orders_by_region_feed`, `orders_enriched_rollup`), and the directory names are DOMAIN words, not ids (`config/postmed/` → `postmed_xdr`; `config/mule/` → `mule_transfers`). The shipped convention is **a per-domain directory an author chose**, so `config/<id>/` would have added a THIRD layout beside flat and per-domain — the opposite of the goal that motivated it. ✅ What matters already holds: a created Pipeline and its satellites land TOGETHER, and `subdir:` gives an author a domain home when they want one. ⚠ It was implemented twice and reverted twice; the second attempt proved the cost — a bare `schema_file: <name>.toon` resolves BESIDE ITS OWN CONFIG, so nesting the Pipeline without moving its schema separates them. 🔴 **The lesson is the premise, not the layout: “every shipped X does Y” is a claim to COUNT before a decision rests on it.** | operator (reversed on the operator's behalf, 2026-09-23) |
| 2026-09-22 | **`GET …/graph` is NOT renamed; it says what it is instead (D9).** The read-only projection stays at its name and gains `links.roundTrip: …/graph/raw` plus a projection flag in its own response. 🔴 PUTting a `/graph` body back is refused 422 on 26 of 26 shipped Pipelines — it fails closed everywhere, and it is still the natural wrong guess for any client. A link buys the discoverability; a rename would have broken every client for the same gain. Shipped as `WB-18` (`readOnlyProjection: true` + `links.roundTrip`, the link derived from the request path so a `/spaces/{id}` caller stays in its space); row `GRAPH-READ-SHAPE-NOT-WRITE-SHAPE-1` | operator |
