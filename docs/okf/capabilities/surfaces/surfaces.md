...
| **UI-8** | Settings drawer + consolidated admin settings pane | Should | ✅ SHIPPED (row corrected 2026-09-07 — it had claimed in-flight, uncommitted work). ⚠ The drawer and the pane are **two different components with near-identical names**: the drawer is a navigation launcher that makes no request; the pane is the master-detail surface, with **12** sections ⚠ **Provenance repaired 2026-09-09.** `REQUIREMENTS.md` cited `7e06463`/`12ead9c`; **neither is a valid object in this repository** — casualties of the history rewrite. Matched by subject and by the files they touch: `425f3f9a` (2026-07-07) created `SettingsDrawerComponent`, and `a8218f29` (**2026-07-14**, not 07-07 as the row claimed) is the master-detail `/settings/<section>` pane. | All |
...
| Settings | `inspecto-ui/src/app/modules/admin/settings/settings.component.ts`, `settings.routes.ts` — the ONE entry point since 2026-09-22; the header flyout `inspecto-ui/src/app/layout/common/settings-drawer/settings-drawer.component.ts` was **deleted** that day (mounted nowhere, UI consolidation plan D2), and `/connections`, `/config`, `/spaces`, `/design`, `/notification-center` now redirect into `/settings/<section>` | — (gap, §7 note) |
