...
Config files live under a space's `config/<adapter>/` directory (e.g. `spaces/<id>/config/<adapter>/`; see [Spaces](#spaces-multi-project-layout) below).  A relative data path (`dirs.*`, `sinks[].database`, an enrichment's `input`/`output.database` and `references.<n>.path`, a `local` connection's `base_path`) resolves under the config's **Space directory** (`spaces/<id>/`), so shipped configs say `data/orders/database` (`DATA-DIRS-RESOLVE-AGAINST-CWD-1`, 2026-09-23; see [config safety](config-safety.md#decision-2026-09-23--a-relative-data-path-resolves-under-the-space-directory)). With no Space — a single-tenant example directory — it keeps the working-directory reading, which is the launch directory the example scripts `cd` into.

**Config refs are the exception: they resolve beside their own config, and ONLY there (W1b 2026-07-31 added config-relative first; `SCHEMA-FILE-RESOLVES-AGAINST-CWD-1` 2026-09-23 removed the working-directory second).** The portable form is a bare basename — `schema_file: orders_schema.toon` next to `orders_pipeline.toon` — and every shipped config now uses it. That makes a space directory relocatable/renamable/importable with no edits, and lets the server launch from any directory. ⚠ A repo-root-relative ref (`spaces/<id>/config/…`) no longer loads: it is refused naming both paths when that file exists from the CWD, and not found otherwise — respell it beside its config. See [config safety](config-safety.md#path-containment-one-primitive-five-callers). Applies to `schemas[].schema_file`, `parsing.<asn1|plugin>.segments` values and — since a later consistency fix — `processing.grammar` as well. `dirs.*` are data paths and resolve under the Space directory instead (above) — two bases, both independent of the working directory: in one file `orders_schema.toon` sits beside the config while `dirs.poll: data/inbox/orders` means `<space>/data/inbox/orders`.

**Since W3 (2026-08-17) the bare form is what the product WRITES**, not merely what it tolerates: the Pipelines Parse drawer, the stream-config import planner and the `orders-starter` space template all emit `<name>.toon`. Reading portable refs had worked since W1b while every writer still emitted `spaces/<space>/config/<name>.toon`, so no config was portable in practice unless hand-edited. Pre-W3 long-form refs were respelled in 2026-09-23 (above); the UI compares a stored reference by **name**, never by path, so it still recognises its own long-form schemas.
