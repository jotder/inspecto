...
* [Catalog vs. real node executors](catalog-vs-executors.md) - why 5 catalog "mapping/transformer" processors (schema validator, whitespace sanitizer, expression builder, cast/rename matrix, lookup transcoder) collapse onto only `transform.map`/`transform.join` with no per-processor class; TRY_CAST-to-NULL not quarantine; the static-map half of `transform.lookup` is unbuilt (2026-09-03).
