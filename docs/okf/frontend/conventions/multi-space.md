...
* It **no-ops** when there's no active space (single-tenant, byte-identical) and **exempts** the eight `SERVER_GLOBAL` paths (`/health`, `/ready`, `/metrics`, `/spaces*`, `/bootstrap`, `/auth`, `/public`, `/exchange`) — see `space.interceptor.ts`.
