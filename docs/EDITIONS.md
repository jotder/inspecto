...
| Packaging | core fat-JAR, `-Dauth.mode=none` | core + `security` + `notify-channels` + `backup` modules, `-Dauth.mode=oidc`, TLS on | + `policy` module (ABAC; shipped) — later + shared-store modules, coordinator |
