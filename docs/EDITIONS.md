...
| Mechanism | Used for |
|---|---|
| **Separate Maven module** (e.g. `inspecto-security`, `inspecto-notify-channels`, `inspecto-backup`, `inspecto-geo-link`, `inspecto-exchange`) | Standard-only code (OIDC resource-server, TLS, RBAC; the webhook + SMTP delivery transports; the backup/restore maintenance tasks; the geo map + link analysis routes; cross-space sharing). Personal simply doesn't bundle it. |
