...
| SP-CTL-07 | 🚨 Alert rule dispatcher (`control.alert.dispatch`) | Control, Governance & Sequence Integrity | — | ✅ | ✅ | `alert-rule` | Alert Rules over the ledgers → Alerts → channels; board CP-11/CP-15. ✅ **Gated 2026-09-24** (`PROCESSOR-RELEASE-READINESS-1` G9): the engine is core code, but `inspecto-ops` declares the feature through the `EditionFeatureProvider` ServiceLoader seam, so Personal refuses every authoring door (`POST`/`PUT /alerts/rules`, `/components/alert-rule`, a Decision Rule's `create-alert`) with a 422 `ERR_EDITION_FEATURE` finding and does not arm a rule copied onto disk |
...
| SP-SNK-03 | 🦆 DuckLake catalog (PostgreSQL) sink (`sink.ducklake`) | Sinks, Storage & Destinations | — | ✅ | ✅ | `sink.persistent` | `output.ducklake` / `sinks[].ducklake` — Professional+. ✅ **Gated 2026-09-24** (`PROCESSOR-RELEASE-READINESS-1` G9; the P cell read ✅ against this note until then): `DuckLakeRegistrar` is core code, so `inspecto-backup` declares the feature through `EditionFeatureProvider`; Personal refuses `ducklake.enabled: true` at save (`ERR_EDITION_FEATURE`), as a pipeline load failure, and in the registrar before any batch registers |
...
| SP-SNK-06 | 📜 Long-term compliance archive (`sink.archive`) | Sinks, Storage & Destinations | — | ✅ | ✅ | `acquisition` | Collector `post_action: MOVE archive_path` + the `backup` maintenance task. ✅ **Gated 2026-09-24** (`PROCESSOR-RELEASE-READINESS-1` G9): the `backup` task was already `inspecto-backup`'s (OPS-06); the MOVE is core Collector code, so the same module declares it through `EditionFeatureProvider` and Personal refuses `post_action.on_success: MOVE` at save (`ERR_EDITION_FEATURE`), as a pipeline load failure, and before a remote acquisition. The other post-actions (`RETAIN`/`DELETE`/`RENAME`/`TAG`) and the local `dirs.backup` move stay in every edition |
