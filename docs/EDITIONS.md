...
| CP-13 | Metrics (`/metrics` Prometheus), events feed, audit CSV export | — | ✅ | ✅ | 🟡 **HALF GATED 2026-09-07 (EDG-01 cell 5).** The **`/metrics` exposition** is now the optional `inspecto-metrics` module: on Personal the path answers **503 naming the module** and leaks no exposition (`NoExchangeShipsInThePersonalBuildTest`). 🔴 This is the cell that made EDG-01 P1 — `/metrics` is a `PUBLIC_PATH`, and Personal ships no authenticator and binds every interface, so a Personal install served its full operational telemetry to anything that could reach the port while this cell read `—`. ⚠ Only the exposition moved: `MetricRegistry` nine classes across three modules call it and it stays in core, so the counters still run — nothing reads them out over HTTP. ⛔ `/metrics/acquisition` is NOT this cell: it is Data-Acquisition telemetry on `AcquisitionRoutes`, a core capability. ⚠ **Still open: the events feed + audit CSV export** (`EventRoutes`, `/events*` incl. `/events/export`). It could move whole — but that removes the Events screen's data source from Personal, a visible product change of a different character from a scrape endpoint. Needs an operator call on scope before it is built (plan §19). |
