...
| Packaging | core fat-JAR, `-Dauth.mode=none` | core + `security` + `notify-channels` modules, `-Dauth.mode=oidc`, TLS on | + `policy` module (ABAC; shipped) — later + shared-store modules, coordinator |
...
| Mechanism | Used for |
|---|---|
| **Separate Maven module** (e.g. `inspecto-security`, `inspecto-notify-channels`) | Standard-only code (OIDC resource-server, TLS, RBAC; the webhook + SMTP delivery transports). Personal simply doesn't bundle it. |
