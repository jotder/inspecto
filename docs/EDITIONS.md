...
| Packaging | core fat-JAR, `-Dauth.mode=none` | core + `security` + `notify-channels` + `backup` + `geo-link` modules, `-Dauth.mode=oidc`, TLS on | + `policy` module (ABAC; shipped) — later + shared-store modules, coordinator |
...
| Mechanism | Used for |
|---|---|
| **Separate Maven module** (e.g. `inspecto-security`, `inspecto-notify-channels`, `inspecto-backup`, `inspecto-geo-link`) | Standard-only code (OIDC resource-server, TLS, RBAC; the webhook + SMTP delivery transports; the backup/restore maintenance tasks; the geo map + link analysis routes). Personal simply doesn't bundle it. |
