...
| Packaging | core fat-JAR + `inspecto-connectors.jar` sidecar (**2 staged jars**), `-Dauth.mode=none` | ⚠ **corrected 2026-09-09 — EIGHT optional modules, 11 staged jars:** core + connectors + `security` + `notify-channels` + `backup` + `geo-link` + `exchange` + `metrics` + `events` + `ops` + `postgresql.jar`, `-Dauth.mode=oidc`, TLS on *(this cell named six modules and omitted both sidecars)* | + `policy` module (ABAC; shipped) = **9 modules, 12 staged jars** — later + shared-store modules, coordinator |
