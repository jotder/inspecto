# Editions — Personal / Professional / Enterprise
...
> branches** (see [`BRANCHING.md`](BRANCHING.md)); there is no `personal`/`professional` branch. An edition is
> *which modules are assembled* from a given commit.

## Matrix

| Aspect | **Personal** | **Professional** | **Enterprise (partially shipped)** |
|---|---|---|---|
...
| Compliance scope | none | SOC 2 / ISO 27001 / FedRAMP / HIPAA / PCI | inherits Professional + multi-node controls |
| Packaging | core fat-JAR + `inspecto-connectors.jar` sidecar (**2 staged jars**), `-Dauth.mode=none` | ⚠ **corrected 2026-09-09, again 2026-09-12 — NINE optional modules, 12 staged jars:** core + connectors + `security` + `notify-channels` + `backup` + `geo-link` + `exchange` + `metrics` + `events` + `ops` + `agent` + `postgresql.jar`, `-Dauth.mode=oidc`, TLS on *(this cell named six modules and omitted both sidecars; `agent` joined on 2026-09-12 — `PKG-5`)* | + `policy` module (ABAC; shipped) = **10 modules, 13 staged jars** — later + shared-store modules, coordinator |
...
> Professional and Enterprise install unreachable on upgrade, so the flag lets a deployment restrict itself
...
| Mechanism | Used for |
|---|---|
| **Separate Maven module** — **all nine** (corrected 2026-09-09; this list omitted `inspecto-events` and `inspecto-ops`): `inspecto-security`, `inspecto-policy`, `inspecto-notify-channels`, `inspecto-backup`, `inspecto-geo-link`, `inspecto-exchange`, `inspecto-metrics`, `inspecto-events`, `inspecto-ops` | Professional-only code (OIDC resource-server, TLS, RBAC; the webhook + SMTP delivery transports; the backup/restore maintenance tasks; the geo map + link analysis routes; cross-space sharing; the Prometheus `/metrics` exposition). Personal simply doesn't bundle it. |
| **Maven profiles** (`-Pedition-professional` / `-Pedition-enterprise`) | Which modules + shade includes go into the fat-JAR. `edition-enterprise` = `edition-professional` + `inspecto-policy`. 🔴 **Corrected 2026-09-07: there is no `edition-personal` profile** — the parent POM declares exactly two (`pom.xml:68,77`). **Personal is the DEFAULT**, i.e. the reactor with no profile at all, which is what `package.ps1` does (`if ($Edition -ne 'Personal')` guards every Professional+ step). ⚠ The old text named a third flag; Maven only *warns* on a profile that does not exist and then builds the default, so a shift following this table would have got the right bundle for the wrong reason and learnt nothing from the build. |
...
There is no `-professional` artifact at all and no `-enterprise` classifier — the two names this line used
...
Professional/Enterprise editions re-introduce it out-of-band (see below) behind an `Authenticator`
...
when no implementation is on the classpath). The `inspecto-security` module ships the Professional
implementation (`OidcAuthenticator`, Nimbus JOSE+JWT + JWKS) and is reactor-gated behind the
`edition-professional` Maven profile, so it is never built or resolved by a routine Personal build.
`package.ps1 -Edition Professional` builds and bundles it; see `docs/okf/capabilities/control-api/control-api.md` (design of record; the 2026-07-06 design is archived)
§10 W6 for the full slice and `docs/api/deployment/` for WSO2/Keycloak blueprints.

## Security direction (Professional)
...
a **superset of Professional** (mirroring the profile relation), so it builds `inspecto-security` *and*
...
feature is in every edition by construction, so most Personal/Professional/Enterprise cells agree; the
...
| JOB-01 | Job framework (`enrich`, `report`, `maintenance`, `pipeline`, `sql.template`, `consignment.process`, `objectstore.export`, `recon.run`, `caserule.evaluate`, `objects.analytics`, `mail.send`) | ✅ | ✅ | ✅ | §F. ⚠ **`caserule.evaluate` and `objects.analytics` are `inspecto-ops` Job Types (EDG-01 cell 7) — unknown on Personal**; the framework cell stays ✅ (corrected 2026-09-08, `okf/capabilities/incidents/incidents.md` §2) |
...
| SP-DQ-02 | ⚠️ Constraint & range checker (Expectations) (`quality.constraint.check`) | Data Quality, Validation & Cleansing | 🟡 | 🟡 | 🟡 | `expectation` | Expectations evaluated per Dataset (`ExpectationEvaluator`), authored in the Expectations pane — not a Step: never on the canvas, so no workbench preview / lift / lower |
...
| SP-ENR-09 | 🏷️ LLM zero-shot classifier & tagging (`ml.llm.classify`) | Enrichment, Entity Resolution & AI/ML | 🔲 | 🔲 | 🔲 | — | still unbuilt; the assist agent now ships Professional+ (CP-14, `PKG-5` 2026-09-12) but this step kind does not exist |
...
| SP-CTL-02 | 🕳️ Sequence-gap & data-loss watchdog (`control.gap.detector`) | Control, Governance & Sequence Integrity | 🟡 | ✅ | ✅ | `gap` | same detector; a gap raises the `SEQUENCE_GAP` event in EVERY edition, but promoting it to a managed ALERT object is `EventObjectBridge`, which ships in the optional `inspecto-ops` module — so Personal is partial (EDG-01 cell 7, operator-approved 2026-09-08) |
...
| SP-CTL-07 | 🚨 Alert rule dispatcher (`control.alert.dispatch`) | Control, Governance & Sequence Integrity | — | 🟡 | 🟡 | `alert-rule` | Alert Rules over the ledgers → Alerts → channels (board CP-11/CP-15), authored in the Alert Rules pane — not a Step: never on the canvas, so no workbench preview / lift / lower. Professional+ (G9): `inspecto-ops` declares it through `EditionFeatureProvider`; Personal refuses every authoring door with `ERR_EDITION_FEATURE` and arms no rule copied onto disk |
...
| SP-SNK-03 | 🦆 DuckLake catalog (PostgreSQL) sink (`sink.ducklake`) | Sinks, Storage & Destinations | — | ✅ | ✅ | `sink.persistent` | `output.ducklake` / `sinks[].ducklake` — needs the postgresql sidecar. Professional+ (G9): `inspecto-backup` declares it through `EditionFeatureProvider`; Personal refuses `ducklake.enabled: true` at save (`ERR_EDITION_FEATURE`), at pipeline load and in `DuckLakeRegistrar` |
...
| SP-SNK-05 | ☣️ Quarantine error-log store (`sink.quarantine`) | Sinks, Storage & Destinations | 🟡 | 🟡 | 🟡 | `sink.quarantine` | structural rejects + `errors/<base>_errors.csv` on every pipeline — not a Step you add: a quarantine node is lifted only for selector / segments pipelines, so there is no workbench preview / lift / lower of its own |
| SP-SNK-06 | 📜 Long-term compliance archive (`sink.archive`) | Sinks, Storage & Destinations | — | ✅ | ✅ | `acquisition` | Collector `post_action: MOVE archive_path` + the `backup` maintenance task. Professional+ (G9): `inspecto-backup` declares it through `EditionFeatureProvider`; Personal refuses `post_action.on_success: MOVE` (`ERR_EDITION_FEATURE`) at save, at pipeline load and before a remote acquisition — `RETAIN`/`DELETE`/`RENAME`/`TAG` and the local `dirs.backup` move stay in every edition |
...
| CP-14 | Assist / Intelligence agents (`/assist/*`, `/agent/*`) | — | ✅ | ✅ | ✅ **CHANGED 2026-09-12 — `PKG-5` shipped.** The **assist** agent (`inspecto-agent`) is now staged as `inspecto-agent.jar` in **Professional and Enterprise** bundles and discovered by jar presence like every other optional module; `/assist/*` answers 503 only on Personal. Operator decision: assistant Professional and above, optional. ⛔ The **intelligence** agent (`/agent/*`) and `inspecto-agent-hosted` are NOT part of this and still ship in nothing — do not read this row as covering them. ⚠ Absence is also the outcome on a host whose Java is older than the module needs (§3.6): `OptionalSpi` skips it and the product starts normally. Requirement-of-record is [`okf/capabilities/assistant/assistant.md`](okf/capabilities/assistant/assistant.md) §3.10; `REQUIREMENTS.md` §3.13 said `All` for every `AGT` row until 2026-09-08 |
...
| SEC-05 | Policy authoring UX (matrix/create editor beyond hand-authored TOON) | — | — | ✅ | **SHIPPED 2026-09-25** — save-time guards in the shared `AccessPolicies.validate` grammar (422s + warnings, self-lockout refused), `POST /access/policies/preview` impact matrix, and the Policies tab as an editor. ⚠ Upgrade: an on-disk doc with a stray key or an un-allowlisted claim becomes unreadable ⇒ deny-all. Evaluation stays Enterprise-only (SEC-04). Owed: a browser pass over the editor. As-built: [`okf/backend/editions/auth-security.md`](okf/backend/editions/auth-security.md) §"Policy authoring guards" |
| SEC-06 | Per-tenant space isolation enforced by seeded policies | — | — | ✅ | engages once a `space` claim is mapped |
| SEC-07 | Secrets: `${ENV}` / `${SYS}` references in every edition; `${FILE}` / `${KEYSTORE}` Professional + Enterprise; Vault / cloud KMS Enterprise-only, gated on a client policy (D4) | ✅ ENV/SYS | ✅ +FILE/KEYSTORE | ✅ +Vault/KMS when a client requires it | **SHIPPED 2026-09-06** — `SecretsProvider` SPI in the core (`inspecto-acquire`); `FileKeystoreSecretsProvider` lives in `inspecto-security` (the Professional/Enterprise profile module) and is found by ServiceLoader; a Personal bundle refuses `${FILE}`/`${KEYSTORE}` with a message naming the edition and pointing at `${ENV}`/`${SYS}` — loud in a connection test, never a silent null |
...
| SEC-09 | Actor-attributed, **append-only** audit log (⚠ *not* tamper-evident — no hash chain or signature exists, `compliance/controls-matrix.md` AU-9; this row said "tamper-evident" until 2026-09-08) | 🟡 | ✅ | ✅ | P has no actor (auth-free) — events carry the default actor **`appUser`** (`ApiContext.actor`; corrected 2026-09-08 — no `anonymous` string exists) |
...
| ID | Feature | P | S | E | Notes |
|---|---|---|---|---|---|
| OPS-01 | Operational stores on DuckDB (status, jobs, dedup ledger, outputs, provenance) | ✅ | ✅ | ✅ | ⚠ **AMENDED 2026-09-08 (EDG-01 cell 7, operator-approved): `objects` left this list for Personal.** The object/link/note/tag-assignment stores are opened by the optional `inspecto-ops` module now (`ObjectEngineProvider`), so a Personal bundle has none of the four — the row previously promised Personal a store that CP-11 was gating away, which is the contradiction this cell had to resolve one way or the other. Professional and Enterprise are unchanged. ⚠ Each of the four is still its OWN DuckDB file (a file-based DuckDB holds a single-writer lock) and still degrades to in-memory rather than blocking startup. ✅ **Durable by default since 2026-09-25 (`OBJECTS-BACKEND-DEFAULT-MEMORY-1`, operator decision):** `-Dobjects.backend` defaults to `db` — each Space's `duckdb/` — on Professional, Preview and the `-DemoAuth` build (whose `serve-demo.*` states `-Dobjects.backend=db` explicitly); `memory` is an explicit opt-in only (the test reactor pins it). Enterprise is OPS-02. |
| OPS-02 | Operational stores on PostgreSQL (`-Dinspecto.db=postgres`, shared roster) | — | ✅ | ✅ | driver sidecar Professional+. ⛔ **Enterprise: MANDATORY for the operational objects (Incidents, Cases, notes, links, tags) since 2026-09-25** (`OBJECTS-BACKEND-DEFAULT-MEMORY-1`, operator decision). The Enterprise `serve.*` passes `-Dobjects.backend=postgres`, and `OperationalDb.verifyObjectsBackend()` refuses the boot unless all four families resolve to a `jdbc:postgresql:` URL (`INSPECTO_DB_URL` → `-Dinspecto.db=postgres -Dinspecto.db.url=…`, or a per-family `-Dobjects*.db.url`) with the driver present; a store that then will not open stops its Space instead of degrading to memory. Never a fallback to DuckDB or memory. Professional/Preview keep PostgreSQL optional; a Preview bundle carries Enterprise's jars, so `package.ps1` names it in the launcher text to keep it on DuckDB. The `-DemoAuth` build runs `serve-demo.*` on `db` (a one-folder hand-over has no PostgreSQL). |
| OPS-03 | Multi-user Postgres deployment (shared state, several operators) | — | 🟡 | 🟡 | plan exists (`archived-documents/plans-archive/postgres-multi-user-plan.md`), **PARKED** by BACKLOG §6 2026-09-06 until a multi-operator install exists |
...
| OPS-06 | Backup / restore (zip + sidecar manifest, hash-verified) | — | ✅ | ✅ | ✅ **GATED 2026-09-07 (EDG-01 cell 2) — the P cell is now true of the build.** `BackupTask` moved out of `inspecto-engine` into `inspecto-backup`, a Professional+/Enterprise-only module that contributes `backup`/`backup_verify`/`restore` through the new `MaintenanceTaskProvider` ServiceLoader seam on `MaintenanceJob`. On Personal the three are **unknown maintenance tasks, refused loudly** (`NoBackupTaskShipsInThePersonalBuildTest` pins it in the DEFAULT build) — never a silent SKIPPED, which would read as a successful no-op to a chained job. ⚠ **Consequence for the bundled `spaces/demo`:** its nightly chain `runlog_retention → db_maintenance → config_backup → backup_verify → maintenance_report` stops at `config_backup` on a Personal install with a FAILED run whose message names the cause; the chain's halt-on-failure guard does the rest. That is the edition boundary working, not a defect. 🔴 **`backup` covers files only, never the operational database** (`DB-BACKUP-POSTGRES-1`, filed 2026-09-15, closed doc-only 2026-09-17): its one DB reference (`BackupTask.catalogRow`) opens a throwaway in-memory DuckDB scratch just to write its own catalog Parquet row — it never reads the operational stores themselves. On a Professional/Enterprise deployment pointed at Postgres (`OPS-02`, `-Dinspecto.db=postgres`), **job runs, the dedup and acquisition ledgers, status, and the run lease have no backup path from this product at all.** A real leg would mean a JDBC `COPY`/dump per `OperationalDb.Family` — each with its own URL and credential grain (see `OperationalDb.java`'s roster) — which is infrastructure-plumbing scope disproportionate to a maintenance task; no `pg_dump`-shelling precedent exists in this codebase (the runtime is deliberately JDBC-driver-free, §OPS-02) and adding one would cut against that. **Operator must arrange an external DBA backup (`pg_dump`/WAL archiving/managed-service snapshot) for the Postgres operational database** — this product's `backup` task does not, and is not planned to, cover it. |
| OPS-07 | Embedded trimmed JVM runtime in the bundle (jlink) | 🟡 | 🟡 | 🟡 | 🟡 **BLOCKER FIXED 2026-09-14; turns ✅ on the first tagged release.** ⚠ Deliberately NOT green yet: no published artifact contains a runtime until a `v*` tag runs `release.yml`, and a cell green over an artifact that does not exist is exactly `SPEC-GREENCELL-1`. What changed is that the packaging step can now run at all — `release.yml` passed `-NoRuntime` on all three steps, so the old ✅✅✅ described a capability of the script and the 🟡 that replaced it described a bundle with no runtime. 🔴 **Its recorded cause was wrong on both counts.** The blocker was never "the runner has no GraalVM jmods cache": `package.ps1` hardcoded **`jlink.exe`** in all three probes (plus a Windows path separator), which cannot exist on the `ubuntu-latest` runner, so the step could only ever throw. Measured inside `eclipse-temurin:25-jdk` — exactly what `setup-java` installs there — `jlink` lives at `$JAVA_HOME/bin/jlink`, the image ships **zero** jmods, and jlink still links a working ~64 MB runtime **from the run-time image itself** (JEP 493). ⚠ So a jmods cache was not what was missing; ⛛ do not provision one. Both launchers are conditional (`serve.sh` takes `runtime/bin/java` only when executable, `serve.bat` only when the Windows `java.exe` is present under `runtime/bin`), so a Linux-built runtime serves Linux targets while Windows targets fall back to system java as before. ✅ **Verified end-to-end 2026-09-14** by a real `package.ps1 -Edition Personal` run: jlink ran, `runtime/bin/java.exe` reports 25.0.3, and the staged bundle passed its own boot smoke. ⚠ **Cost, measured:** the embedded runtime is ~68 MB on disk and the Personal `inspecto-deploy.zip` is **205 MB** with it — operators who already supply Java can still pass `-NoRuntime`. Owner: [`okf/capabilities/editions/editions.md`](okf/capabilities/editions/editions.md) §2 |
...
| CMP-05 | Control matrix + auditor evidence (`compliance/`) | — | ✅ | ✅ | P has no compliance scope by definition — ⚠ **that is a product stance, not a build fact** (clarified 2026-09-08): the matrix and the seven evidence files are plain repo files, so nothing withholds them from a Personal deployment; the gating is about who is *promised* the posture. Requirement-of-record: [`okf/capabilities/compliance/compliance.md`](okf/capabilities/compliance/compliance.md) |
