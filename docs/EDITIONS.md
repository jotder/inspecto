...
| Mechanism | Used for |
|---|---|
| **Separate Maven module** — **all nine** (corrected 2026-09-09; this list omitted `inspecto-events` and `inspecto-ops`): `inspecto-security`, `inspecto-policy`, `inspecto-notify-channels`, `inspecto-backup`, `inspecto-geo-link`, `inspecto-exchange`, `inspecto-metrics`, `inspecto-events`, `inspecto-ops` | Standard-only code (OIDC resource-server, TLS, RBAC; the webhook + SMTP delivery transports; the backup/restore maintenance tasks; the geo map + link analysis routes; cross-space sharing; the Prometheus `/metrics` exposition). Personal simply doesn't bundle it. |
...
| SP-ENR-09 | 🏷️ LLM zero-shot classifier & tagging (`ml.llm.classify`) | Enrichment, Entity Resolution & AI/ML | 🔲 | 🔲 | 🔲 | — | still unbuilt; the assist agent now ships Standard+ (CP-14, `PKG-5` 2026-09-12) but this step kind does not exist |
...
| CP-14 | Assist / Intelligence agents (`/assist/*`, `/agent/*`) | — | ✅ | ✅ | ✅ **CHANGED 2026-09-12 — `PKG-5` shipped.** The **assist** agent (`inspecto-agent`) is now staged as `inspecto-agent.jar` in **Standard and Enterprise** bundles and discovered by jar presence like every other optional module; `/assist/*` answers 503 only on Personal. Operator decision: assistant Standard and above, optional. ⛔ The **intelligence** agent (`/agent/*`) and `inspecto-agent-hosted` are NOT part of this and still ship in nothing — do not read this row as covering them. ⚠ Absence is also the outcome on a host whose Java is older than the module needs (§3.6): `OptionalSpi` skips it and the product starts normally. Requirement-of-record is [`okf/capabilities/assistant/assistant.md`](okf/capabilities/assistant/assistant.md) §3.10; `REQUIREMENTS.md` §3.13 said `All` for every `AGT` row until 2026-09-08 |
