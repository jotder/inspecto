...
| **C12** | `design_analysis.md` (file:// links to `c:/sandbox/ucc-inspecto/...`) | actual repo `C:\sandbox\inspecto` | Repo path in links | **Low** | `ucc-inspecto` paths are wrong/broken — a transitional name that never matched the repo. |
| **C13** | `superpowers/2026-05-27-*` (repo root `C:\sandbox\URA\sandbox`; product path `…\file-processor`) | actual repo `C:\sandbox\inspecto` | Repo root path | **Low** | Historical path; superseded. |
