...
>
> ⚠ **Only the 0<!--count:backlog-p1--> P1 + 28<!--count:backlog-p2--> P2 rows are queued work.** §0 defines **P3 as demand-gated — "build only when
> someone asks by name"** — so those 29<!--count:backlog-p3--> are mostly a list of things deliberately *not* being built, not a
> backlog to burn down. Reading all 57<!--count:backlog-rows--> as pending work overstates what is owed by roughly half.
...
- ~~**P3** · **`ENRICHMENT-MIDWALK-LANES-DISAGREE-1`**~~ ✅ **SHIPPED 2026-09-24 — refused at save, per the operator's 2026-09-24 decision.** `PipelineEditable.lower` refuses an enrichment node with an edge to another node: 422 `refusals[{code: ENRICHMENT_NOT_TERMINAL}]`, pinned by `ControlApiEnrichmentMidWalkRefusalTest`. A terminal enrichment and the companion edge still save. *(Original:)* **an enrichment authored mid-walk runs differently in the two lanes.** `PipelineExecutor.execute` skips it (the graph lane starves downstream sinks) while `PipelineEditable.lower` drops the node (the flat lane feeds them). The save gate should refuse a mid-walk enrichment, or one lane should change. → `okf/backend/engine/pipeline-test-run.md`.
