...
| 1 | ✅ **DONE 2026-09-10** — first cut 2026-09-08 (`7d2a3c27`) — `REQUIREMENTS.md` §8's four archive rows now point at current homes with a provenance line; `review-coverage.md`'s `resource:` names the reactor, not the archive; **all 5 bookkeeping defects fixed** (INDEX 46→54 rows incl. the 2 missing; `legacy-surface-removal` header; `4x-public-pkce` note; `metadata-network-design` links). ✅ **CLOSED 2026-09-10 (Sprint 7.1).** 🔴 **The remaining-list was wrong in three ways.** (a) `GLOSSARY.md`'s "four rationales" were **eight** sites — the header's two companion docs, plus the persona header, the Grammar template rule (a bare `See <archive>` that was the rule's only pointer), `Load`, `Job`'s authoring contract (stated twice, §6-A and §13), §11's header, GraphSource's `Design:` and the Geo header. (b) `completeness-kpi-plan.md:21` was **already resolved on 2026-09-09** (§8's own banner records it) and `observability.md:471` already names §3.9 as the design of record — this row was stale against its own plan. (c) The okf sweep found **six more** "design of record" delegations the list never named: `exchange-sharing.md` (the *heading* was `## Design-of-record` over an archive path), `data-plane.md` DAT-5, `jobs.md`'s parameter contract, `security.md` SEC-7 (two of them), `component-registry.md`, `tree-table.md`'s *as-built* design, plus `rule.md`'s unbuilt north star (now a `BACKLOG.md` pointer) and `assistant.md`, which labelled an archived plan **live**. 🔴 **A FIFTH instance of a guard's scope being a silent exemption: five SOURCE files.** The citation guard reads markdown only, so a javadoc could delegate freely — and three of them (`NodeAttribute`, `NodeAttributes`, `NodeConfigNameContractTest`) pointed "current knowledge" at `okf/frontend/features/pipelines.md`, which contains **zero** mentions of the node config vocabulary, making the archived plan the only authority a reader could actually reach; `grammar-editor.dialog.ts` had a bare `See <archive>`. Repointed to `pipeline-config-keys.md` + `pipeline-editor.md`. Found on the way and fixed: `security.md` SEC-7 cited **§3.13** for `permissions[]` when its own §3.14 owns it (the same file cites §3.14 correctly 630 lines later), and `GLOSSARY.md`'s header sent the reader to "§6" for the rename map, which is **§13**. **Total: 19 doc sites + 5 source sites** | no current-tier doc cites an archive file *as its authority*; link guard green |
| 1b | ✅ **DONE 2026-09-10 (Sprint 7.2) — all 7.** The two ACQ ORPHANs landed in the pilot (`acquire-controller-service-design` → §5; `brainstorm-tingly-storm` → §6.2); the remaining five closed this shift: `modularization-optimization` C2/C4/C6 → **`editions.md` §6.1** · `snazzy-painting-platypus` → **`surfaces.md` §3.3** (not §6 — the residue was *as-built*, not refused) · `system-maintenance-plan` COULD list → **`observability.md` §5.1**, extending the existing row rather than adding a parallel one · `frontend-review-and-completion-plan` → **the `angular-ui` skill** · `claude-usage-audit` → **none, meta, confirmed**. 🔴 **Grounding refuted or corrected an item in FOUR of the five.** `C2` is half shipped (`AbstractJdbcStore`, 2026-08-18 — 20 days before the recount that called it untouched) and the pair count is 5, not 4, so landing the board row verbatim would have imported a false "nothing built" into the current tier. The maintenance list's first COULD (**growth-trend + archive recommendations**) had SHIPPED as `storage_trend`, which the destination file already documented **twice** — landing it as deferred would have made one file contradict itself; its "Dev/Prod profile presets" is refuted as framed (no Dev/Prod tiering exists in `spaces/`); and its agent-session blocker **is** still live (a `ConcurrentHashMap`, no durable ring), unlike the agt-6 precedent. The console genesis plan was **~80 % already absorbed**, leaving two real facts: static files are public **by call order, not by the public-path allowlist** (the static branch runs before authentication), and CORS exists for a dev topology the dev-server proxy made unnecessary. The frontend review's **boundary lint was never built** — no ESLint config is tracked at all — and four of its six "pure" libraries import `@angular/*`, so its purity table could not be copied; its R1–R8 protocol is recorded as **retired** (its R6 step was mock parity). Fixed on the way: the skill said the a11y gate runs in CI via `test:ci` when CI runs `test:coverage` | each ORPHAN named in the audit has a destination; BACKLOG §6 stops citing an archive file as the sole home of C2/C4/C6 |
...
> ### 🔴 RE-GROUNDED 2026-09-09 — the register is itself partly stale, in BOTH directions
>
> Step 6 was scoped as "reconcile the 15 clusters". Measured before executing, **most of §8.1 and §8.2 were
> already reconciled by later shifts**, and one register entry is **wrong where the board is right**. A
> checklist of contradictions is not exempt from going stale — re-measure it before working it.
>
> | Cluster | Measured state 2026-09-09 |
> |---|---|
> | §8.1 — the five `GLOSSARY.md` §13 rows marked NOT STARTED, called *"the worst one, in the BINDING authority"* | ✅ **All five reconciled.** Each now carries a dated verdict (Node→Step DONE, `mapping:` PARTIAL, `dedup` DONE end-to-end, `materialize` DROPPED done-by-absence, Enrichment NARROWED-to-nothing). "NOT STARTED" survives in §13 only inside those rows, quoted as the history it is. |
> | §8.2 — four deployment-topology items shipped under other IDs, three inherited by `BACKLOG.md` §3 | ✅ **Board already re-grounded 2026-09-08.** `SCR-11`/`SCR-8` are gone; `GAP-2` and `GAP-8` were dropped from the row as "shipped work this row had inherited". |
> | §8.2 — `GAP-4`, the DuckDB memory cap, *"on by default, shipped as D11"* | 🔴 **The REGISTER is wrong and the board is right.** `okf/backend/engine/duckdb.md` states the DuckDB caps are **all opt-in**, and the board's open item is precisely *choosing a default* — which remains open. A plan's claim about the board is a hypothesis too. |
> | §8.2 — compliance `G1`/`G3`/`G5` called open | ✅ **Reconciled.** `compliance/controls-matrix.md` closed all three on 2026-09-02 as `COMPLY-1/2/3`, and `BACKLOG.md` already narrows the over-broad "org-gated" claim to `N3`/`N7`. |
> | §8.3 — `STAKEHOLDER_OVERVIEW.md` §9/§10.3, *"the document an executive reads"* | ✅ **FIXED 2026-09-09 — it was still broken.** Eleven corrections: three §9 rows marked **Planned** for work shipped 2026-07-07/07-08/07-24, a §10.3 heading promising "not yet started" over four delivered items, and §10.4 listing shipped push discovery as Later. Each verdict re-checked against the capability spec that owns the row. |
> | §8.3 — `ROADMAP` `L4` push discovery listed LATER | ✅ **FIXED 2026-09-09.** Shipped 2026-07-08 as `ACQ-6`; the row's own stated trigger had already been met. |
> | §8.3 — `N4` edition realignment "uncommitted/ungated" | ✅ **FIXED 2026-09-09**, with the distinction the old cell flattened: `EDG-01` is **built and gated** (all seven cells by 2026-09-08, real Maven profiles) and only the **release** half is open — which is true of the entire product, since the newest master-ancestor tag is `v3.11.0`. |
> | §8.3 — `ACQ-4` NFS/SMB "has four different answers" | ✅ **Down to one.** The `ACQ` spec says PARTIAL with the share half **REFUSED**; `BACKLOG.md` no longer mentions it at all; `STAKEHOLDER` and `ROADMAP` were corrected today — including the summary block that read "still open", because ⛔ **a refusal framed as a delay re-opens it every planning round.** |
> | §8.3 — `ROADMAP` §3.5 commits to the `PATH-2` standing refusal; priority inversion vs `BACKLOG` | ✅ **Re-derived 2026-09-10 — the register was wrong in both halves.** The jail was never a `PATH-2` refusal (that row refuses the static-serving jail and the store `fileFor` helpers); it was unbuilt and unfiled, and is now shipped (`ConfigSafetyValidator`, `duckdbTempDirectoryOutsideRootIsRejected`). The priority inversion was real and is corrected in ROADMAP §3/§6 and STAKEHOLDER §9/§10.6 — the board's §0 order is the one statement of the queue. |
