...
> 🔴 **ARCHIVED 2026-09-23 — SHIPPED.** Every `WB-nn` on this register is terminal: **18 shipped**, `WB-15` ⛔ **REFUSED** (decision `D3` reversed — its premise did not survive being counted), `WB-17` ⛔ **DEFERRED** with its residue tracked on the board as `DEMO-CORPUS-FORMAT-COVERAGE-1`. All eight owed decisions (`D2`–`D9`) were signed 2026-09-22 and written into their OKF homes the day they were signed.
>
> ✅ **Verified against a LIVE control plane 2026-09-23** (§6b), not inferred from tests: 26/26 shipped Pipelines lift and validate with zero ERROR findings and zero `csv_settings` warnings, and both segment-routed frontends test past the decoder.
>
> ⚠ **This page is history now — never cite it as current.** The durable facts live in `okf/backend/engine/pipeline-test-run.md` (the segment walk), `okf/backend/pipeline-graph/pipeline-config-keys.md` (one arming predicate), `okf/backend/engine/consignment-status-flow.md` (rejected files vs rows), `okf/backend/pipeline-graph/editable-round-trip.md` (verbatim = the decoded map), `okf/backend/build-run/guard-coverage.md` (why the round-trip guard goes through HTTP), and `okf/capabilities/pipeline-authoring/pipeline-authoring.md` (the reversed `D3`). Open work is on `BACKLOG.md`.
type: Plan
...
| **WB-15** | **Created Pipelines land in `config/<name>/`** | ⛔ **REFUSED 2026-09-23 (D3 reversed); its read-path fix SHIPPED** | M | **D3** ✅ | Server-side in the `configApi.write('pipeline', …)` handler (the client sends no path — `pipeline-scaffold.ts`, `pipeline-editor.component.ts:1214-1219`): choose `config/<id>/<id>_pipeline.toon`; audit every SATELLITE-WRITE-1 call site (`pipeline-config-definition.component.ts:311`, `schema-editor.dialog.ts:42`, `grammar-editor.dialog.ts:197`) to pass the same subdir. Guard: a test that creates through the route and asserts the path. Row: `UI-CREATED-PIPELINE-FLAT-HOME-1`. 🔴 **SHIPPED AND REVERTED THE SAME DAY (2026-09-22) — read this before re-attempting it.** The redirect was written server-side in `ConfigWriteRoutes`, bound late (only when the target did not already exist, so an EDIT could never be relocated into a shadow config). That half was right. What it missed: **a freshly-written config becomes UNREACHABLE.** An unregistered config is resolved at the write ROOT, and only a REGISTERED pipeline is found wherever its file actually sits (`resolveRegisteredConfigFile`) — so nesting the file left `/config/read`, `/config/patch` and `/config/delete` answering **404** for it. **23 tests** across `ControlApiConfig{Write,Patch,Delete,IfMatch}Test`, `ControlApiDeadPropertyTest` and `ControlApiDedupWindowArmingTest` caught it. ✅ **The READ half LANDED 2026-09-23**: `ConfigFileSupport.resolveConfigFile` now falls back to `<dir>/<name>/<name>_<type>.toon`, so a config in its own subdirectory is reachable to read, patch and delete **without being registered** — previously only `resolveRegisteredConfigFile` could find one, by asking the running service, which an unregistered draft has nobody to ask. Flat forms still win, so nothing relocates. Falsification-probed (remove the fallback → the new test goes red). ⛔ **THE WRITE HALF IS REFUSED, and `D3` with it (2026-09-23).** 🔴 **The premise did not survive being measured.** The row and the decision both said *“every shipped Pipeline lives in a per-Pipeline directory”*. Counted: `spaces/demo/config/orders/` holds **three** Pipelines (`orders`, `orders_by_region_feed`, `orders_enriched_rollup`), and the directory names are DOMAIN words, not ids — `config/postmed/` holds `postmed_xdr`, `config/mule/` holds `mule_transfers`. The shipped convention is **a per-domain directory an author chose**, not `config/<id>/`. So the redirect would have introduced a THIRD layout beside flat and per-domain — the exact opposite of this row's own goal (*“pick one layout; two is not defensible”*). ✅ **What the row actually cared about is already true:** a created Pipeline and its satellites land TOGETHER, and an author who wants a domain home passes `subdir:`, which works today. ⚠ **And the cost was real:** a Pipeline's bare `schema_file: <name>.toon` resolves BESIDE ITS OWN CONFIG, so nesting the Pipeline while its schema is written flat separates the two — the very coupling the change was supposed to protect. *(Kept for provenance — the second attempt's finding:)* with the read fix in place the redirect drops from 23 failures to 15, and the survivor is the one that matters — **a Pipeline's bare `schema_file: <name>.toon` resolves BESIDE ITS OWN CONFIG**, so nesting the Pipeline while its schema is still written flat separates the two and the save warns that its own schema does not resolve. ⚠ `D3`'s own text says *“satellites get the subdir too”*; **no row has scoped that**, and a `/config/write` for a schema with no `subdir:` still lands flat. Land the satellite half first. 🔴 **And the process lesson: this was verified with a targeted `-Dtest=ControlApiPipelineCrudTest` (20/20 green) and pushed — a change to a SHARED write path needs the module suite, which is the same green-alone/red-in-suite trap `WB-01` had already paid for once this shift.** The do-not-relocate guard is kept for the next attempt. |
| **WB-17** | **Samples for the sample-less, demos for the synthetic-only** | ⛔ **DEFERRED 2026-09-23 — partially done; the residue is a BOARD row** | M | `WB-16` | `lookup_step` gets a sample file; one domain-shaped demo each for ASN.1, Excel, fixed-width, XML and `route` in `spaces/demo`. Could-tier: value is in coverage, not features. Row: `DEMO-CORPUS-FORMAT-COVERAGE-1`. ✅ **`route` HAS ITS DOMAIN DEMO 2026-09-23** — `premed_events` (`spaces/demo/config/premed/`): a pre-mediation event feed routed by record type into voice/sms/other, `active: false`, 12 synthetic events covering all three branches plus a lower-case type that exercises the mapping's `UPPER` normalisation. It lifts, validates clean, and routes 5:3:4 as designed. 🔴 **And it earned its keep immediately: it exposed `ROUTED-WRITE-COUNTS-PER-BRANCH-1`** — a routed Pipeline reports each row once per branch (`route_step`, SHIPPED: 9 rows in, 18 written; `premed_events`: 12 in, 36 written), which the 26-Pipeline sweep could not see because it read row counts with no independent expectation to compare them against. That is what a hand-authored corpus buys. ✅ **HALF SHIPPED 2026-09-22 — `lookup_step` has a sample**, covering all three of its `mappings` plus an UNMAPPED status, so the one Pipeline that could not be test-run as shipped now can. ⚠ The five domain-shaped demos remain (Could-tier). 🔴 Building it also corrected `tools/seed-samples.mjs`, whose “cannot be test-run” warning inferred emptiness from a NAMING convention and so was wrong in both directions: it flagged a Dataset-fed Pipeline that has no inbox by design, and one that SHARES the `orders` inbox. It now warns when a FILE-FED Pipeline's inbox is actually empty — falsification-probed by hiding a sample dir and watching it name that Pipeline. |
...
| **D3** 🔴 | Where does a UI-created Pipeline live? | 🔴 **REVERSED 2026-09-23 — creation stays FLAT; `config/<id>/` is REFUSED.** ⛔ Its premise was false, measured: a created Pipeline and its satellites land together at the space config root, which is what actually matters, and an author who wants a domain home passes `subdir:` — which already works and is evidently what every shipped Pipeline's author did. *(Signed 2026-09-22 as `config/<id>/`, server-side, on the premise that)* every shipped Pipeline uses it; *"one identity for id, file and directories"* (`pipeline-authoring.md:405`) points the same way; satellites get the subdir too. Written into `okf/capabilities/pipeline-authoring/pipeline-authoring.md` §4 and `okf/frontend/features/pipeline-editor.md` the same day. | E4, E3 | flattening 26 Pipelines | WB-15 |
