...
**Status: COMPLETE — ALL PHASES SHIPPED.** P0 + P1 + P4 on 2026-07-24; P2 + P3 on 2026-07-25. Every
phase landed GAUNTLET-green (full offline reactor). **Durable as-built home is now
[`docs/okf/backend/control-plane/onboarding-authoring.md`](../okf/backend/control-plane/onboarding-authoring.md)**
(sections "Reference Phase-2 …"); this plan is retained for provenance only — see `docs/BACKLOG.md` for
the two deliberate deferrals that outlived it (D5 delete-tombstone input convention, D6 dedup tie-break).
...
| **P3 ✅** | Compaction task (MaterializeTask idiom) + `refresh_seconds` timer + cache read-path switch | ✅ SHIPPED 2026-07-25 — `ReferenceCompactor` + a `reference_compact` case in `MaintenanceJob` (a `maintenance` sub-task, the house pattern — not a new Job Type), reusing `PartitionCompactor`'s journal/hide/`ATOMIC_MOVE` safety model; winner derived **store-wide** then each partition dir rewritten to its slice (a key whose partition value changed has versions in different dirs, so per-dir "latest here" would resurrect one); `history_days` = `0` winners-only / `>0` horizon / **negative keep-forever** (D4). Timer: `CollectorService.armReferenceRefresh` (cancel-and-rearm per pipeline id, armed in `start()`+`registerPipeline`, cancelled in `unregisterPipeline`). No age cutoff needed — a concurrent commit only creates a *new* file, outside the candidate set. `ReferenceCompactorTest` (7) + `CollectorServicePipelineForgetTest.unregisterCancelsTheReferenceRefreshTimer` green; full reactor green. **No cache read-path switch was needed** — compaction rewrites the store in place, so `versionedView` reads the compacted files through the unchanged glob. |
...
| D4 | History retention default for `scd2` (keep-forever vs horizon) | **Resolved & shipped (P3): keep-forever.** `history_days` took a third mode — negative = keep every version, merge files only — so keep-forever is still worth compacting (read amplification is file count, not row count). `scd2` timers pass `-1`; `-Dreference.compact.history.days` sets a horizon instead. `upsert` passes `0` (winners only, tombstones dropped). |
