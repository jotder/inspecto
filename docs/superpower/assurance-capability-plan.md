...
| D-P11 | Wave 1: may a machine heal resolve a per-key Incident? | 1.1, 1.3 | ✅ **DECIDED 2026-09-26: never.** The Alert resolves; the Incident stays open until a person records a Disposition. A relapse re-opens a human-resolved Incident. |
| D-P12 | Wave 1: may impact change on a terminal object (late recoveries)? | 1.3 | ✅ **DECIDED 2026-09-26:** on a RESOLVED Incident or CLOSED Case only `recovered` and `prevented` may change, audited; ARCHIVED refuses everything. |
| D-P13 | Wave 2.1: who applies an approved Pending Change? | 2.1 | ✅ **DECIDED 2026-09-26 (operator):** the **author**, whose write capability is re-checked at apply time; the approver needs only the approver capability (a checker need not be a builder). The audit records both. |
| D-P14 | Wave 2.1: which files may an import never write? | 2.1 | ✅ **DECIDED 2026-09-26 (operator):** `approval.toon`, `pending-changes/**`, `roles.toon` and every settings document are refused from every import, whatever the policy. |
