...
1. **Dedup fields** (`duplicate__*`): shown by onboarding, deliberately absent from the acquisition node (owned by `transform.dedup.fingerprint`, D9). → **Follow pipeline**: remove from the collection pane. *Flagged consequence: onboarding then has no dedup surface — engine default applies until configured in the Pipelines editor. Do not invent a new stage.* The D9 header note in `node-attributes.ts:80-92` ("the fix is NOT to prune… would break Onboarding") must be rewritten in the same slice — its premise changes.
