...
| **S5** | ✅ **BACKEND SHIPPED 2026-08-30** — `GET /config/schema/derived?pipeline=…` in `ConfigPreviewRoutes`, deriving every declared schema's sink shape by DESCRIBE without executing. 🔴 **`typedSource` is derived from the pipeline's own config (`ingesterClass != null`) and REPORTED in the response** (`sourcePath`/`typedSource`/`ingesterClass`) — §7's trap was a route that guesses it and shows authoritative-looking wrong types for ASN.1/fixed-width. The parser refuses a plugin ingester without a non-empty `segments` map, so the predicate cannot disagree with the engine. ✅ **UI HALF SHIPPED 2026-08-30** — `<inspecto-derived-schema-panel>` (`inspecto/schema/`), read-only, mounted in the `transform.map` definition pane beside the mapping that authors those columns. 🔴 It **always states which source path produced the types** (`CSV path — raw columns read as VARCHAR` / `Plugin path — declared field types`): showing derived types without saying which assumption they rest on would re-create the very trap the route was built to close. A **422 renders as an actionable warning carrying DuckDB's binder message**, never as an empty panel — it is the most useful thing the route says. S5 is now COMPLETE | ✅ `ControlApiDerivedSchemaTest` (real HTTP: 400 · 403 traversal · 404 · 422 non-binding · happy path · reports-which-path) + `TypeFlowTest.typedSourceIsWhatSeparatesThePluginPathFromCsv`. **11/0/0/0, BUILD SUCCESS.** ⚠ The footer-parity gate stays where it already was (`TypeFlowTest.derivedSinkSchemaMatchesTheWrittenParquetFooter`, asserted against a written file) — note the derived SINK shape includes the partition columns the footer omits (Hive encodes those as directories); both are correct answers to different questions |
