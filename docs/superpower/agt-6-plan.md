...
| **D9** *(A5)* | **Does A5.2 build the `ConfigSpec` → JSON Schema projection (§3.4 F3), or do we accept prose-guided nested payloads plus the repair loop?** This is the phase's effort fork: the projection is the difference between "M" and "L". | **Build it** — and ✅ **BUILT 2026-07-27, ahead of the rest of A5.** `ConfigJsonSchema` (`inspecto-config`, `com.gamma.config.spec`) projects any `ConfigSpecs.forType(kind)`; the new **`config_schema`** L1 read tool (non-mutating, `READ_DOCS`) exposes it, so A5's derive hop has its constraint source ready. Coverage is complete — all 8 draftable kinds resolve to a real spec. ⚠ **Not** wired as a tighter `component_draft` input schema, deliberately: that tool is a *validator* and must be able to receive a malformed draft in order to return findings on it, so the schema travels as fetchable data. ⚠ A required nested path marks its **whole ancestor chain** required (`ConfigLoader.validate` treats required as absolute); `additionalProperties` is never `false` (a spec enumerates what it validates, not every accepted key). ⚠ **Still bare and still A5's problem:** `query_author.when` and `pipeline_author.flow` are not `ConfigSpec`-shaped, so they need hand-written schemas in A5.1/A5.3 — D9 does not cover them. So A5.2's "M vs L" fork is resolved toward M for the *projection* itself; the bounded repair loop (D11) is untouched. → `okf/frontend/features/inline-ai-authoring.md` |
