...
> | §8.3 — `ROADMAP` §3.5 commits to the `PATH-2` standing refusal; priority inversion vs `BACKLOG` | ⚠ **NOT verified this shift.** A probe for the temp-directory jail wording found nothing at §3.5, so either it moved or the entry is imprecise. Re-derive both from source before acting — do not trust this register's wording for them. |
