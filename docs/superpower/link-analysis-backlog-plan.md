...
| Id | Item | State | Size | Blocked on | Detail |
|---|---|---|---|---|---|
| **LA-17** | Entity model + resolution + reference lists | ⬜ **UN-DEFERRED 2026-09-24 (operator)** — reverses D-S4's "stay value-projected"; design first — 🟡 **IN FLIGHT 2026-09-26** — design approved (D-M1..D-M7) in [`link-analysis-entity-model-design.md`](link-analysis-entity-model-design.md); Entity Types, the Identity Fact log and the Entity List routes shipped (steps 1–3), `excludeBy`/`seedBy` next | L | — (D-E8 decided) | Typed Entities, cross-identifier resolution, enrichment attributes; named reference lists that persist across Enquiries. |
...
| **LA17-NORMALISER-CHANGE-1** | Changing an Entity Type's normaliser strands existing Entity List members | ⬜ open (found in review 2026-09-26) | S | LA-17 | Members are stored under the normaliser in force when added; after a change, `remove` (normalised by the new rule) cannot match them. Options: refuse a normaliser change while lists of that type exist (422), or record the normaliser on `list.created` and keep using it. Design §4.3.2. |
