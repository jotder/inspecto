...
**Raised by LA-19 (2026-09-24) — owed, NOT answered; the implementer shipped only annotation and day coverage:**
...
✅ **ANSWERED 2026-09-24 (operator) — all five:**

* **D-U7 → pending request OUTSIDE the log + approver.** An `expand` above a sensitivity threshold becomes a
  pending request in a rewritable per-Investigation record; a holder of a NEW capability may see that request
  (not the Investigation) and approve it; only the approved op enters the log, carrying `requestedBy` /
  `approvedBy` (approver ≠ requester). Professional and up. `/inv/projection/neighbors` and
  `/inv/traversal/recursive-paths` are gated as part of it.
