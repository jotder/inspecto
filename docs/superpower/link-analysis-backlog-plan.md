...
| Id | Item | State | Size | Blocked on | Detail |
|---|---|---|---|---|---|
| **LA-03** | `POST /inv/snapshots` + `POST /cases/{id}/evidence/graph` | ⬜ backend · ✅ SPA mock store — **UNBLOCKED 2026-09-22** | M → **S–M** (content model already ~80 % built; mostly a persistence swap) | — (D-S1, D-E2, D-E3 answered) | Serialise sub-graph, scores, positions, viewport, annotations, predicate, origin, pinned Dataset version; persist as an Artifact anchored to `opSeq`; swap `LinkAnalysisSnapshotsService.add/attach`. Contract §5.4. |
