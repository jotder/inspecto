...
| R3 | the acquisition ledger backend is `-Dacquire.ledger.backend` in **`AcquisitionLedgers.java:152`**, **"not an `OperationalDb.Family` member at all"** | 🔴 **The correction is itself wrong.** `OperationalDb.java:147` **does** declare `ACQUISITION_LEDGER("Acquisition ledger", "acquire.ledger.backend", "memory", Mode.DB_FLAG, …)`. Both statements are true at once: the `Family` entry exists **and** `AcquisitionLedgers` resolves the property independently with its own `System.getProperty` rather than through `OperationalDb.resolve()` — a **duplicated definition of one toggle**. 🔴 **AND THAT FINDING WAS ITSELF REFUTED 2026-09-16 — the third wrong framing in a row.** Reading it as a defect is wrong: `resolve()`/`resolveAll()` have exactly ONE caller repo-wide (the diagnostic `OperationalDbReport.java:47`), **no store opener calls them**, and `ServiceStores.java:62` says outright *“Mirrors OperationalDb.resolve”* — so all **FIFTEEN** families read their own toggle by `System.getProperty` and the ledger is the house idiom, not an outlier (⚠ this said ELEVEN until 2026-09-16 — `Family.values()` is fifteen; the as-built now lives in [`okf/backend/engine/db-layer.md`](../okf/backend/engine/db-layer.md) §5.0-b). The de-duplication is also **impossible as stated**: `inspecto-acquire` is a leaf and `inspecto/pom.xml:96` depends ON it (module cycle), and `resolve()` returns the package-private `record Resolved` (`OperationalDb.java:209`). ⚠ For K4's purposes nothing changes — `StoreHealth.of(spaceId)` remains the seam, for the reason §R3 already gives. Also, `shared()` is at `:39`; `:152` is inside the `build(url, spaceId)` helper it delegates to |
