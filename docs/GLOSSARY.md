...
**Load** *(added 2026-08-13 — operator decision, `superpower/definition-surface-unification-plan.md` §4b)* —
...
| Sync *(definition stage C)* / wizard "Schema & Mapping" + "Dataset & Go-live" panes | **Load** *(stage/drawer)* | **NOT STARTED — definition-surface unification P4/P6** (`superpower/definition-surface-unification-plan.md` §4b-C). "Sync" was a working name only, never shipped — banned before first use. Touchpoints when the drawer lands: checklist chip label, drawer title, `<inspecto-load-pane>` (plan's `<inspecto-schema-pane>`/`<inspecto-output-pane>` merge), plan docs. Go-live/activation is NOT part of Load (pipeline-level toolbar action). |
