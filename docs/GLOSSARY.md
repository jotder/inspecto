...
**Investigation** — The Link Analysis **enquiry object**: an ordered, replayable log of analyst ops (`seed`, `expand`, `exclude`, `hide`, `keep`) over a graph, plus the Working Set it evaluates to. ⛔ It is a **child of a Case**, never a Case itself (a Case groups Incidents; an Investigation is one line of enquiry within one). Its Type half is an **Investigation Template**. Named by operator decision `D-E1` (2026-09-22): the code already implied it — the routes are `/inv/*`, the capability is `INV-1`, and `InvRoutes` spells it out. ⚠ **Not yet built** — `LA-10` in [`link-analysis-backlog-plan.md`](superpower/link-analysis-backlog-plan.md); the word is reserved here so nothing else takes it. ⛔ Do not confuse it with the Assistant's **Triage Run** (§9), which took this word in the pre-2026-09-22 plan and gave it up under `GLOSSARY-CASE-1`.
