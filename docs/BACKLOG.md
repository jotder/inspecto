...
**Accurate: 13 of the 16 gates were RUN here — all 13 still gated. 0 discharged.** Plus 2 gates outside
this section (§3 Platform Services Stage 2, §5 D8-SUPPRESS-1), also run, also holding: **15 run, 15 hold.**
The 3 not run, and why: **AGT-5** and **AGT-6b** need network (`gh search code` against the public
`jotder/inspect-agent` that `ci.yml` already clones — runnable, just not offline); **Completeness KPI
hold** has nothing to run, because its gate turned out to be an engineering action, not a check.
...
| **AGT-5 `DryRunProvider` + `incident_explain`** | Per-tool preview seam on `PlatformBuilder` (lets inspecto drop its parallel `AgentApprovals` previewer); `incident_explain` waits on the eoiagent host seam. Local-models-only scope cut stands. | ⬜ **NOT RUN here (needs network)** — but ✅ **made RUNNABLE 2026-09-07: it is NETWORK-gated, not unrunnable.** `jotder/inspect-agent` is a public repo `ci.yml` already clones on every run. **Check (needs network): `gh search code 'DryRunProvider' --repo jotder/inspect-agent`** — no hit ⇒ still gated. Cross-repo: **reopen when it ships a per-tool `DryRunProvider` seam.** ("Demand" dropped — it named no watcher and no definition.) → `superpower/agt-6-plan.md` §4.2 G2 |
| **AGT-6b multi-step agent graphs** | First cut = generalize `RunbookActions`, never free-form ReAct over mutating tools. | ⬜ **NOT RUN here (needs network)** — ✅ **made RUNNABLE 2026-09-07: same two `gh search code` probes as AGT-5**, for `DryRunProvider` and for the synchronous-per-call approval gate. Same upstream repo, two concrete preconditions: the eoiagent approval gate is synchronous per-call (nested gates deadlock) and there is no per-tool `DryRunProvider` seam. Reopen when both are gone. → `superpower/agt-6-plan.md` §4 |
...
- **P1** · **EDG-01 edition gating** — ✅ **mechanism DECIDED 2026-09-07 (operator): ServiceLoader modules for all six**, not `-D` switches (a switch leaves the code in the bundle; Personal ships no authenticator and binds every interface, so a mis-set flag re-exposes an unauthenticated surface — a module Personal never bundles cannot be). Covers SEC-08 + CMP-08 (Enterprise-only) and OPS-05/OPS-06/SEC-10/CP-09/CP-11/CP-13/CP-15 (not Personal) — all core code today, so four of the six are real extractions, not a gating pass. 🔴 **This is the only substantive Personal-scoped work on the board**, and it is live: `/metrics` is in `ControlApi.PUBLIC_PATHS`, so a Personal bundle serves it unauthenticated to every interface while CP-13 reads `—`. ✅ **Cell 1 (`CP-15` delivery channels) SHIPPED 2026-09-07** — `inspecto-notify-channels`; Personal registers **zero** channels (`NoChannelShipsInThePersonalBuildTest`). ✅ **Cell 2 (`OPS-06` backup/restore) SHIPPED 2026-09-07** — `inspecto-backup` contributes `backup`/`backup_verify`/`restore` through the new `MaintenanceTaskProvider` seam; on Personal they are **unknown tasks, refused loudly** (`NoBackupTaskShipsInThePersonalBuildTest`) — ⚠ the bundled `spaces/demo` nightly backup chain therefore FAILS at `config_backup` on Personal, by design. ✅ **Cell 3 (`CP-09` geo map + link analysis) SHIPPED 2026-09-07** in two commits: 3a made the route SPI PUBLIC (`RouteModule`/`ApiContext`/`Handler`/`ApiException`/`WriteGates`), appended `ServiceLoader`-discovered modules after the built-in list, added a **duplicate-registration guard** (first-match dispatch means a duplicate never fails otherwise), and widened `CapabilityManifestTest`'s one-directory scan; 3b moved `GeoRoutes`+`InvRoutes` into `inspecto-geo-link`, with a core **503 stub** for the absent case, a `/bootstrap` `features.geoLink` flag derived from what registered, and the SPA hiding nav + widget offers on it. ✅ **Cell 4 (`SEC-10` exchange/sharing) SHIPPED 2026-09-07** — `inspecto-exchange`; 11 paths 503 on Personal, `features.exchange` now requires the module (it previously advertised a Share button that 404'd), and a new `SharedItemConsumers` seam carries the component delete fence 🔴 that the census MISSED because `ComponentRoutes` used a fully-qualified name instead of an import. ✅ **Cell 5 (`CP-13`, the `/metrics` half) SHIPPED 2026-09-07** — `inspecto-metrics`; the Prometheus exposition left `ControlApi` and Personal answers the path **503 naming the module**, with no `# HELP` in the body. 🔴 **That closes the exposure this row was ranked P1 on.** ⚠ Only the exposition moved: `MetricRegistry` nine classes across three modules call it and it stays in core, so the counters still run and nothing reads them out over HTTP. **Two things remain, and BOTH need an operator call on scope before anyone builds them:** (a) **CP-13's events feed + audit CSV export** — `EventRoutes` could move whole, but that removes `/events`, the Events screen's data source, from Personal: a visible product change of a different character from a scrape endpoint (gate the feed / gate only the audit export / mark the row partial-by-decision); (b) **`CP-11` operational objects** — `com.gamma.ops` is 12 classes in the MANDATORY `inspecto-engine` with **13 classes in that same module** depending on it, several holding typed `ObjectService` fields, so it is a fork: full extraction behind a new `ObjectAccess` SPI (the largest piece in EDG-01) versus gating only the routes + `incident_purge`. Measurements in plan §§18–19. **Plan + ranked census: `archived-documents/plans-archive/edg-01-edition-gating-plan.md`** — six cells ranked easiest→hardest (CP-15 · OPS-06 · CP-09 · SEC-10 · CP-13 · CP-11), each with the seam it needs and its route-order check. 🔴 Two census claims were corrected by grounding: the **connector sidecar is NOT edition-gated** (so `SmtpEmailChannel` reaches Personal too, and CP-15 needs its OWN module), and `WebhookChannel` ships **inert**, not active — the defect is reachability, not live traffic. 🔴 **`CP-13`'s registry cannot be a module** — `MetricRegistry` has dependants all over core — so cell 5 gated the **exposition**, which is the whole of the exposure. Deliver cell-wise; a scheduled cell cites its row. → `EDITIONS.md` §Feature × edition matrix + §Edition-gating debt ✅ **Cells 3a/3b/4/5 SHIPPED 2026-09-07** — route SPI public + duplicate guard (`91b6c9de`), `inspecto-geo-link` (`1de693a3`), `inspecto-exchange` (`f39b531f`), `inspecto-metrics` (`d409921a`). ✅ **Cell 6 (`CP-13`'s events feed + audit CSV export) SHIPPED 2026-09-08** — `inspecto-events` takes the whole `/events*` surface; moved **whole** by operator decision because the audit CSV is a `?format=` branch inside `GET /events/export`, not a route. ⚠ It cost more than the plan said: **four** SPA panes consume the feed (Events, Audit log, Dashboard, Incident detail), and gating it would have removed Personal's Audit-log screen — which `EDITIONS` §Audit PROMISES Personal. So cell 6 also added the narrow core `AuditLogRoutes` (`/audit/search`, `/audit/export`, fail-closed to AUDIT/ACCESS_DENIED so it cannot serve as the feed), an Ops lens-home fallback, and explained `<inspecto-alert>` states on three panes. ⛔ **REMAINING: `CP-11` operational objects only.** Scoped by the operator 2026-09-08 to a **full extraction behind a new `ObjectAccess` SPI** — grounded at **37 files in `com.gamma.ops`** (12 top-level + 25 across 7 subpackages) and **27 dependants**: 13 in `inspecto-engine` and **14 in `inspecto`** the earlier census missed, several service-layer (`ServiceBootstrap`, `ServiceStores`, `ReadModel`, `CollectorService`, `RcaTemplateRegistry`). This is the largest single piece in EDG-01; next shift starts here. ✅ **CELL 7 (`CP-11` operational objects) SHIPPED 2026-09-08 — and with it EDG-01 is COMPLETE: all six “not for Personal” cells are now true of the build.** `inspecto-ops` takes the whole `com.gamma.ops` domain (31 files), the `/objects|/notes|/queues|/tags` routes, two Job Types and `incident_purge`; Personal 503s ~50 paths and reports `features.ops=false`. Baselines: Personal 23 modules / **3777** tests, Standard 31 / **4106**, Enterprise 32 / **4126**, zero failures. 🔴 Unlike cells 1-6 this feature was CONSTRUCTED by mandatory core, so it needed a host-declared `ObjectEngineProvider` handle beside the `ObjectAccess` seam — the provider could not live in the engine, because opening the stores needs `SpaceRoot`/`OperationalDb` from the module above it. ⚠ It also forced **two other rows to be amended** (operator-approved): `OPS-01` no longer promises Personal an `objects` store, and `SP-CTL-02` is now 🟡 for Personal — a gap still raises the EVENT everywhere, but promoting it to an ALERT object is `EventObjectBridge`, which left with the module. 🔴 Three defects the compiler could not see, all caught by verification: a Platform Service registered EAGERLY beside a neighbour that defers (632 NPEs from one line); `DecisionRoutes` reporting "Incident already open" when the module was merely ABSENT; and an optional-module dependency legal in only one direction, which `-DskipTests` passed anyway off a stale `~/.m2`. Recipe distilled as items 18-27 in `okf/backend/editions/editions-model.md`. **Nothing remains on this row.**
