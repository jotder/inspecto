...
| **Source-timezone for temporal data (2026-08-22, operator-requested)** | Naive timestamps are zone-less end-to-end today: every temporal cast is `COALESCE(TRY_STRPTIME(...))::TYPE` (`SchemaFieldTypes.castSql` -> `SqlBuilder.appendCoalesce:35-47`), partitions/`__event_time` derive from the same naive expr (`TransformCompiler.java:169-190,224-227`), BI grains are naive `DATE_TRUNC` (`MeasureCompiler.java:158-162`), and no connection ever issues `SET TimeZone`. 🔴 **Live trap:** `TIMESTAMPTZ` is an honored field type (`82c947ef`) but casts through the SAME naive chain, folded into the TIMESTAMP branch (`TransformCompiler.java:200-210`) — the parsed text is interpreted in DuckDB's **session zone = host** (this box +05:30), so declaring TIMESTAMPTZ today silently imports server-local time. **Wanted options (operator):** per-column zone policy — (i) wall-clock as-is (default, today's behavior) · (ii) fixed IANA zone `fields[].timezone` (incl. `UTC`) · (iii) per-row zone column `fields[].timezone_column` — plus a pipeline-level `source_timezone` beside `date_formats` in the parsing block; precedence row-column > column > pipeline > none. Compile shape: `timezone('UTC', timezone(Z, <naive-parse>))` -> naive-UTC (session-independent; downstream partitions/BI/dedup stay naive and consistent); `TIMESTAMPTZ` targets get the instant and should REFUSE at load without a zone source (fail closed on the trap above). ONE choke point: apply inside `dateExpr`/`castSql` so casts, partitions and `__event_time` normalize identically. `meta.domain.timezone` exists, IANA-validated, and is deliberately display-only (D6 row, this file) — activating it as the pipeline default would reverse that decision; prefer the NEW parsing-block key unless the operator says otherwise. ⚠ Before building: live-probe `TRY_STRPTIME` with `%z` formats (an offset in the data should WIN over any configured zone) — run DuckDB, don't assume. UI home: tab 2 'Source timezone' select + a zone column in the metadata grid. | `okf/backend/engine/duckdb.md` |
