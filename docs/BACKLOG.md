...
**Verdict counts:** UNBLOCKED **37** · BLOCKED-OPERATOR **10** *(8 + `MIGRATE-ENRICH-1` and `MIGRATE-MATERIALIZE-1`, filed 2026-09-16, both waiting on ONE shared call)* · DESIGN-FIRST **7** *(both graph-lane rows were filed, designed, decided and SHIPPED on 2026-09-16 and are off the board)* · BLOCKED-EXTERNAL (trigger not
...
scale-out phase B → `DUCKLE-C10` feature · a Postgres host → Postgres multi-user acceptance. ~~`GRAPH-LANE-MULTISCHEMA-1`~~ (shipped 2026-09-16) **and** `GRAPH-LANE-RULE-ROUTED-1` → §2 Row 15's deletion
half (only the second is left; re-run `mvn -Dingest.lane=graph test` when it lands). Satisfied
...
| **Where do `*_enrich.toon` and the `materialize` task GO?** (filed 2026-09-16 from the §6 step-1 converter build) — (a) both stay **Jobs** ⇒ strike those two clauses from the amendment's §6 step 1 and the converter skips enrich files; or (b) `*_enrich.toon` really becomes a **table-entry Pipeline** ⇒ it waits on Phase 3 S3 | `MIGRATE-ENRICH-1` · `MIGRATE-MATERIALIZE-1` (§3) · whether any space can be migrated at all | 🔴 **`GLOSSARY.md` currently says BOTH**, and the vocabulary is binding: §Enrichment (`:398`) migrates the file kind to a table-entry Pipeline, while §Job (`:456`) and the §13 row (`:908`) record the operator's own 2026-08-06 REVERSAL — which rests on the fact that the enabling slice (Phase 3 S3) was deferred as genuine new design. ⛔ A shift cannot pick a side from the code: both readings are written down as current, by the same authority. ⚠ Until this lands, `inspecto migrate-configs` refuses every space that owns an enrich config — including the demo space |
