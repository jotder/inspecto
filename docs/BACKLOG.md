...
| **Source-timezone for temporal data (2026-08-22, operator-requested)** — **S1 + S2 BOTH SHIPPED 2026-08-29; plan ARCHIVED** | The three-tier policy (`raw.fields[].timezone_column` > `raw.fields[].timezone` > `parsing.source_timezone` > none) is live end to end, engine and surfaces, and the `TIMESTAMPTZ` host-zone trap is closed by refusal. **The as-built, every finding and every deliberate exemption now live in the concept home** — `okf/backend/engine/duckdb.md` §*The source time zone for temporal data*; the plan and its full live-probe transcript are at [`archived-documents/plans-archive/source-timezone-plan.md`](archived-documents/plans-archive/source-timezone-plan.md). **Two items stay OPEN, and only these two:** (a) ⛔ **no editor for `timezone_column`, by decision** — offering a per-row column beside ~418 zone names in one cell invites the ambiguity the engine's mutual-exclusion rule exists to prevent; a hand-authored one is carried through a save and shown read-only. Reopen only if an operator asks for it by name. (b) 🔴 **the `%z` defect — GROUNDED 2026-08-29, and this row's own framing was wrong in three ways.** Measured against DuckDB 1.5.2.1 across four host zones; the full transcript is in the concept home. **It is NOT the plain branch** — `appendCoalesce`'s trailing `::TIMESTAMP` converts back through the same session zone, an identity for every unambiguous local time. **It does NOT need a mixed list** — a `%z`-ONLY list has it too. **The real defect:** `TRY_STRPTIME` parses the offset correctly and the `::TIMESTAMP` then **throws the instant away by rendering it in the HOST's wall clock** — `2026-03-01 10:00:00+00:00` becomes `15:30`/`11:00`/`10:00`/`05:00` under Asia/Calcutta / Europe/Berlin / UTC / America/New_York. With a declared source zone it is doubly wrong and lands on a **different calendar day** by host; a `DATE` list with `%z` moves `DATE_*` **partition keys** by host too. Mixing adds only a narrow DST-gap shift on the plain branch. ⚠ Nothing validates a format string (`ConfigValidator:84` only warns on an empty list). **Still latent: no `.toon` under `spaces/` uses `%z`.** **DECISION OWED before any build — the intended semantics are on record** (the original row asked to probe “an offset in the data should WIN over any configured zone”; today it loses, to the host): **(i) fail closed** — refuse `%z` in a format list at config load, the posture S1 took for a zone-less `TIMESTAMPTZ`, closing the silent corruption cheaply; or **(ii) build “the offset wins”** — emit `timezone('UTC', …)` for a `%z` list, making the data's offset a fifth precedence tier above `timezone_column`. ⚠ (ii) first needs a grounded compile-time predicate for “does this list contain a LIVE `%z`” (`%%z` is a literal). | `okf/backend/engine/duckdb.md` |
