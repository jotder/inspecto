...
| **Parsing (Stage-1)** | **JSON flatten DSL (engine)** — deferred from the 2026-07-29 parsing-UX reflow (operator call: UX now, DSL next). Today `parsing.json` honors exactly `format` + `records_path`, and `records_path` is **locked to `$`** (any other value hard-fails, `PipelineConfigParser.java` `mergeParsing`/`parseJson`); nested values are stringified into one column (`DuckDbCsvIngester.buildJsonReadSpec` — `json_extract_string`/`read_json` keyed on top-level selectors). The slice: unlock `records_path` as a JSONPath subset (`$.a.b[*]`) + allow nested field selectors (`$.a.b.c`) — DuckDB's `json_extract_string` already accepts nested paths, so the work is parser validation + read-spec assembly + `ComponentPreview` parity + schema-stage selector derivation + tests. The onboarding Tree view (`jsonSampleToTree`) already shows builders the nested shape with an honest "top-level keys only" note pointing here · **XML / ASN.1 frontends** stay out of the engine until the `asn-parser/` integration decision lands (a separate Maven project, not in the reactor — the other active workstream); the Pipelines Parser dialog's 9-type grammar catalog + tree preview remains a **mock-only prototype** (`/components/grammar/preview` has no server route; `/asn1/modules` mock-only, see the Pipeline-graph row) | `okf/frontend/features/onboarding.md` |
