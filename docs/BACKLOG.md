...
- **P2** · **`SPACES-GOVERNOR-1` — `IntakeGovernor` is process-wide with NO Space key.** Found 2026-09-10 while grounding the scale-out plan. `inspecto-acquire/src/main/java/com/gamma/acquire/IntakeGovernor.java` (`:11-16`, `:39`, `:83-88`) documents itself as *"process-wide and keyed by pipeline id"*, while its stated sibling idioms — `StabilityGate.shared()`, `AcquisitionLedgers`, `EventLog` — are all keyed per Space. Consequence **today, single node**: two Spaces running a same-named pipeline share T15 admission-control state, so one tenant's surge throttles another's. A Spaces-isolation defect, independent of scale-out — but a hard prerequisite for it, since partitioning by Space puts several Spaces on one pod by design. Fix: key the governor by `(space, pipeline)` exactly as `EventLog` does; test: two Spaces, same pipeline name, one saturated — the other's admission is untouched. → `superpower/enterprise-scale-out-plan.md` §3.8 · `okf/capabilities/spaces/spaces.md`
