...
| State | P2 rows |
|---|---|
| **Startable now** — no gate, no owed decision | `MAT-SELECT-SWEEP-1` (§3.11, mechanical) · `DUCKLE-C1-DATASET-FRESHNESS-1` residual (4), the Dataset badge (§3.5) · `STREAM-CONSUMER-1` close-out — the `SP-ACQ-09` cell (§3.3) · Onboarding D5-ref — ground the delete-feed first (§3.4) · `RELEASE-PIPELINE-NEVER-EXECUTED-1` — the checksum/signature split (§4) |
