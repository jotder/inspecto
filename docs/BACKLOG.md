...
- **D7 — `rename`/`removeTag` have no route** (the largest remaining D7 gap). Both are implemented and
  tested on `TagAssignmentStore` — rename is the operation the whole central-store architecture was chosen
  for — but nothing calls them: there is no rename endpoint, and `POST /tags` has no delete counterpart,
  so a tag removed from the registry leaves its assignments orphaned. ⚠ **A rename route must also
  re-project every affected object's CSV** — the store rename alone leaves the projection stale, which
  `ObjectTagProjectionTest.renamingATagReachesTheObjectCsvOnceReprojected` pins on purpose. Wire rename +
  registry-delete + re-projection together, in one change.
...
- **D7 — no UI.** The backend is complete end-to-end; nothing in the SPA calls the assignment routes, so
  cross-entity tagging is API-only today.
...
| Generic tag system | D7 (rescoped) · **phases 1+2 shipped 2026-07-26**; rename/delete routes + UI open (§6) · `okf/backend/control-plane/tags.md` · `superpower/generic-tags-plan.md` · ⚠ supersedes the old §4 "`category`/`tags` params on `GET /objects`" row — the *existing* `Tag`/`TagRule`/`/tags` system is what gets generalized, not a separate feature. Keep aligned with **D10** (generalized notes): same `(kind, id)` addressing problem, so one adopts the other's scheme |
