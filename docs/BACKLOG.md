...
| State | P2 rows |
|---|---|
| **Startable now** — no gate, no owed decision | assurance wave 2, in order once wave 1 lands: `ASSURE-MAKER-CHECKER-1` (§3.7) → `ASSURE-ACTION-REQUESTS-1` (§3.5); `ASSURE-ENTITY-LISTS-1` ⏸ on hold for Link Analysis |
...
| 3.5 | `D8-SES-SNS-1` | ⏸ adapter ON HOLD 2026-09-25 (D1–D12 parked); D13 answered by per-user read state; D14 done 2026-09-25 (the category label is now "Security"; id `security` unchanged); open: GeoIP and security triggers, §6 and §8 of `superpower/ses-sns-adapter-design.md` (§7 per-user preferences shipped 2026-09-25) |
...
- **P2** · **EXECUTION-RESIDUALS X4 + X1 deferrals** — **X4 record-level replay: first slice SHIPPED 2026-09-25.** Eject-and-continue (the operator's default) turned out to be ALREADY how CSV ingest behaves — a short record goes to `<errors>/<file>_errors.csv` with line, reason and raw line, the rest lands, the file commits `SUCCESS` with `error_rows > 0` (now pinned for both engines); what was missing was the replay. `POST /runs/{name}/replay-rejects {file}` (`canOperateRuns`) replays one file's sidecar `raw_line`s as a real Consignment through the normal lane, attributed by a replay record (`<status_dir>/replays/<sidecar-sha256>.json`, original line numbers kept), idempotent (a second replay of the same sidecar is 409). ⚠ Wire change: `raw_line` now doubles `"` instead of turning it into `'` — sidecars written before 2026-09-25 can still corrupt quoted values on replay. The dry run also reports per-record rejects (line + reason, first 100 + total). As-built: `okf/backend/pipeline-graph/execution-lanes.md`. ✅ Run Detail's rejected-rows dialog has **Replay rejected records** (2026-09-25; disabled with a reason without `canOperateRuns`), the audit row is `pipeline.rejects_replayed`, and an empty claim older than 30 minutes is treated as abandoned and re-taken. **Still open:** ⚠ a crash between the replay commit and its record write also leaves an empty claim, and reclaiming it would land the records twice (documented, not detected); the dry run's rejects are not on its HTTP response; and `processing.reject_mode: all_or_nothing` is NOT small — the single-file DuckDB path streams to output before it knows its reject count, so all five ingest paths would change (the nearest today is `csv_settings.ignore_errors: false`, DuckDB engine only) — build it only if someone asks. ✅ **X1 SHIPPED 2026-09-25**: `GET /runs/{name}/retries` (per pipeline; tells "no retry state" apart from "none pending"), `POST …/retries/retry-now` (clears the backoff, keeps the attempt count) and `POST …/retries/cancel` (quarantines now as `retry_cancelled`), both `canOperateRuns`, refused 409 while the pipeline is mid-cycle; and the per-pipeline `processing.retry` block (`max_attempts`, `initial_backoff`, `max_backoff`, each falling back to its `-Dingest.retry.*` global). **Still open:** a Run Detail UI for the retry queue (Q3 — routes first). → `okf/backend/pipeline-graph/execution-lanes.md` · `archived-documents/plans-archive/execution-residuals-plan.md`
#### Consignments
...
- **P2** · **VIRTUAL-DATASET-SQL-1 — a virtual Dataset authored as SQL is unusable end to end** (cricket pilot 2026-09-25) — the Dataset editor's Run evaluates the SQL client-side in AlaSQL (no DuckDB functions: `strptime` → "alasql.fn.strptime is not a function"; `QueryPanelComponent` never forwards `serverRun`/`runOnServer`); a saved hand-edited SQL lands as `QueryModel.sqlOverride`, which `compileSql` (`inspecto/query/query-sql.ts`) silently DROPS for row previews; and `DatasetRelation.baseRelationSql` knows only `view`/`physicalRef`, so `/bi/query` over a `kind: virtual` Dataset 422s. Builders fall back to calculated columns on a physical Dataset. Fix: teach `DatasetRelation` to compile `dataset.query` (prefer `sqlOverride`, SqlGuard-checked), and run the editor preview on the server. → `okf/frontend/features/studio.md`
...
- **P2** · **Signal / Decision networks — cross-Space consequence** — trigger FIRED 2026-09-15: a Signal in one Space must cause something in another. Re-grounded 2026-09-17: this is structurally absent, not merely unguarded — each Space is isolated at the runtime-instance level (`EventLog` keeps one ledger per Space; `SpaceContext` gives each Space its own services), `Signal.space` is descriptive only, and there is no target-space parameter anywhere in the apply path. ⛔ No small safe increment exists — it needs a **design pass** on the cross-space controller (S8): does a consequence need its own authorization, independent of the triggering caller? Under what identity does a target Space accept an externally originated Signal? Scope to the consequence that was asked for; connector-direct emission rides with it; ⛔ RFC 6902 JSON Patch deltas for AG-UI have no consumer and do not ride along. ⏸ **ON HOLD (operator, 2026-09-25)** — build nothing until someone names the concrete consequence (D10: origin Space, signal type, target Space, Job); D2 (two-party consent on the Exchange) stands. 📐 **Design pass written 2026-09-24:** [`superpower/cross-space-consequence-design.md`](superpower/cross-space-consequence-design.md) — 12 decisions owed (§8). **Decided 2026-09-25 (operator):** D2 — two-party consent on the Exchange (Option D). ⚠ D10, the concrete consequence that was asked for, is still owed and gates the build. → `okf/backend/control-plane/signal-backbone.md` §"Open / deferred" · `okf/backend/control-plane/decision-rules.md`
- **P2** · **Notification residuals (`D8-SES-SNS-1`)** — Soft-bounce retry shipped 2026-09-15. Remaining: ⏸ **the SES/SNS adapter is ON HOLD (operator, 2026-09-25)** — do not build it until the operator names an SES deployment; its design and D1–D12 stay parked in the design doc. For the record it is: SNS subscription confirmation plus a cert-chain fetch from a validated `amazonaws.com` URL; ⚠ an outbound fetch induced by an unauthenticated callback gets its **own security review and its own commit**; ✅ Per-user preferences SHIPPED 2026-09-25 (design §7: an admin default grid plus a sparse per-Subject override; email only to a verified claim; `PUT /notifications/preferences/default` on `canAdminister`; as-built in `okf/backend/control-plane/events-metrics.md`). **Still open (not held):** GeoIP and security triggers (design §6, §8). **Designed + security-reviewed 2026-09-24:** [`superpower/ses-sns-adapter-design.md`](superpower/ses-sns-adapter-design.md) (14 operator decisions owed). → `okf/backend/control-plane/events-metrics.md`
- **P2** · `ASSURE-ENTITY-LISTS-1` — ⏸ **ON HOLD 2026-09-26 (operator)** — Link Analysis leads the Entity List (`LA-17` slice 1 shipped `887b5215b`). A stopped assurance lane built it as a `ComponentStore` kind (branch `worktree-agent-a02fa4140798df276`: 4 unverified backend slices + a WIP UI commit); `LA-17` D-M8 below makes that shape **likely superseded** — evaluate it only for parts worth porting (range / CIDR normalisation, the `sql.template` relation, the Parquet sidecar). Do not merge it. **Entity Lists — allow / block / watch / exclusion as one component kind (WS-12, wave 2.2 of [`superpower/assurance-capability-plan.md`](superpower/assurance-capability-plan.md)).** After `ASSURE-MAKER-CHECKER-1`. Typed keys (msisdn, imsi, imei, iccid, prefix / range, country, IP, CIDR, instrument token, BIN …), entries with reason, added-by and expires-at, a Parquet sidecar for 10⁵ entries, range / CIDR matching (the equal-key join cannot). Expiring (≤ 24 h) entries apply at once and are reviewed after (D-P5). ✅ **D-P10 (operator 2026-09-26): ONE kind with the Link Analysis `LA-17` Entity List, named Entity List** — never *Control List*. ⚠ **Reconciled the same day (`LA-17` D-M8):** one concept in one store, the Identity Fact log `LA-17` ships (`/inv/entity-lists*`) — not a `ComponentStore` kind; this item extends it with ranges / CIDR, expiry, the sidecar and four-eyes.
- **P2** · `ASSURE-ACTION-REQUESTS-1` — **approval-gated outbound dispatch (WS-24, wave 2.3 of [`superpower/assurance-capability-plan.md`](superpower/assurance-capability-plan.md)).** After the two rows above. The Decision Rule `invoke-api` consequence is still a stub Signal (`DecisionRoutes.java`). Build an `ActionDispatcher`: target Connection, payload template, `draft → pending → approved → dispatched → succeeded / failed`, idempotent retries, linked Incident, audit. Accept against a stub HTTP endpoint.
...
- **P2** · `ASSURE-MAKER-CHECKER-1` — **four-eyes approval for human config changes (WS-13, wave 2.1 of [`superpower/assurance-capability-plan.md`](superpower/assurance-capability-plan.md)).** Step 0: `PipelineSettingsRoutes.java` (`/label`, `/settings`, `/save-as-template`) and `PipelineRenameRoutes.java` (`/rename`) write outside `SaveGate` — route them through it or record why not, plus a guard test enumerating every mutating route. Then a per-kind approval policy, a **Pending Change** (proposed content, base version, author), approve / decline / expire, approver ≠ author (403), stale base (409), a shared diff view. Copy the Link Analysis four-eyes pattern, not its names.

- **P3** · `ERRORCODE-DEFAULTED-1` — **explicit error codes on the remaining bare throw sites.** The 403 slice is done (zero bare 403 sites; `ErrorCodes` and its constants are public; `ApiContractTest` pins the catalog). **85 of 891 `ApiException` sites still take `ErrorCodes.defaultFor(status)`** (2026-09-25, after seven slices; the total grew as routes landed — geo-link `InvestigationRoutes`/`InvRoutes` 107 sites, then `ReconRoutes` · `AgentRoutes` · `JobRoutes` · `RunRoutes` · `DossierRoutes` · `InvestigationTime` · `PatternQueryCompiler` 130 sites, then `ConnectionRoutes` · `ConfigPreviewRoutes` · `PipelineBundleRoutes` · `AccessRoutes` · `DbBrowserRoutes` · `TagRoutes` 108 sites, then `SpaceRoutes` · `SchedulerRoutes` · `Roles` · `PipelineRenameRoutes` · `ExpectationRoutes` · `AccessPolicies` · `ShareRoutes` · `NotificationRoutes` 98 sites, then `QueryRoutes` · `ParserRoutes` · `ConfigReadRoutes` · `RuleRoutes` · `EnrichmentRoutes` · `RouteErrors` 58 sites (one wire change: the rule-template simulate 503 is now `CONTROL_PLANE_READ_ONLY`), then `SettingsRoutes` · `PipelineSettingsRoutes` · `ConfigWriteRoutes` · `ComponentAccess` · geo-link `InvestigationTemplateRoutes` · `InvestigationMeasureRoutes` · `GeoRoutes` 56 sites, all status defaults otherwise; three four-eyes 403s moved `PATH_JAIL_VIOLATION` → `PERMISSION_DENIED`) — sweep by file, re-deriving each count first (the row's original figures were wrong by ~3×). Swept 2026-09-24: `ComponentRoutes` 40 · `inspecto-ops/ObjectRoutes` 39 · `PipelineGraphRoutes` 33 · `ExchangeRoutes` 31 — zero bare sites left in each; codes are the status defaults except the ad-hoc-run 503 (now `CONTROL_PLANE_READ_ONLY`, the write-root gate it is) and the two run-to-here 501s (new `NOT_SUPPORTED`, also `defaultFor(501)` — it was `INTERNAL`). Largest remaining (5 each): `PipelineListRoutes` · `BiTemplates` · `BiRoutes` · `AssistRoutes` · geo-link `WorkingSetRoutes` · `PatternRoutes`; ⚠ `DeliveryStatusRoutes` (4) is the public callback — check its codes deliberately. Slice 7 moved the no-write-root 503 on `GET /datasources` and `/export` to `CONTROL_PLANE_READ_ONLY`. Derive with `git grep -cP 'new ApiException\([^,()]+,(?!\s*(\w+\.)?[A-Z][A-Z_]+\s*,)' -- '*/src/main/**.java' | sort -t: -k2 -nr` (sum the counts; drop `-c` and pipe to `wc -l` for the total; all sites: `git grep -hPo 'new ApiException\(' -- '*/src/main/**.java' | wc -l`). ⚠ One site emits a code **outside the catalog**: `PipelineGraphRoutes` `new ApiException(422, PipelineValidator.JOIN_REFERENCE_MISSING, …)` — not in `ErrorCodes`, so not in the OpenAPI `ErrorCode` enum.

### 3.8 Security, Policy, Editions & Compliance

- **P2** · **Security: policy-authoring UX** — ✅ **BUILT 2026-09-25** (all nine calls answered; D1 = guard every failure mode rather than wait for the incident report). `PUT /access/policies` now refuses or warns on all nine design failure modes before they take effect: 422 `unknown-ref` (F2), `would-lock-out` (F7 — the save would deny the saver `PUT /access/policies`), `unknown-key`/`ambiguous-key` (F8), `deny-everything` (F9); warnings `unknown-capability`/`unknown-role` (F3), `unknown-resource-kind` (F4), `resource-ref-at-route-level` (F5), `seed-override` (F6); an unreadable doc's error now names the policy and the check (F1). `POST /access/policies/preview` returns a role × action impact matrix, and the Policies tab is an editor. ⚠ Breaking: an on-disk doc with a stray key or an un-allowlisted claim becomes unreadable (deny-all on Enterprise) on upgrade — release-noted. As-built: `okf/backend/editions/auth-security.md`. Close-out done 2026-09-25 (`security.md` §3.10, EDITIONS SEC-05 ✅, design archived). **Still open:** a browser pass over the editor. → `okf/backend/editions/auth-security.md` · design: [`archived-documents/plans-archive/policy-authoring-ux-design.md`](archived-documents/plans-archive/policy-authoring-ux-design.md)
...
- **P1** · `ASSURE-IMPACT-LEDGER-1` — **typed financial impact + Disposition on Incidents and Cases (WS-10, wave 1.3 of [`superpower/assurance-capability-plan.md`](superpower/assurance-capability-plan.md)).** Today `ObjectService.analytics()` (`inspecto-ops/src/main/java/com/gamma/ops/ObjectService.java`) sums one flat `impactAmount`. Build: suspected / confirmed / recovered / prevented with currency, `outstanding` derived and never stored; on a Case it types the impact its Findings already carry; a Disposition required on resolve for Incidents, extending the GLOSSARY §9 ladder; an impact ledger Dataset any Measure can read; server-validated and audited. ✅ **BUILT 2026-09-26 (S1–S4, lane branch, unit-tested):** `PUT /objects/{id}/impact` (`canWorkIncidents`, 400/422/409/404, audited before/after), `outstanding` derived on read, Findings `impactAmount` retired, per-currency analytics; Disposition required on Incident resolve (+ `DUPLICATE`, `ACCEPTED_RISK`); SPA impact panel + Disposition in the resolve dialog; `impact_ledger` Dataset from the `objects.analytics` run. As-built: `okf/capabilities/incidents/incidents.md` §3.4–3.5, `okf/backend/control-plane/jobs.md`, `okf/frontend/features/objects.md`. **Still open:** a browser pass over the impact panel + resolve dialog; the enterprise reactor gate; the ledger is as fresh as the job's cadence (no live view of the objects table); a stored object that carries only the retired flat `impactAmount` (e.g. in a seeded Space's DuckDB — not checked) stops counting in analytics until it is given a typed impact; no migration by design (breaking changes are free).
