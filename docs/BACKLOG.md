...
| **Source-timezone for temporal data (2026-08-22, operator-requested)** — **S1 (engine + config) SHIPPED 2026-08-29; S2 (surfaces) OPEN** | **Shipped:** the three-tier policy is live end to end in the engine — `raw.fields[].timezone_column` > `raw.fields[].timezone` > `parsing.source_timezone` > none, compiling to `timezone('UTC', timezone(Z, <naive-parse>))` so a value stops depending on which host ran the batch. Applied at the mapped column, the `DATE_*` partitions and `__event_time`, and `CONCAT_DT`; ⛔ `FILENAME_DATE` and `DATE` are zone-exempt by design (a date has no instant to shift). 🔴 **The live trap is CLOSED**: a `TIMESTAMPTZ` field with no zone source is now refused at config load instead of silently importing server-local time. No shipped config declared one, so nothing broke. The default path is unchanged and byte-identical, pinned by a test. 🔴 **Three findings the row did not predict** — `TRY()` does NOT catch DuckDB's *Unknown TimeZone* (so a per-row zone column resolves through `pg_timezone_names()`, nulling a bad row instead of killing the batch; ~2µs/row, paid only when configured) · the zone-column reference needs `CAST(… AS VARCHAR)` (`lower()` binds only to VARCHAR — a typed column was a binder error) · a blank cell in TOON's tabular field form is **absent, not empty**, so null-only checks would have refused every schema that gives any field a zone. ⚠ The row's **"ONE choke point" was wrong** — there are four temporal sites, not two; `CONCAT_DT` was missed. ⚠ Offset zones (`+05:30`, `Z`) are **rejected by DuckDB**, so the gate is membership in `ZoneId.getAvailableZoneIds()`, pinned against the live engine. ICU turned out to be preloaded and to survive the `SqlSandbox` seal — no extension work was needed. **S2 (surfaces) SHIPPED 2026-08-29** — the `source_timezone` select on the Grammar editor's **Types** tab (all four frontends, parsing-level, no default), a **Source zone** column in the columns table rendered only for columns carrying an instant, one shared zone vocabulary (`inspecto/schema/time-zones.ts`), and mock parity mirroring the server's refusals. ⚠ `ConfigSpecs` needed nothing (parsing keys are frontend specs; no allow-list gates an unknown key). ⚠ Placement deviates from this row on purpose: the metadata grid is documented as never-ETL-read, so the zone went beside the TYPE it qualifies, on the DECIMAL-parameters precedent. 🔴 **The preview found a pre-existing defect the 2816-test suite missed** — an `<inspecto-option-picker>` asks in a MatDialog attached to `document.body`, so the pick never bubbled a click through the pane and **Apply stayed greyed out over a choice just made**, for every select on all three `pipelines/*-definition` drawers; fixed with `@HostListener('document:click')` and pinned. **Still open:** no editor for `timezone_column` (a hand-authored one is carried through a save and shown read-only — offering it beside ~418 zone names invites the ambiguity the mutual-exclusion rule prevents). **Also open, pre-existing and latent:** a format list mixing a `%z` pattern with a plain one unifies the whole `COALESCE` to TIMESTAMPTZ, session-interpreting the plain branch (no shipped config uses `%z`). Plan + full probe transcript: [`superpower/source-timezone-plan.md`](superpower/source-timezone-plan.md) | `okf/backend/engine/duckdb.md` |
