...
| **Archive / exotic-compression inputs (2026-08-23, operator question)** | Input `compression` is a VERBATIM pass-through to `read_csv(compression=...)` (`PipelineConfigParser.java:361` → `DuckDbCsvIngester.readOptions`), no input-side whitelist — DuckDB itself accepts only none/gzip/zstd/auto, so `zip`/`tar`/`Z` fail at query time (file → `QuarantineManager.REASON_UNREADABLE`). The `SafetyPolicy.DEFAULT_COMPRESSION` allow-list validates OUTPUT compression only. The Java/univocity fallback transparently decompresses by EXTENSION (`Compression.java:36-66`: .gz/.bz2/.zip first-entry-only), so .bz2 and single-entry .zip already work on `engine: java` but NOT on the native path — a same-file engine divergence. Multi-member archives (.tar/.tar.gz/.tgz, multi-file .zip) are handled ONLY by the manual CLI pre-stage (`MainApp copy-tars/extract` → `TarUtil.java:25`, inspecto-util Tar*), never by the automatic poll loop (`CollectorProcessor` has zero unpack references). `.Z` (LZW) is supported nowhere. **Design direction (not started):** an UNPACK stage in the collector, not more `compression` values — a multi-member archive is a BATCH question (member ordering vs `mtime` arrival semantics, lineage `file_name` = member or archive?, retire/backup of the archive vs members), not a reader flag; Commons-Compress is already on the classpath (bz2) and covers tar/zip/.Z (`ZCompressorInputStream`). Smaller safe slices first: (a) publish/validate the input `compression` values fail-closed at config load so `compression: zip` is refused with a message instead of quarantining every file at run time; (b) route .bz2/.zip extensions to the Java engine under `engine: auto` (today auto picks native and fails on them). **Plan COMPLETE + ARCHIVED 2026-08-26** — archives (.zip/.tar/.tar.gz) and streams (.gz/.bz2/.Z) both expand at the Collector: `archived-documents/plans-archive/unpack-stage-plan.md`; as-built in `okf/backend/engine/unpack-stage.md`. | `okf/backend/engine/duckdb.md` |
