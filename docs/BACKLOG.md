...
|---|---|---|
| **Pipeline definition drawers (P4)** | **B1's mapped-row preview has no consumer** — `POST /config/preview/schema` returns `mappedColumns`/`mappedCount`/`mappedRows` when the draft carries `mapping.rules`, but the Load pane cannot render them: it needs sample rows and the editor has **no sample thread**. ~~Decide between wiring the thread into the editor or leaving the preview server-side~~ **✅ DECIDED 2026-08-16 (operator) and the THREAD half is SHIPPED**: one `DefinitionStateService` **per tab** (a Map beside `cachedModels` — 🔴 not a `providers:` entry, which on this one-instance-hosts-every-tab component would leak one sample across every open pipeline), the strip mounted in the parse drawer, and Test parse mirrored into it through the grammar editor's `previewFn` (⚠ needed for the FAILURE path — `previewed` fires on success only, so a stale "parsed" chip would otherwise stand over a grammar that no longer parses). **What remains of B1 is the Load drawer rendering `mappedColumns`/`mappedRows`** — its blocker is gone, the rendering is its own change. 🔴 And it will only ever have rows where the parse node is **per-format**: a generic `parser` node still opens the grammar-editor dialog, which has no thread. ⚠ Not a defect — an unconsumed capability. · **P6-e's two deliberate narrowings** (neither a bug): the retired shell's *View as graph* deep-link was NOT carried onto the editor (it needs the stream-vs-reference kind, i.e. an extra settings hop; the Catalog's lineage tab is directly reachable) · *Export configuration* now sits in the editor's `canAuthor()` menu, so it is hidden from the Business lens where the shell offered it in every lens — consistent with its neighbour *Export document*. · Per-**segment** schemas (`<id>_<segmentKey>`) are still not swept by a pipeline delete; the wizard never swept them either, and enumerating them needs the parsed block a delete no longer has. · The Load drawer offers `DIRECT`/`EXPR` only; `CONCAT_DT`/`FILENAME_DATE` are preserved when already authored but cannot be created there (they carry source semantics the grid has no affordance for). | `okf/frontend/features/pipelines.md` |
