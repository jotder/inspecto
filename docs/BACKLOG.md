...
**Verdict counts:** UNBLOCKED **37** · BLOCKED-OPERATOR **9** · DESIGN-FIRST **8** *(7 + `GRAPH-LANE-RULE-ROUTED-1`; `GRAPH-LANE-MULTISCHEMA-1` had its design pass the day it was filed and moved to operator-blocked)* · BLOCKED-EXTERNAL (trigger not
...
- M — `TYPEFLOW-DATASET-COLUMNS-1` steps 3+4 (⚠ relies on the dead-seam verdict (2) = keep `temporalColumn`) —
  **the M tier is otherwise DRAINED (2026-09-16)**: `AGT-5` → BLOCKED-EXTERNAL (no `PlatformBuilder` seam) · the
  parity gate → RUN, NOT MET (two graph-lane gaps, FILED as rows 2026-09-16: `GRAPH-LANE-MULTISCHEMA-1`, `GRAPH-LANE-RULE-ROUTED-1` — both design-first, neither an M) · pre-materialise cap → §1 owed input.
...
**Blocked on the operator (§1), by rows unblocked:** the multi-schema lane flip → `GRAPH-LANE-MULTISCHEMA-1` (design done, code is small and waits only on that call) · the FOUR route calls → the P1 (`ROUTE-UNGATED-DEFAULT-1`
...
**Design-first (a decision pass before code, by the row's own words):** ~~`GRAPH-LANE-MULTISCHEMA-1`~~ ✅ **design pass DONE 2026-09-16** (`superpower/graph-lane-multischema-design.md`) — the executor question was answered empirically and the row shrank; it is now **operator-blocked on one call**, below · `GRAPH-LANE-RULE-ROUTED-1` (where does a rule-routed destination become a graph node, when the lift cannot see the rules?) · AI drafting on non-schema kinds ·
...
scale-out phase B → `DUCKLE-C10` feature · a Postgres host → Postgres multi-user acceptance. `GRAPH-LANE-MULTISCHEMA-1` **and** `GRAPH-LANE-RULE-ROUTED-1` → §2 Row 15's deletion half (BOTH must close;
re-run `mvn -Dingest.lane=graph test` after each). Satisfied
...
| **Does `-Dingest.lane=auto` divert MULTI-SCHEMA writes to the graph lane, or only `-Dingest.lane=graph`?** (filed 2026-09-16 after the design pass) | `GRAPH-LANE-MULTISCHEMA-1` (§3) → §2 Row 15's deletion half | The ELT §6 parity gate goes green **either way**, so this is not an engineering choice the code can make for itself. Flipping `auto` changes the live write path for every `segments:` pipeline at once and is the only option that gives the lane real production exposure; `graph`-only changes nothing live but creates a path only the gate exercises — ⚠ a shape this repo has already been bitten by. ⛔ Do not settle it while implementing. → `superpower/graph-lane-multischema-design.md` §5 |
