...
D6's own premises were wrong and are corrected in its row. **D8 PLANNED** — plan written, not started, and
it surfaced three gaps the D8 row never anticipated, chief among them that nothing correlates a sent message
back to a Notification. Same day: **sign-out fixed end to end** — the user-menu button was navigating to a
...
| D8 | Track **all three** of `delivered`/`bounce`/`complaint` — complaint is the only class with a *deliverability* consequence, so it can't be the one dropped. Needs per-status timestamps (not one mutable enum — `delivered` then `complaint` is the normal spam-button case), adapter-edge normalization of provider vocabularies, a hard/soft bounce split, and a signature-verified fail-closed callback. **📋 PLANNED 2026-07-26, not started** — `superpower/delivery-status-webhooks-plan.md`. ⚠ It surfaced **three gaps this row did not anticipate**: (1) **nothing correlates a sent message back to a `Notification`** (`deliver` returns `void`, SMTP discards the `Transport.send` result, the outbound webhook throws away the response body) — that correlation is the bulk of the work, not the status field; (2) **handlers cannot see raw request bytes** (`ApiContext.body` parses immediately and the stream is single-read), so signature verification needs a new `rawBody` seam — re-serializing the parsed map would break signatures non-deterministically; (3) the store already updates in place, so the status write itself is cheap. Decided in-plan: our-id-embedded correlation (no SPI break), a new `DeliveryReceipt` store (`Notification` gains no fields), SendGrid Ed25519 + generic HMAC adapters, and **no** auto-disable on bounce (a separate call) | `okf/…/events-metrics.md` |
...
| AGT-6a · A4 | Read-only "explain this screen" on the remaining panes | **OPEN, cheapest breadth win.** Needs **no new backend** — `POST /agent/tools/{name}` gates on `ToolSpec.mutating()`, so the read belt (`glossary_lookup`, `status_get`, `timeline_build`, …) is already reachable |
...
| Delivery-status webhooks | D8 (answered + now planned) · §4 Notifications row · `superpower/delivery-status-webhooks-plan.md` |
