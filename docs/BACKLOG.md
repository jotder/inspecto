...
|---|---|---|
| **Pipeline definition drawers (P4)** | **B1's mapped-row preview has no consumer** — `POST /config/preview/schema` returns `mappedColumns`/`mappedCount`/`mappedRows` when the draft carries `mapping.rules`, but the Load pane cannot render them: it needs sample rows and the editor has **no sample thread** (`DefinitionStateService` is onboarding-only). Decide between wiring the thread into the editor (natural in P6, where wizard and editor state merge) or leaving the preview server-side. ⚠ Not a defect — an unconsumed capability. · The Load drawer offers `DIRECT`/`EXPR` only; `CONCAT_DT`/`FILENAME_DATE` are preserved when already authored but cannot be created there (they carry source semantics the grid has no affordance for). | `okf/frontend/features/pipelines.md` |
