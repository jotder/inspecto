...
**Open:**
- **`SpacesService.reconcile` — ⚠ CONTAINED 2026-07-28, STILL OPEN.** `refresh()` now coerces a non-array
  `GET /spaces` body to `[]` with a `console.warn` (pinned by a spec), so bootstrap no longer throws. **This
  is containment, not a fix, and the row stays open**: the root cause of the observed non-array is still
  unknown, and a warn-and-continue trades a crash for a wrong state that a shift has to notice in the
  console. Close it only when the source of the bad body is identified. Original finding follows.
