...
| **Grammar config** | Unification SHIPPED 2026-08-04 (one `<inspecto-grammar-editor>` for both hosts, `ParserConfigDialog` → `GrammarEditorDialog`, inline `parsing:` by default with explicit extract to a `grammar` component). Plan archived to [`archived-documents/plans-archive/grammar-config-unification.md`](archived-documents/plans-archive/grammar-config-unification.md). Open: **the unknown-`use:`-prefix refusal is still unbuilt** — nothing reads `node.use()` for validation (`ComponentRegistry.resolve/isKnown` has **no caller**), so a non-grammar `use:` binding with a typo or an unregistered id fails silently at engine load rather than at save time; it was blocked on the `connection/` vs `connections/` vocabulary split, fixed 2026-08-04 (`375757ae`), so only the refusal itself remains · **the slice-6 live browser smoke was never run** — `GrammarEditorDialog` shipped verified by the full reactor suite plus `grammar-editor.dialog.spec.ts` (10 cases: inline-default save, extract, bound-edit round-trip, unbind, the legacy `parser_type` read, the plugin-save refusal), but the Browser pane denied navigation to the offline preview that shift, so a real `ng serve` round-trip (inline edit → save → `parsing:` in the file; extract → save → `use: grammar/<id>` survives a reload) is still outstanding | `okf/frontend/features/grammar-config.md` |
