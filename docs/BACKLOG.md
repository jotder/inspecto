...
- M — `TYPEFLOW-DATASET-COLUMNS-1` steps 3+4 (⚠ relies on the dead-seam verdict (2) = keep `temporalColumn`) —
  **the M tier is otherwise DRAINED (2026-09-16)**: `AGT-5` → BLOCKED-EXTERNAL (no `PlatformBuilder` seam) · the
  parity gate → RUN, NOT MET (two graph-lane gaps named on the Pipeline graph row) · pre-materialise cap → §1 owed input.
