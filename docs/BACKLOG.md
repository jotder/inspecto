...
|---|---|---|
| **AUTHOR-SCHEMA-1 — schema/mapping/transform authoring UX is close but not complete** (documented 2026-09-03; **its first plan — validation gates over the existing surface — was SUPERSEDED the same day** by a redesign: [`superpower/parse-pane-redesign-plan.md`](superpower/parse-pane-redesign-plan.md) + [`superpower/sql-transform-v1-plan.md`](superpower/sql-transform-v1-plan.md), both ACTIVE. Four operator decisions are recorded in those plans — do not re-ask. Of the 8 gaps below, the redesign absorbs 1/2/4-partial/7; gap 3 (join reference-existence) and the schema-drift-into-mapping check remain valid small follow-ons; 5/6/8 are dropped as premature for a surface being redesigned.) | 8 confirmed gaps against a complete end-user authoring experience: (1) no live/as-you-type validation or preview for `EXPR` mapping rules (bare free-text input, zero syntax checking); (2) no expression-builder UI anywhere — no function/column picker, CodeMirror exists in the codebase but is wired only to the unrelated SQL workbench and enrichment editor, never to `EXPR`; (3) no reference-existence or `on`-column-existence validation before saving a `transform.join` node — `reference` autocomplete only suggests, never constrains; (4) no automatic schema-drift detection linking a schema rename/retype back to mapping rules that reference the old column (the only drift warning is generic parse-vs-schema mismatch prose, and it never names the broken rule); (5) mapping/join preview ("Test mapping" / "Test this Step") is opt-in, depends on stale sample rows from an earlier Test-Parse, and doesn't auto-recompute after an edit; (6) no drag-and-drop field mapping, and auto-map-by-name only fires once on an empty map node; (7) a `DIRECT` mapping rule referencing a stale/renamed column silently defaults to VARCHAR (`TransformCompiler.direct()`) instead of erroring at save or test time; (8) two disconnected test/preview mechanisms — drawer-level buttons vs. the more powerful canvas-level `RunToHereDialog` — with no cross-linking. | [`frontend/features/schema-mapping-authoring.md`](okf/frontend/features/schema-mapping-authoring.md) (UI/UX truth) + [`backend/engine/catalog-vs-executors.md`](okf/backend/engine/catalog-vs-executors.md) (engine truth for the same node types) |
