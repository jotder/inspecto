...
> **36<!--count:backlog-rows--> rows: 0<!--count:backlog-p1--> × P1 · 22<!--count:backlog-p2--> × P2 · 14<!--count:backlog-p3--> × P3** —
...
- **P2** · `AI-ASSIST-SCHEMA-DIALOG-1` — **re-enable inline AI assist for `schema` inside `SchemaEditorDialog`** (design S1 / Option C in `superpower/ai-drafting-non-schema-design.md`). Filed 2026-09-25 by operator decision D2: cheap and low-risk, but it does NOT answer the `transform` ask, which is why it is its own row. → `okf/frontend/features/inline-ai-authoring.md`
