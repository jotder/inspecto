...
| State | P2 rows |
|---|---|
| **Startable now** — no gate, no owed decision | X4 replay residuals — a replay button on Run Detail, the crash-mid-replay claim, auditing the actor (§3.2) · AI drafting for `transform` (§3.1, every call answered 2026-09-25) · Bundle "load as draft" (§3.7, every call answered 2026-09-25) · X1 retry affordance + per-pipeline `processing.retry` (§3.2) · policy-authoring save-time guards F1–F9 (§3.8) · parser plugins P2–P4 (§3.3) |
