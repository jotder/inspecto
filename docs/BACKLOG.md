...
- **P2** · **`PIPELINE-NODE-TEST-STATE-STALE-1` — the pipeline editor reports "not yet tested" on a Pipeline that has demonstrably run.** All four nodes rendered *"<node>: not yet tested."* while the Pipeline had three completed runs, four `SUCCESS` batches and 1219 rows in its own status store. The per-node signal is dry-run state only and never reconciles with `/provenance`, which the editor already calls for the last-run overlay. A trust surface that reports *untested* for a Pipeline in production teaches the reader to ignore it, which costs more than showing nothing. Remedy hypothesis: fold batch provenance into the node state so "tested" means *ran*, not *dry-ran in this tab*. → `superpower/postmed-xdr-pipeline-build.md`.
- **P2** · **`DRYRUN-SEEDS-AFTER-PARSE-1` — dry-run cannot exercise the parse stage, and its sample rows must be hand-typed as JSON.** The panel states its own scope (*"a bounded sample through the transform→sink subgraph"*) and works — two seeded rows produced `map → data=2 → sink → 2 row(s)`. But the delimiter, header handling, comment character, quoting and `null_strings` are **upstream of the seed**, and for a delimited feed those are the only genuinely risky decisions; everything downstream was 1:1 mapping. Meanwhile the sample is a JSON array the author types by hand (~850 characters for an 18-column feed) while correctly-typed rows sit in the Pipeline's own inbox and store. ⚠ A builder that cannot test the stage where the mistakes live is a diagram editor. Needed: run the real parse frontend over the first N lines of a chosen inbox file and show the typed columns plus rejects, and seed from an inbox file / last batch / Dataset. ⛔ A client-side approximation that disagrees with the engine would be worse than nothing — it manufactures the same false confidence as `PIPELINE-NODE-TEST-STATE-STALE-1`. → `superpower/postmed-xdr-pipeline-build.md`.
...
- **P3** · **`DUPLICATE-CHECK-GRAIN-UNSTATED-1` — `processing.duplicate_check` is file-grain and its name does not say so.** With `duplicate_check.enabled: true` an exact duplicate RECORD passed straight through to the store (verified: one `REC_SEQ` present twice). That is correct for the key as built — it is marker/path-keyed, per file — but the name reads as if it covered record duplicates, and a re-emitted interval file is the normal failure mode for a mediated feed. Record-grain dedup needs an explicit dedup Step, which in turn needs top-level `output_store:` to arm. State the grain beside the key in the ingestion concept and name the alternative. → `superpower/postmed-xdr-pipeline-build.md`.
- **P3** · **`PALETTE-ADD-DROPS-ORPHAN-1` — adding a Step from the palette drops a DISCONNECTED node, and there is no insert-on-edge.** With a node selected, *Add Row filter* created `transform_filter_1` unconnected below the chain. Validation caught it (*"has no input connection."*) so it **fails safe**, but inserting one Step mid-chain is then three graph operations (connect upstream, connect downstream, delete the superseded edge). Either wire an added Step after the selection, or accept a drop onto an edge and split it. The typed config pane itself is good — the filter offered `Row predicate (after parsing)` and accepted a predicate. → `superpower/postmed-xdr-pipeline-build.md`.
- **P3** · **`GRAPH-READ-SHAPE-NOT-WRITE-SHAPE-1` — `GET /pipelines/{n}/graph` is not a valid body for `PUT` on the same path.** Taking the graph exactly as the server served it and PUTting it straight back is refused **422** (`NO_PERSISTENT_SINK`, `PARSER_NO_SCHEMA`) because `/graph` is a display projection carrying no node `config`. The editor knows to load **`/graph/raw`** instead (confirmed on the wire), so this bites other clients, not the UI. ✅ It fails closed — the refusal reported `written:false` and the config was byte-identical afterwards. Either give the projection its own path or make one shape serve both. → `superpower/postmed-xdr-pipeline-build.md`.
