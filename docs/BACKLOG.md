...
> **41<!--count:backlog-rows--> rows: 0<!--count:backlog-p1--> × P1 · 21<!--count:backlog-p2--> × P2 · 20<!--count:backlog-p3--> × P3** —
> derived by `tools/check-doc-counts.mjs` from the rows between `## 3.` and `## 6.`; never hand-count.
> ↔ **42 on 2026-09-26**: P1 `ASSURE-IMPACT-LEDGER-1` closed (wave 1.3 shipped — assurance wave 1 complete); its residuals re-filed as P3 `ASSURE-IMPACT-LEDGER-RESIDUALS-1`.
...
> ⬇ **42 → 41 on 2026-09-26** (telco shift 3 integration of five backlog lanes): closed P2 `DUCKLE-C1-DATASET-FRESHNESS-1`
> (owner-routed alerting; as-built `okf/capabilities/studio/studio.md` §3.4) and P2 `VIRTUAL-DATASET-SQL-1` (a virtual
> Dataset's SQL is its server relation); GAP-4 closed with a `memory_limit` code default, so *Deployment topology gaps*
> became P2 `DUCKDB-INMEMORY-SCRATCH-UNCAPPED-1`; P2 **D-8 XLSX export** narrowed to one `package.ps1` run; filed P3
> `VIRTUAL-DATASET-SQL-RENAME-1`.
...
| State | P2 rows |
|---|---|
| **Startable now** — no gate, no owed decision | assurance wave 2 (wave 1 has shipped), in order: `ASSURE-MAKER-CHECKER-1` (§3.7) → `ASSURE-ACTION-REQUESTS-1` (§3.5); `ASSURE-ENTITY-LISTS-1` ⏸ on hold for Link Analysis |
