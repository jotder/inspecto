...
| **Delimited error-handling knobs — open items** (shipped `d6cd55b7`/`83b46a79`) | The six knobs (`ignore_errors`/`null_padding`/`store_rejects`/`rejects_table`/`rejects_scan`/`rejects_limit`) are engine-honoured, published and offered on the delimited Robustness tab, all tri-state. **Open:** **(a) ⚠ The two knobs whose ENGINE default is ON render as OFF-looking toggles** — a tri-state control has no third visual state. Workaround: the help text leads with "Engine default is ON". Restriction: the toggle alone reads as the OPPOSITE of the truth; label inversion is unavailable because the spec key IS the config key. Fix = a 3-way picker per knob (operator decision). **(b) Offered on the DELIMITED tab only.** The engine applies them to every `csv_settings`-based frontend, so fixed-width/JSON/text-regex Robustness tabs silently lack them; authoring requires hand-editing. **(c) `null_padding`'s per-frontend default divergence** (false delimited / true line-readers) is preserved in the engine but not surfaced anywhere in the UI. | `okf/backend/config/parsing-options-reference.md` |
