...
- **Config-declared paths resolve unjailed against the server CWD** — one systemic pass, **not** the
  ...
  - ⚠ **The blocker:** `PipelineConfigParser.parse(Map, String)`
    (`inspecto-etl/…/PipelineConfigParser.java:52`) receives no space/root handle and `PipelineConfig`
    carries no root field; likewise `Asn1RecordIngester.ingest(File, RecordSink, int, PipelineConfig)`.
    So the four config-layer reads — `parsing.plugin.segments[].schema` (`:273-277`),
    `processing.schemas[].schema_file` (`:305-309`), legacy `processing.schema_file` (`:341-345`),
    `processing.grammar` (`:857-861`) — plus `Asn1RecordIngester:96` (`ingester_config.grammar`)
    **cannot be jailed without threading a root through public config-layer signatures.** That is the
    real cost of this item, and why it is not a patch.
