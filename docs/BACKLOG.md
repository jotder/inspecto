...
> P2 `ASSURE-MAKER-CHECKER-1`, `ASSURE-CONTROL-LISTS-1`, `ASSURE-ACTION-REQUESTS-1`.
...
| State | P2 rows |
|---|---|
| **Startable now** — no gate, no owed decision | assurance wave 2, in order once wave 1 lands: `ASSURE-MAKER-CHECKER-1` (§3.7) → `ASSURE-CONTROL-LISTS-1` (§3.5, name owed first — D-P10) → `ASSURE-ACTION-REQUESTS-1` (§3.5) |
...
- **P2** · `ASSURE-CONTROL-LISTS-1` — **allow / block / watch lists as a component kind (WS-12, wave 2.2 of [`superpower/assurance-capability-plan.md`](superpower/assurance-capability-plan.md)).** After `ASSURE-MAKER-CHECKER-1`. Typed keys (msisdn, imsi, imei, iccid, prefix / range, country, IP, CIDR, instrument token, BIN …), entries with reason, added-by and expires-at, a Parquet sidecar for 10⁵ entries, range / CIDR matching (the equal-key join cannot). Expiring (≤ 24 h) entries apply at once and are reviewed after (D-P5). 🔴 **D-P10: the Link Analysis `LA-17` Entity List is the same concept — ONE kind, name still open; settle it before either builds.**
