...
*(Priority 0 was SEC-INCIDENT-1 — **CLOSED BY DECOMMISSION 2026-08-29**, the system no longer exists so
rotation is moot. The record stays in §5; one action falls due outside this repo, named there.)*
...
4. **Dependent chains (sequence behind a build, no longer behind a decision):** ~~Lens Access P3~~
   **ALREADY SHIPPED — this entry was STALE (corrected 2026-07-27).** Lens Access P3 *is* RBAC **R2**,
   ...
   built by a shift. ⚠ **Corrected 2026-08-28: that is true of the CERTIFICATIONS, not of every
   workstream.** **C2 (the control matrix) was pure repo work and SHIPPED** →
   `compliance/controls-matrix.md`; its §4 lists nine gaps (G1–G9), of which **G2/G4/G6/G7 are
   unblocked writing/CI work** owned here. **C1 genuinely IS org-gated** — a scope statement needs
   facts the repo does not hold; ⛔ do not generate one from the matrix. ·
...
| D11 | **Conservative fixed per-instance cap + spill, on by default.** **✅ SHIPPED 2026-08-26** as the measured pair — `memory_limit=2GB` **+** `maxConcurrentRuns=4`, both on by default, both owned by the **server configuration** (`scheduler.toon` → `GET/PUT /system/scheduler`) and surfaced in the UI at **Settings ▸ Scheduler ▸ Resource caps** with `file`/`property`/`default` provenance. ⚠ The shape was forced by `SchedulerRoutes`' ban on split ownership (⛔ no key served by the settings tier may also be read from `-D` at use time), so both use-time reads moved behind installed values: `DuckDbUtil.memoryLimit(configured)` (per-pipeline > installed > `-D` > DuckDB default) and `JobService.setMaxConcurrentRuns` (hot-resizable; a shrink drains; `0` still = unbounded). ⚠ Two things deliberately NOT done: preview/dry-run connections stay **uncapped** (bounded samples — only `EnrichmentEngine`, `PipelineJobRunner`, `BatchIngestStrategy` resolve a limit), and `max_temp_directory_size` still gets no default. ⚠ `JobService` is **per space**, so hosted multi-space worst case is the bound x space count; single-tenant (default) is the process-wide bound the arithmetic assumes. ⚠ The UI seeds its inputs **only from a stored value** — pre-filling an inherited one would make the next Save silently take ownership from the `-D` flag. Follow-on 2026-08-26: the **consignment cap on the same form had exactly that provenance-seizure defect** (required in every PUT body, always written, seeded from the effective value) — fixed by bringing it onto this row's pattern: nullable end-to-end, merged per `body.containsKey`, provenance keyed on the stored value not file presence, both tiers (`writeSystem` + `writeSpace`); the `PUT {}` = 422 test pin was consciously revised to a 200 no-op. History: declined by the operator 2026-07-25 ("spill routing only"), measured 2026-07-27 | `okf/…/duckdb.md` |
| D12 | **Chunking on with a large threshold.** **SHIPPED 2026-07-25** at **8 GiB** (`processing.chunking.max_file_bytes`, was `0`/disabled) — far above routine inputs so normal workloads never change shape. ⚠ It was meant to land *after* D11, because a memory cap turns the failure mode into "spill" and makes a high threshold safe. D11 was declined, so chunking was for a while the only bound on a pathological single file. ⚠ **Caveat stale since 2026-08-26:** the resource-caps work (the *other* D11 numbering space, §1) shipped `memory_limit=2GB` per instance + `maxConcurrentRuns=4`, restoring the memory bound this caveat said was missing | `okf/…/duckdb.md` |
...
| NFR-7 | Compliance certifications | PARTIAL (not started) — **UNGATED 2026-07-25 (D1): runs in parallel, C1 is not a predecessor.** Real remaining constraints are the Type II observation window (needs CC6 live — it is) and external-party pacing. **C2 SHIPPED 2026-08-28** — `compliance/controls-matrix.md`, the single mapping table the ISO SoA and FedRAMP responsibility matrix export from; its §4 consolidates gaps G1–G9 — **G2, G4 and G7 CLOSED 2026-08-28**: the two auditor runbooks under `compliance/evidence/`, and `tools/check-dependencies.mjs` + `tools/dependencies.lock` wired into `ci.yml`. **G6 was WRONG as written** — `docs/ops/backup-restore-runbook.md` already exists; only an RTO/RPO statement and a recorded drill are owed. Still open: **G1** SBOM (⚠ per packaged BUNDLE, not the reactor), **G3** key custody, **G5** retention config, **G8** (RBAC R5), **G9** FIPS, **G10/AUDIT-CSV-1**. 🔴 The matrix corrected the plan twice: **release checksums + GPG signing were already shipped** in `package.ps1` though C3 listed them as work, and the **SBOM is confirmed still absent** (prose only). C1 stays org-gated. `superpower/compliance-certifications-plan.md` |
...
|---|---|---|
| ~~**AUTHOR-SCHEMA-1 — schema/mapping/transform authoring UX is close but not complete**~~ **CLOSED 2026-09-04** — its first plan (validation gates over the existing surface) was SUPERSEDED 2026-09-03 by the authoring redesign, and the redesign SHIPPED 2026-09-04: [`parse-pane-redesign-plan.md`](archived-documents/plans-archive/parse-pane-redesign-plan.md) (`d012f721`) + [`sql-transform-v1-plan.md`](archived-documents/plans-archive/sql-transform-v1-plan.md) (`98ffc90b` engine, `7e13dd82` pane), both archived with their grounding [`sql-only-transform-feasibility.md`](archived-documents/plans-archive/sql-only-transform-feasibility.md). Of the 8 gaps: 1/2/7 absorbed by `transform.sql` (Simple mode never shows SQL; a binder error names the stale column); 4 partially absorbed; 3 (join reference-existence) and the schema-drift-into-mapping check survive as follow-ons; 5/6/8 dropped as premature for a redesigned surface. The eleven operator decisions (Parse D1–D4, SQL D3–D7) live in the archived plans — do not re-ask. Every open remainder is the next row. | — | [`frontend/features/schema-mapping-authoring.md`](okf/frontend/features/schema-mapping-authoring.md) §0/§6 · [`frontend/features/grammar-config.md`](okf/frontend/features/grammar-config.md) · [`backend/engine/catalog-vs-executors.md`](okf/backend/engine/catalog-vs-executors.md) |
| **AUTHORING-WIDE-1 — the authoring panes are drawn for a 7-column file and this product's feeds are not that shape** (filed 2026-09-04 from driving the clickable mockup; D8–D10 DECIDED, none built) | 🔴 **Root cause worth keeping: the mockup's sample was chosen to flatter the design.** Both boards were drawn over a 7-column orders CSV, where "one row per field" reads as clarity; at the few-hundred fields an ASN.1 / fixed-width / network feed actually carries, the same table is a wall. The evidence was already in the repo (the parser lanes, the 121-processor catalog) and went unapplied. **(a) D8 — Parse must NOT drop columns; `transform.sql` owns exclusion.** Parse settles what a column *is* (exists · name · type · synonym); leaving a field out of the output is Transform's job and only its job. ⚠ The reason is COST, not tidiness: excluding in Parse edits the **schema**, which `SchemaCompatibility` gates BACKWARD — undoing it can be refused or force a re-test — while excluding in Transform is a SELECT clause, reversible for free. Two affordances that looked identical had very different costs; stop offering the expensive one. 🔴 Deliberate knock-on: the stored schema becomes the FULL column set, so the Catalog lists columns that never reach a sink — anything reading the schema as "what lands in storage" reads it wrongly (schema describes the FILE, the SELECT describes the OUTPUT). Check `schema-fields-editor` for an include/exclude control and remove it if present. **(b) D9 — both field tables built for a wide feed:** search over name + synonym, view-only filter chips WITH COUNTS, page sizes 10/20/100, and 🔴 `#` = position in the FULL list, never the row index in the current view (on Parse it is the physical column position in the file — renumbering it under a filter is a lie; page 2 starts at #11). The sample preview renders only the fields on the current page and says so. **(c) D10 — the Transform grid opens on *Changed*, not *All*,** with a banner stating the implication ("N other fields pass through unchanged — nothing to do for them") and a **Show them** action; the grid's model is *everything passes through; show me what I changed*, never *enumerate every field*. ⚠ This INVERTS the Simple-vs-SQL argument that justified the grid: the grid is O(n) in fields, SQL is O(1) in fields and O(k) in changes, so for wide data `SELECT *` with three overrides is genuinely the simpler surface. Weigh that before any feature that needs the grid to list all fields. **Reference:** clickable mockup Artifact `b5e7ec6c-5bb2-467a-a513-93bfda987f3d`; artboards in the session scratchpad `design/` (`Main.dc.html` = Parse, `Transform.dc.html` = Transform), both now drawn over 65 columns. | [`frontend/features/schema-mapping-authoring.md`](okf/frontend/features/schema-mapping-authoring.md) §7 (D8–D10) · [`frontend/features/grammar-config.md`](okf/frontend/features/grammar-config.md) (Parse pane as-built) |
| **RECORD-TRANSFORMER-1 — what remains after the Record Transformer took over the projection slot** (filed 2026-09-05; **(a)(b)(f) CLOSED 2026-09-05 — `transform.map` DELETED**, plan `plans-archive/delete-transform-map-plan.md`: the four EXPR schemas migrated via the block-list TOON form, `date.concat_parts` + `date.from_filename` landed, the lift always emits `transform.sql`, `rules[]` converts at read time; still open: (c) audit narrowing, (d) ingest cascade, (e) — now moot, the convert action went with the Load pane; the plan shipped as `dd4b8c37`·`7927d703`·`2f000e22`·`7b7c7a5e`·`95164683`·`f1d538ca` and is archived as `plans-archive/record-transformer-replaces-map-plan.md`; as-built in the OKF home on the right) | **(a) 4 schemas still on `rules[]`** — `demo/orders`, `demo/payments`, `ucc/sites`, `_templates/orders-starter/orders`. All four carry the 6 `EXPR` rules, and `MappingMigrator` REFUSES them by name and row rather than half-migrating: `EXPR` needs a nested `args.expression` that a flat TOON tabular row cannot hold. Closing it means teaching the migrator the block-list form (`fields[N]:` with one `- name:/fn:/args:` block per entry) — a bigger diff, but those four files are small. ⛔ Do not "solve" it by emitting `custom` from the tabular row: `args` cannot ride a 3-column row, and approximating loses the audit denominator. **(b) `CONCAT_DT` / `FILENAME_DATE` have NO catalog function**, so a schema using them is permanently unmigratable and `RecordTransform.fromMappingRules` refuses them. Two catalog entries (`date.concat_parts`, `date.from_filename`) with a Java+TS pair and a contract-test line would close it — that, plus (a), is what makes deleting `transform.map` a real option rather than a wish. **(c) Two-source audit narrowing:** a field's `from` is ONE column, so `text.join` tests only its primary input for non-blankness; `CONCAT_DT`'s audit covered both. Weaker, not wrong — document or widen `from` to a list. **(d) No ingest cascade** — one projection slot, and SQL cannot reference a sibling alias, so nesting or `custom` is the only route to a two-stage shape on ingest. `DataTransformer.materialize` already wraps its SELECT in a subquery for the row filter, so "wrap N times" is the shape of the fix. **(e) The Load pane's "Convert to a Record Transformer" action has specs but has never been clicked against a running server** — it re-types the projection slot, so the node's next edit opens in the Transform pane; that transition is unproven in a browser. **(f) `transform.map` is deprecated, NOT deleted** — deliberate: `mapping.rules[]` has ~20 readers and stays readable permanently, and `PipelineLift:313` still creates the slot node unconditionally for every stored pipeline. Deletion is a separate decision, gated on (a)+(b). | [`backend/engine/catalog-vs-executors.md`](okf/backend/engine/catalog-vs-executors.md) · [`frontend/features/schema-mapping-authoring.md`](okf/frontend/features/schema-mapping-authoring.md) |
| **AUTHORING-REDESIGN-1 — follow-ons and deliberate deferrals of the shipped authoring redesign** (filed 2026-09-04 when `98ffc90b` · `7e13dd82` · `d012f721` landed; as-built truth in the OKF homes on the right, decisions in the archived plans. **Re-scoped the same day** when the operator chose SQL-first for the Transform Step — the Simple five-verb grid, its generator and the D5/D6 lock were removed; none of (a)–(m) were grid-specific, so all stay, with (b) and (c) re-framed below) | **Transform Step (`transform.sql`):** ~~(a) **`/describe` route**~~ **✅ SHIPPED 2026-09-04**: `POST /components/transform/describe` (`ComponentRoutes.describeTransform`) over `TypeFlow.describe` — body `{sql, inputColumns}` → `{columns:[{name,type}]}`, 400 malformed / 422 guard-or-binder. It landed with the other `/components` previews rather than at the planned `POST /pipelines/authored/{id}/nodes/{nodeId}/describe`: it needs neither a pipeline nor a node, only the SQL and the upstream columns, and like the previews it reads and writes nothing (no write-root/path-jail gate applies). 🔴 `TypeFlow.describe` opens a PLAIN in-memory DuckDB connection — not the sealed `SqlSandbox` the executor and preview use — so the route applies `SqlGuard.check` itself; `DESCRIBE` never executes the plan but the binder still OPENS a `read_csv('…')` target to infer its schema. The pane's grid fills "Comes out as" from a 300ms-debounced call and shows DuckDB's binder message as-you-type; only a 422 blocks Apply. 🔴 Two more traps found by DRIVING it live: the pane must send the DECLARED upstream types (it assumed VARCHAR for every column, so `AMOUNT * 2` over a declared DOUBLE came back "No function matches … *(VARCHAR, INTEGER_LITERAL)" — a FALSE refusal that blocked Apply); and the driver's wrapper line ("Attempting to execute an unsuccessful or closed pending query result") led every message, so the first thing the author read was plumbing — `ComponentRoutes.duckDbMessage` strips it, pinned by test. ~~(b) **CodeMirror for the SQL**~~ **✅ SHIPPED 2026-09-04**: `pipeline-transform-sql-definition.component.html` now hosts the shared `<inspecto-sql-codemirror>` (`app/inspecto/data-table/sql/sql-codemirror.component.ts`) — the SAME wrapper the SQL workbench and enrichment editor use — instead of a plain `<textarea>`; no new CodeMirror setup was written. Verified live (Builder ▸ Edit ▸ Expression builder on `csv_example`): syntax highlighting and typing both work over the drawer's `EditorView`. The one behavior change: `.cm-content` renders each line as its own block element, so the seeding spec now joins `.cm-line` text with `\n` rather than reading a `<textarea>.value`. (c) **v2 — a smart table OVER the SQL (AST)** — now explicitly the future *structured* surface on top of the SQL the pane persists, never a second model beside it (`json_serialize_sql` → one row per output column with in-place alias/literal editing → `json_deserialize_sql`; quick actions add-TRIM / add-cast / rename as AST edits; `JOIN reference/x` inside the SQL). 🔴 **Precondition — a five-minute probe, not a design:** `json_serialize_sql` lives in the `json` extension and `SqlSandbox.seal()` sets `autoload_known_extensions=false`; `json` is normally statically linked in the JDBC jar, but so was assumed of `excel`, which is NOT. Measure on the SEALED connection first. **D4 SUPERSEDED 2026-09-05 (operator):** a bounded *reconciler* (`pipeline-transform-sql-reconcile.ts` — no AST, no extension, 21 specs) reads a flat `SELECT … FROM input` projection back into rows: catalog templates matched in reverse and RE-COMPILED to prove the round-trip (a recognition that would rewrite the SQL is not one — bare `CAST` stays custom), anything else a verbatim custom row, anything beyond a projection leaves the Fields tab disabled with the reason. The pane's `Fields | SQL` header switch makes the two peer views of one Step; switching never changes the SQL. This (c) AST table remains the way to make a `WHERE`/`JOIN` *structurally* editable, and the extension probe still gates THAT. (d) **v3 — macros as the UDF registry.** Measured 2026-08-29 (`duckdb_jdbc 1.5.2.1`): `CREATE MACRO` scalar + table works and SURVIVES the seal; `DuckDBConnection` has NO Java-side UDF API, so "UDF" = SQL macro only, per-connection — needs a component kind, a registry, and re-creation on every scratch connection (`EnrichmentEngine`, `PipelineJobRunner`, `BatchIngestStrategy`, preview). A real project, demand-gated. (e) **Column metadata (description / unit / classification) editing** belongs on the Transform pane (Parse D2) — the plan's "Describe the fields" section was NOT built because no backend home exists for metadata on a `transform.sql` node; until then the keys ride through the Parse pane's by-selector carry-through (`schema-fields-editor.component.ts:563-587`). Needs the backend contract first. (f) **`transform.join` reference-existence + `on`-column check** before save (survived from AUTHOR-SCHEMA-1 gap 3; `reference` autocomplete only suggests, `on` has no validator — unlike `transform.summarize`). (g) **Schema-drift-into-mapping check** (gap 4) — a schema rename/retype should name the `transform.map` rules / `transform.sql` SELECT columns that reference the old column; valid precisely because Layer 1 stays a declarative rule table. (n) **The fields grid was RESTORED 2026-09-04 (second flip, same day)** — the operator rejected the SQL-only Step (*"mapping with functions and parameters is not realistic"*), so `24171333`'s deletion is itself superseded: the grid is back over a **function catalog with typed parameters** (`sql-functions.ts`, ~20 functions across Keep · Text · Numbers · Dates · Logic · Convert · Custom), it persists `{ sql, fields }` so it round-trips without an AST, and D5/D6 are superseded (the catalog replaces the five verbs; persisting `fields[]` removes the need for a lock). Still open from it: ~~**(n1)**~~ **✅ SHIPPED 2026-09-04** — the legacy `transform.map` pane no longer marks a Step "unapplied" from a single click: seeding ALREADY-AUTHORED rules emits `dirtyChange(false)` explicitly instead of re-deriving it from the form's dirty flag, while the straight-through proposal for a node with no rules still arms Apply on purpose (both arms pinned by specs); ~~**(n2)**~~ **✅ SHIPPED 2026-09-04** — the recipe verb is **`sql`**, ordered between `transform` and `summarize` (`PipelineProjection.RECIPE_VERBS`, the TS `RECIPE_VERBS` mirror labelled "Transform (SQL)", and the regenerated `step-types.contract.json` — `mvn -o test -Dstep.types.write=true`, order pinned by `StepTypesContractTest`). `RecipeCompiler` compiles `sql: {sql, fields}` on the trunk and mid-branch (blank `sql` ⇒ `MALFORMED_STEP`), `RecipeConverter.sqlStep` converts it back, and `RecipeConverterTest.aConfigWithSqlStepRoundTrips` pins the round-trip that used to throw `UNSUPPORTED_STEP`. 🔴 **Mid-branch it compiles but does not ARM**: `RouteArming.BRANCH_STEP_KINDS` is `{FILTER, DEDUP, SUMMARIZE}`, so a route branch chaining a sql step is a save-time finding and an `IllegalStateException` from `PipelineConfig.prepare()` once active — deliberate (fail-closed until the branch walker runs one), and the compiler still accepts it so an inactive draft's Recipe round-trip stays lossless. **Open decision:** admit `sql` to `BRANCH_STEP_KINDS` (needs the branch walker to execute it) or keep the refusal permanently. **(n3)** ~~the legacy map pane hid a legacy `columns:` projection~~ **✅ SHIPPED 2026-09-04**, found while finalizing: `MAP_AUTHORED` is `{columns, rules}` and `RowShaper.columnsOf` prefers `columns`, but the Load pane only ever read `rules` — a lifted legacy node showed an EMPTY grid where the engine ran a real projection, and (worse) writing rules beside a surviving `columns` would have saved an edit the engine never runs. The pane now seeds from `columns` as DIRECT rules and an Apply that writes non-empty rules DELETES `columns`; an empty grid deletes nothing. **Parse pane:** ~~(h) **Stale-Apply race over `partitions[]`**~~ **✅ SHIPPED 2026-09-04** (the first of the two proposed fixes): `writeSchemaThenApply` re-READS the companion schema immediately before writing and takes `partitions[]` plus every unmodeled top-level key from that read, falling back to the load-time seed only when the read fails. The keys this pane owns (`raw`, `mapping`) are still written from the on-screen grid — last write wins there, by design. (i) **Per-row "sample resolves to" line** (mockup; R12) — not wired: no host resolves a sample against an `AttributeSpec` today (the detected delimiter is the obvious first). (j) **`engine: auto` stored literally vs the spec's blank Automatic option** — `PipelineConfigParser.java:974` treats an absent `engine` and a literal `auto` identically (`getOrDefault("engine","auto")`), so a lift that maps `auto` → blank on load and blank → absent on save is safe; the spec deliberately has NO default (`parsing-attributes.ts:301` — do not "fix" it into `default: 'auto'`). Decide whether the lift should normalise a stored literal `auto` away. ~~(k) **Collector pane's own "Additional config" chevron**~~ **✅ SHIPPED 2026-09-05** — plain header like the Sink pane, shown only when there are rows; the Collector's three schema-forms (collector block · Duplicate handling · Unpack) are `[flat]` so the Advanced gear is gone; and `app-pipeline-extra-config` on EVERY pane now renders property rows (label · value · pencil; a boolean is a toggle) instead of outlined Material fields — the operator's "not like the pen-style edit" finding. (l) **`GrammarEditorDialog` for grammar-bound (`use: grammar/x`) / dangling-binding / binary fixed-width parse nodes** (`openNodeConfig`, `pipeline-editor.component.ts:2126-2154`) — the drawer already serves every drawer-capable delimited node (R7). Removing the dialog is a Components-registry decision (where does a grammar-bound node get edited?) — not made. (o) **`docs/superpower/step-workbench-design.md` is now partly superseded** — its §0 finding ("TypeFlow's DESCRIBE path has zero production callers") died when `/components/transform/describe` shipped, and the Transform pane covers the field-list + sample-test half. Unbuilt: the one-surface workbench (input-relation picker, column filter, grouping beside the field list) and `RowShaper.fuse` (still caller-less, still untested). Its header now says so. **Operator call:** distil the unbuilt design into an OKF concept + this row and `git mv` it to `plans-archive/` (the lifecycle default for a plan that is not in flight), or keep it in `superpower/` as scheduled work. **Vocabulary:** (m) **`GET /provenance/batches?flow=` and `GET /provenance?flow=&batch=`** (`JobRoutes.java:289-302`) take a query parameter literally named `flow` — a banned term in product code. It is exempted by inline `// vocab-allow` comments, and the guard's Java pass (`tools/check-vocabulary.mjs:318`, `SOURCE_GLOBS`) would otherwise flag it; the UI callers send it. Renaming is a wire change (`pipeline=`) with a deprecation window — file under the API vocabulary rollout (GLOSSARY §13), not a quick edit. | [`frontend/features/schema-mapping-authoring.md`](okf/frontend/features/schema-mapping-authoring.md) §0 (Transform pane) · [`frontend/features/grammar-config.md`](okf/frontend/features/grammar-config.md) (Parse pane) · [`frontend/features/pipeline-editor.md`](okf/frontend/features/pipeline-editor.md) · [`backend/engine/catalog-vs-executors.md`](okf/backend/engine/catalog-vs-executors.md) (engine + parked v2/v3 facts) · [`backend/pipeline-graph/pipeline-config-keys.md`](okf/backend/pipeline-graph/pipeline-config-keys.md) |
| ~~**MOCK-LIFT-1 — the offline lift invents a third destination**~~ **CLOSED 2026-08-29** (found and fixed the same day; the description below is kept because it names the trap) | The mock's TS lift (`inspecto/mock/pipeline-editable.ts`) ALWAYS emits a trunk `sink` node from `output:`/`dirs.database`, then adds one node per `sinks[]` entry whose database differs from `dirs.database`. The Java parser does the opposite: an authored `sinks:` block IS the destination list, and the `output:` shorthand is synthesised **only when `sinks:` is absent** (`PipelineConfigParser` — "Absent ⇒ PipelineConfig synthesises the single-`output:` one-element shorthand"). They agree only while a config follows the convention that `sinks[0].database == dirs.database`. **Repro:** author a pipeline whose `sinks:` entries are all different from `dirs.database`, open it in the editor and save — the lower writes the invented trunk back, so 2 destinations become 3 and the pipeline silently gains a write to `dirs.database`. Offline only (the server's lift is correct), but it is the "a mock must never diverge from the server" rule failing in the direction that corrupts config. **Second half, same root cause:** the extra sinks get the mock's own ids (`sink_2`, `sink_3`) where the Java lift emits `sink__d<i>`. So a per-Step disable authored OFFLINE writes `processing.disabled_steps: ["sink_2"]`, an id the server's `StepDisableArming` cannot match — the offline editor authors a config the real backend refuses. **Fix:** mirror the Java emission — when `sinks:` is authored, emit exactly one node per entry with the `sink` / `sink__d<i>` id grammar and no trunk; ⚠ that renames the extra sinks' ids (`sink_2`/`out_2` today), so `pipeline-editable.spec.ts` and any spec pinning those ids move with it. 🔴 **And the naive fix introduces a WORSE corruption** — scoped 2026-08-29 and not attempted for that reason: the mock's LOWER derives `dirs.database` (and `backup`/`temp`/`threads`/`batch`/`intake`) from `primarySink`, i.e. it assumes "the primary sink IS `dirs.database`". Drop the trunk node and the lower starts writing `dirs.database = sinks[0].database`, losing the real one. A faithful fix has to change the mock's sink MODEL on both sides (lift and lower) so `sinks:` is the destination list and `dirs.database` is independent — its own slice, with its own grounding, not a one-line emission tweak. The lower is at least id-agnostic (it keys on `database`), so the rename itself is safe. **Design, grounded 2026-08-29 — do this, not the one-liner:** ⚠ `dirs.database` is load-bearing EVEN when `sinks:` is authored — `IngestSinkWriter` re-roots each destination as `dest.database() + (dbDir relative to dirs.database)`, so it is the fan-out's base, not a destination. The mock currently conflates the two by putting `database: dirs.database` on the trunk node and reading `dirs.database` back off `primarySink` (`pipeline-editable.ts:396,779`). The faithful model separates them: (1) the LIFT emits the `output:`/`dirs.database` shorthand node ONLY when `sinks:` is absent, else one node per entry with the `sink` / `sink__d<i>` ids; (2) the config-level keys the lower reads off `primarySink` (`backup`/`temp`/`threads`/`batch`/`intake`/`store`) move to the first emitted sink, but (3) the LOWER must stop deriving `dirs.database` from a sink node when a `sinks:` block exists — it keeps the raw value instead. Pin the whole thing with a round-trip spec over a config whose `sinks[]` all differ from `dirs.database`, which is the case that exposes it. | [Pipeline graph design](okf/backend/pipeline-graph/pipeline-graph-design.md) |
| **EXECUTION-RESIDUALS-SKETCHES — X4 / X5 (filed 2026-09-02 when X1-X3 were built)** | The plan's three buildable items SHIPPED 2026-09-02 (X3 PARK-1(a) `ca3e885c` · X2 cross-lane provenance `452fdc7a` · X1 bounded COMMIT retry `d54772f8`); the plan is archived at [`archived-documents/plans-archive/execution-residuals-plan.md`](archived-documents/plans-archive/execution-residuals-plan.md). Two design SKETCHES remain, neither scheduled: **X4 record-level replay from quarantine** — whole-Consignment `reprocess` stays the affordance; if record grain is ever prioritized, the shape is sidecar error manifests naming offset/reason beside the quarantined file plus an all-or-nothing vs eject-and-continue policy as per-pipeline CONFIG (the B side already exists as `errors/*_errors.csv` + kept rows). ⛔ No build without a driver. **X5 one-Consignment cross-lane drill-down + the StepInfo envelope** — the data half is now X2's `derivedFrom`/`derivedRuns`; the remaining idea (steps exchange a ~1 KB pointer+schema+diagnostics envelope, failure routed by PORT) belongs to Phase-7 convergence, which stays major-bump gated (spec §13 D2). **X1 deferrals, same home:** a per-pipeline `processing.retry` block (rides the node-attributes + step-types contracts — regen both), and an operator cancel / retry-now affordance (today: delete the sidecar under `<status_dir>/retries/`, or `reprocess`). | [Execution lanes](okf/backend/pipeline-graph/execution-lanes.md) · [db-layer §3.5](okf/backend/engine/db-layer.md) · [operations-reference](okf/backend/build-run/operations-reference.md) |
| **PARK-1 — park/drain residuals (ELT Phase 4 S4, D-13; filed 2026-08-29 when S4 shipped whole)** | Three deliberate deferrals, all decision-gated, none blocking: (a) ~~no manifest-level park detail is served~~ **CLOSED 2026-09-02 (execution residuals X3)** — premise re-driven and found TRUE, then closed WITHOUT a new route: `GET /runs/{name}/batches` merges the manifest's `parkedAt`/`parkedTables` onto each `status=PARKED` row at the route layer (`RunRoutes.withParkDetail`, reading the same `dirs.manifestsDir` the drain does), and the Batch detail dialog renders them as a *Parked at* table. ⚠ The row's own remedy (“needs a new read route”) was wrong — and merging at the ledger would have meant a column across the `_batches_` header's five mirrors and both status backends. (b) **A batch with unpack-EXPANSION members is REFUSED, not drained** — its archive original stayed in the inbox (moving a shared original would strand sibling batches), so it re-expands and re-parks as a fresh batch each cycle; draining the stale one would commit rows the new one is about to write. Making those drainable needs the shared-original lifecycle solved first. (c) ~~The offline mock seed has no `route:` pipeline~~ **CLOSED 2026-08-29** — `cdr_route_demo` seeded (MOCK_STORE_KEY v24); driving it found a real defect in the switch's own visibility rule (see the OKF concept). The original text: the canvas Step switch could not be exercised in the preview (it is unit-tested at both levels); adding one to the seed would close that. | [Per-Step enabled — park and drain](okf/backend/pipeline-graph/step-park-drain.md) |
| **EXPORT-1 — outbound object-storage export (S3 / HDFS), filed 2026-08-28 from an operator question** | Open, unscheduled — "keep the discussion, we will work on later." Grounded posture + full option table live in the OKF concept: nothing pushes outbound today (all connectors are acquisition-side; `DuckLakeRegistrar` registers local paths, never relocates bytes), a full space on S3 is recommended AGAINST (the engine's atomic-rename/marker/jail semantics don't survive object storage — space local, data tier exported). Sequence of record: operator `aws s3 sync`/rclone of `data/<store>/database/` first (zero code, works today); build the **push post-action** (outbound mirror of the connector SPI, reusing `AwsSigV4` + the object-store HTTP plumbing) only if demand shows; HDFS only via an S3-compatible gateway (Ozone S3/MinIO) — ⛔ never `hadoop-client` (lean-SBOM non-negotiable), WebHDFS-REST only if mandated. | [Object-storage export](okf/backend/engine/object-storage-export.md) |
| **Consignment concurrency (2026-08-25, plan SHIPPED `1b872515`→`275bf764`)** | ~~**(a) Journal a scheduler PUT?**~~ — **ANSWERED YES and SHIPPED 2026-08-26**: `SCHEDULER_SETTINGS_CHANGED` carries the deltas (`"<old> -> <new>"` per changed key + `tier`/`scope` + actor), emitted to the BOUND SPACE's log, only when something actually changed. One decision left: **(b) Should `processing.priority` also weight ACQUISITION?** Priority is scoped to ingest execution slots; the fetch lane (`acquirePermits`) stays plain FIFO. Revisit only if remote-fetch contention shows up in practice (plan §B6 Q2). ⛔ **Do NOT "finish" the plan by retiring `PipelineScheduler.runPermits`** — the plan's own row saying so was REFUTED by the code: it is the only bound on the pre-broker phase (checksum dedup reads whole files; `UnpackStage` writes temp and is itself parallel). Deferred by decision, not oversight: per-space *weights/reservations* (only a hard per-space cap shipped), and a dedicated ops pane duplicating the Settings ▸ Scheduler lists. | [Consignment concurrency](okf/backend/engine/consignment-concurrency.md) |
| **Source-timezone for temporal data (2026-08-22, operator-requested)** — **S1 + S2 BOTH SHIPPED 2026-08-29; plan ARCHIVED** | The three-tier policy (`raw.fields[].timezone_column` > `raw.fields[].timezone` > `parsing.source_timezone` > none) is live end to end, engine and surfaces, and the `TIMESTAMPTZ` host-zone trap is closed by refusal. **The as-built, every finding and every deliberate exemption now live in the concept home** — `okf/backend/engine/duckdb.md` §*The source time zone for temporal data*; the plan and its full live-probe transcript are at [`archived-documents/plans-archive/source-timezone-plan.md`](archived-documents/plans-archive/source-timezone-plan.md). **Two items stay OPEN, and only these two:** (a) ⛔ **no editor for `timezone_column`, by decision** — offering a per-row column beside ~418 zone names in one cell invites the ambiguity the engine's mutual-exclusion rule exists to prevent; a hand-authored one is carried through a save and shown read-only. Reopen only if an operator asks for it by name. (b) ✅ **the `%z` defect — GROUNDED and CLOSED FAIL-CLOSED 2026-08-29 (operator's call).** This row's own framing was wrong in three ways and missed a fourth directive. **REFUTED: it was never the plain branch** (`appendCoalesce`'s trailing `::TIMESTAMP` converts back through the same session zone — an identity) and **never needed a mixed list**. The real defect: `TRY_STRPTIME` parses the offset and the `::TIMESTAMP` then re-renders that instant in the **HOST's** wall clock — `2026-03-01 10:00:00+00:00` becomes `15:30`/`11:00`/`10:00`/`05:00` under Asia/Calcutta / Europe/Berlin / UTC / America/New_York; worse with a declared zone (a different calendar day by host) and it moved `DATE_*` partition keys too. 🔴 **And there are TWO directives, not one — `%Z` (zone name) is identical and was named nowhere**; both were found by sweeping every ASCII letter against the live engine, not by reading the docs. **Shipped:** `SourceZones.assertNoZoneDirective` refuses `%z`/`%Z` in `date_formats`/`timestamp_formats` at config load, from `PipelineConfigParser`'s single parse site — same posture as the zone-less `TIMESTAMPTZ` refusal. `%%` is an escaped literal and stays admitted; the gate's scope is pinned by re-running the live sweep in a test and was **falsified in both directions** before being trusted. No shipped config affected. ⚠ **The feature is deliberately NOT built** — making the data's offset WIN (a fifth precedence tier) is a separate build; ⛔ do not reach it by relaxing this gate. ✅ **The divergence found in passing is CLOSED the same day** — and the mock was **ahead of the server**, not wrong: it 422'd a bad `parsing.source_timezone` while **no Java route validated it on write at all**. `ConfigSpecs.pipeline()` now carries `parsing-source-timezone-resolvable` **and** `parsing-formats-carry-no-zone-directive` as ERROR rules, so both refusals are a 422 at authoring as well as a load-time throw — the house pattern `template-cannot-be-active` states outright. 🔴 The shared predicate moved **DOWN** into `com.gamma.config.spec.SourceZoneGrammar` (inspecto-config sits below inspecto-etl) and `SourceZones` **delegates** to it — no mirror, so nothing can drift. ⚠ `ConfigSpecs.meta()`'s `domain-timezone-resolvable` stays on plain `ZoneId.of` — that value is resolved by the **JVM**, not DuckDB; ⛔ do not unify the two. | `okf/backend/engine/duckdb.md` |
| **Archive / exotic-compression inputs (2026-08-23, operator question)** — **CLOSED 2026-08-26** | **The design direction this row proposed is what shipped: an UNPACK stage in the Collector, not more `compression` values.** `CollectorProcessor.run` expands candidates through `UnpackStage.expand` before the planner freezes the list (`CollectorProcessor.java:117`), so archives (.zip/.tar/.tar.gz) and streams (.gz/.bz2/.Z) all become ordinary members from birth — `.Z` is served by `BuiltinDecompressors` via commons-compress `ZCompressorInputStream`, and multi-member archives are no longer CLI-only. Both "smaller safe slices" are resolved too: **(a) SHIPPED** — input `compression` is a fail-closed enum (`auto\|gzip\|zstd\|none`) at both config load (`PipelineConfigParser.java:396`) and spec publication (`ConfigSpecs.java:131`), so `compression: zip` is refused with a message naming the unpack stage instead of quarantining every file at run time; **(b) DISSOLVED, not built** — routing .bz2/.zip to the Java engine under `engine: auto` is moot because the reader never sees a compressed file any more. ⚠ **One residual, deliberate:** with `processing.unpack.enabled: false` the pre-existing same-file engine divergence returns (`Compression.java:39` decompresses .gz/.bz2/.zip by extension on `engine: java`; `engine: auto` picks native and quarantines them). That is the operator opting out of the stage built for exactly this, so it is left as-is. Remaining unpack work is tracked by the row below, not here. | `okf/backend/engine/unpack-stage.md` |
| **Unpack stage — open items after the 2026-08-23 build** (plan ARCHIVED 2026-08-26: `archived-documents/plans-archive/unpack-stage-plan.md`; shipped `d6cd55b7`) | The stage is live end to end (stream `.gz`/`.bz2`/`.Z` + archive `.zip`/`.tar`/`.tar.gz` expanded at the Collector before consignments are planned; fail-closed bomb caps; extension-insensitive duplicate identity; `processing.unpack.*`; parallel expansion; FAILED members recorded in the manifest). **What is NOT built, each with the workaround now in force and its restriction:** ~~**(1) No run-level `unpack` ledger**~~ **✅ SHIPPED 2026-08-26** (§6 Q1 answered, unblocking it). `<pipeline>_unpack_<runTimestamp>.csv` — the fourth ledger beside status/batches/lineage — carries ONE row per archive per run: `run_id, archive_relpath, format, entries_found, entries_ingested, entries_failed, entries_skipped, bytes_in, bytes_out, status, error, consignment_ids`. `status` is the signed-off `UnpackStatus` vocabulary. ⛔ Its columns are declared ONCE (`UnpackLedger.COLUMNS`; `HEADER` is `String.join`ed from it and the codec is index-aligned) — the plan's explicit instruction not to repeat the batches ledger's five-mirror drift, pinned by a test. Accumulated in memory and flushed at end-of-run in `CollectorProcessor.run` AFTER the batch futures join — ⚠ deliberately NOT at the `UnpackOrigins.consume()` release points, because a batch that fails at COMMIT runs neither finalize nor quarantine, so the release never fires for exactly the archives an operator most needs a row for. Keyed on the archive's absolute PATH, never its basename (`in/east/data.zip` and `in/west/data.zip` are two archives). ~~Still open: no READ surface~~ **✅ READ SURFACE SHIPPED 2026-08-26** — `StatusStore.unpack(cfg)` (a `default` returning empty, so the six anonymous test stores and any external impl keep compiling), `FileStatusStore` globbing `<pipeline>_unpack_*.csv`, `DbStatusStore` projecting into `inspecto_status_unpack` (sync/delete/rename/browse all carry it), and `OperationalTables.UNPACK` — which **references `UnpackLedger.COLUMNS`** (`static final List<String> UNPACK = UnpackLedger.COLUMNS;`), never restates it, exactly as this row demanded; `unpack` joined `STAGE1_NAMES` so report-sql can query it. ~~**(2) `origin` SHIPPED 2026-08-23**; `logical_name` still open.~~ **✅ BOTH HALVES SHIPPED — `logical_name` 2026-08-26.** The per-file status ledger carries `origin` (the archive/compressed original a member came out of, surfaced as the 'From archive' column on Processing Status ▸ Problem files) AND `logical_name` (the same file's extension-insensitive IDENTITY — poll-relative, so unlike `origin` it IS a key, and it is what a report can now group a re-delivery on instead of reading the alias hit out of the dedup log). ⚠ Both captured on `MemberAudit`/computed in `BatchProcessor.logicalNameOf` at INGEST time, not where the row is written: `writeAudit` runs AFTER `commit`, whose `UnpackStage.cleanup` consumes the mapping — resolving either later reads blank for every expanded file. ⚠ For an expanded ARCHIVE, `logical_name` is the ARCHIVE's identity, shared by every entry — never the entry's own name. Five header mirrors were updated together for `logical_name` (BatchAuditWriter header + `statusLine` codec + `FileRow` record + two compat arities + `OperationalTables.FILES`); Run Detail's Files tab needed no UI change (`autoColumns`). ~~**(3) 🔴 Lineage records the INTERNAL member name.**~~ **✅ FIXED 2026-08-26** — lineage/`output.filename_column` now records the ENTRY name: the name is stored at register time (`UnpackOrigins.register(actual, original, lineageName)`, kind-aware in `UnpackStage`; `ArchiveDecompressorPlugin.entryName` is the exact reverse of `entryPath`, kept in the same class so the two halves cannot drift) and **FIVE** sites now read `UnpackOrigins.lineageName(m.file())` — the three `srcIdToFile` puts (`CsvBatchStrategy`, `NativeCsvStreamingEngine`, `UnionModeIngester`) **plus the two `DuckDbRecordSink` constructions the row's original scope missed** (`GenerationModeIngester`, `UnionModeIngester`: the wrap lane names the file at construction, not per row, so grepping `srcIdToFile` alone left that lane leaking). Site count pinned in `UnpackOrigins.lineageName`'s javadoc. Includes single-entry archives, where a `totalFor > 1` test would have misclassified. A stream expansion keeps its decompressed name. §6 Q2's defect half is gone; what remains of Q2 is ratifying the entry grain (the plan's own recommendation, now in force). ~~**(4) 🔴 An unreadable archive member is SILENTLY SKIPPED.**~~ **✅ Honesty half FIXED 2026-08-26** — `expand(..., skippedOut)` (a default SPI overload, non-breaking for external plugins) reports every entry `canReadEntryData` refuses; `UnpackStage` records them against the ORIGINAL (`UnpackOrigins.registerSkipped`), and `finalizeSource` drains them exactly once (atomic `takeSkipped`, first of the archive's batches to finalize, only when a manifest will actually be written) into `SKIPPED_UNREADABLE` manifest rows (`archive!entry` address, srcId -1, no backup/marker — nothing was planned, there are no bytes). An all-unreadable archive still fails whole. Still open under (1): the archive-LEVEL roll-up verdict — the status vocabulary stays §6 Q1's. **(5) An archive with one bad member still COMMITS and is MARKED** — **RATIFIED 2026-08-26 (§6 Q1b), no longer a restriction.** Today's per-file semantics (a bad file never blocks its batch-mates) are the decision, not a gap: failing whole would discard 499 good ingests for one bad entry in a 500-entry archive. The `UNPACKED_PARTIAL` verdict now EXISTS and is reported in the unpack ledger — it is reporting, never a gate. ⛔ Do not "fix" this by making a partial archive fail its Consignment. ~~**(6) `processing.unpack.data_extensions` is NOT configurable**~~ **✅ SHIPPED 2026-08-26 (§6 Q4 answered).** The seven-extension default is CONFIRMED and the key is now published (`FieldType.LIST`), parsed, normalised (bare/upper/padded → leading-dot lower-case) and honoured at all three production call sites via `LogicalNames.logicalName(rel, cfg)`. An explicitly empty list (`data_extensions[0]:`) is the deployment's opt-out and is honoured as a CHOICE, never as "unset" — verified expressible in TOON, since a published escape hatch nobody can author would be worse than none. ⚠ The collision itself is **inherent, not a defect**: rule 2's data-extension strip is precisely what makes `cdr.csv.gz` / `cdr.Z` / bare `cdr` one logical file — they meet ONLY at the fully-stripped tier — so `report.csv` + `report.json` colliding is the same mechanism, and a "safer" compression-stripped-only scheme would break the operator's actual requirement. ⛔ Do not "fix" the collision; narrow or empty the list instead. ⚠ ONE permitted mirror: `ConfigSpecs` restates the default because `inspecto-config` sits BELOW `inspecto-etl`; `LogicalNamesTest` pins the two equal — do not add a third. ~~**(7) No UI**~~ **✅ SHIPPED 2026-08-26** — the acquisition node publishes seven `unpack__*` attributes (`NodeAttributes.UNPACK`, borrowed from `processing:` exactly as the marker-dedup keys are, NOT folded into `COLLECTOR` because Onboarding renders that list whole), lifted/lowered as the nested `unpack` map by `PipelineEditable` (like the sink's `intake`), and rendered as its own **Unpack** group in the collector pane beside Duplicate handling. ⛔ **`depth` is deliberately NOT published** — `PipelineConfig.Unpack` refuses any value but 1 by name, so offering it would author a config that cannot load. ⚠ Defaults are stated in help text, never as spec `default`s, so an untouched form writes no `unpack:` block. ⚠ The UI cannot author the **explicit empty-list opt-out**: schema-form's `list` type writes an empty list as `null` (dropped key), so `data_extensions[0]:` stays a pipeline-file edit and the help text says so rather than implying otherwise. Five sites moved together — `NodeAttributes` · both contract JSON regens (`-Dnode.attributes.write=true` / `-Dstep.types.write=true`; **step-types moved too**, since the collect step reuses ACQUISITION's list) · `NodeConfigNameContractTest` rows · `PipelineEditable` lift+lower · the TS mirror `UNPACK_ATTRIBUTES` + its spec. **(8) Nested archives refused by design** (`depth` must be 1, refused by name). ⚠ Restriction: a `.tar.gz` INSIDE a `.zip` is extracted as an opaque member and then quarantines when the parser cannot read it. ~~**(9) `UnpackOrigins` is a process-local map cleared only by `consume()`**~~ **✅ FIXED 2026-08-28** — `UnpackOrigins.sweep(pollRoot)` runs in `CollectorProcessor.run` after every batch joins (same position as the `UnpackLedger` flush), dropping the mappings COMMIT-failed batches never released; scoped to the run's poll root so a concurrent pipeline on another root keeps its in-flight entries. Restores exactly the documented crash posture (originals stay in the inbox, next cycle re-expands). Test: `ArchiveUnpackTest.sweepDropsOnlyThePollRootsLeftovers`. **(10) Crash mid-archive re-ingests committed members**: the container is only marked after its LAST member, so a crash re-expands the whole archive. Relies on `OVERWRITE_OR_IGNORE` output idempotence — the same guarantee a failed multi-file batch already leans on. **(11) Absent codecs**: xz and zstd have no plugin (codecs not on the classpath; DuckDB reads `.zst` natively). `.Z` has no round-trip test — commons-compress ships no `.Z` writer, so it is covered at `supports()` + corrupt-input level only. Multi-part/split archives (`.z01`, `.part1.rar`) unhandled — arrival-completeness is a Collector question. ~~**(13) ⚠ Code says "member" where the GLOSSARY now says "Entry"**~~ **✅ SWEPT 2026-08-26** — one sweep across the 8 files of `com.gamma.etl.unpack` + its two test classes (34 occurrences incl. the `ArchiveDecompressorPlugin` WARN string, now "skipping unreadable entry"; two test methods renamed). Three occurrences deliberately KEPT: the ones that genuinely mean *Consignment member* ("an N-entry archive's members land in N batches" ×2, `UnpackStatus`'s container-vs-member contrast). `Batch.Member`/`MemberAudit`/`MemberEntry` untouched as required. Original row (for provenance): The plan's step 19 warned that `Batch.Member` already means "a file in a Consignment", so an archive's inner file must be an **Entry** — the GLOSSARY entries + a §11 resolved-collision row now make that canon. The code has **~21 occurrences across the 8 files of `com.gamma.etl.unpack`**, mostly javadoc/comments (pre-existing, extended by the 2026-08-26 work) plus **one operator-facing log string** (`ArchiveDecompressorPlugin`'s "skipping unreadable member '<x>'" WARN) which is the only one a user ever sees. Cosmetic but binding per the canonical-vocabulary rule; do it as one sweep, not opportunistically. ⚠ Do NOT rename `Batch.Member` or `MemberAudit`/`MemberEntry` — those are correctly named for Consignment members. ~~**(12) Two decompression vocabularies now coexist**~~ **✅ UNIFIED 2026-08-28.** Both paths remain — they are not interchangeable (the SPI materialises FILES, `Compression` wraps a STREAM for `SchemaSelector`/`BoundaryScanner`/`FileChunker`/`CsvIngester`) — but the hand-mirrored LIST is gone: `Compression.INLINE_SUFFIXES` is the one declaration, read by `isCompressed`, by `UnpackStage.laneReadsItself` and by `LogicalNames.LEGACY_SUFFIXES`; all three previously spelled `.gz`/`.bz2`/`.zip` out separately. The drift is guarded ONE-WAY by `CompressionTest.inlineVocabularyIsOwnedByPlugins` (falsified in both directions: planting `.xz` turns it red), because only that direction is unsafe — `laneReadsItself` lets the stage SKIP a file, so a suffix the inline path claims and no plugin owns would reach a lane that cannot open it. ⛔ Do NOT add the reverse containment: the SPI is wider on purpose (`.Z`, `.tar`, `.tar.gz`/`.tgz`) and the stage expands those first. ⚠ DuckDB's branch of `laneReadsItself` stays literal — it states a FOREIGN engine's capability, not our vocabulary. 🔴 Two stale comments found by grounding: `LEGACY_SUFFIXES` claimed `.zip` was owned by "no plugin **yet**" (`ArchiveDecompressorPlugin$Zip` has owned it since 2026-08-23), and the `META-INF/services` header still says "ORDER MATTERS: first-match linear scan" though `Decompressors.forFile` resolves by LONGEST matching suffix and its javadoc says it deliberately does not rely on service order — that one is left untouched and recorded in the OKF concept. | `okf/backend/engine/duckdb.md` (as-built home owed: `okf/backend/engine/unpack-stage.md`) |
| **Delimited error-handling knobs — open items** (shipped `d6cd55b7`/`83b46a79`) | The six knobs (`ignore_errors`/`null_padding`/`store_rejects`/`rejects_table`/`rejects_scan`/`rejects_limit`) are engine-honoured, published and offered on the delimited Robustness tab, all tri-state. **Open:** **(a) ⚠ The two knobs whose ENGINE default is ON render as OFF-looking toggles** — a tri-state control has no third visual state. Workaround: the help text leads with "Engine default is ON". Restriction: the toggle alone reads as the OPPOSITE of the truth; label inversion is unavailable because the spec key IS the config key. Fix = a 3-way picker per knob (operator decision). ~~**(b) Offered on the DELIMITED tab only.**~~ **✅ SHIPPED 2026-08-26 for the two line-reader frontends** — the six now reach `fixedwidth` and `text_regex` from a single `sharedRobustnessAttributes(padsShortRowsByDefault)` factory (one declaration, three consumers — never a mirror). 🔴 **The obvious implementation would have been DEAD CONFIG:** the keys stay `delimited__*` on every frontend because `PipelineConfigParser.mergeParsing` FLATTENS the `delimited:` block into the shared `processing.csv_settings.*` the engine reads, while copying `fixedwidth:`/`text_regex:` through as NESTED sub-blocks — so a mirrored `fixedwidth__ignore_errors` would be written, validated, saved and NEVER read. Pinned by `shared-robustness.spec.ts`, which fails if anyone "tidies" them into per-frontend keys. No backend change was needed (the keys were already published as `processing.csv_settings.*`). **JSON deliberately excluded:** it carries its own distinct `json.ignore_errors` (a document-level option with its own refusals), so offering both would be two similar controls with different meanings. **(c) `null_padding`'s per-frontend default divergence** (false delimited / true line-readers) is preserved in the engine but not surfaced anywhere in the UI. | `okf/backend/config/parsing-options-reference.md` |
| ✅ **Cross-pipeline ET problem-files view — SHIPPED, row was STALE (closed 2026-08-26)** | 🔴 **This row still read "no route serves this today" long after BOTH halves shipped** (`fa0e3a6e` the view, `43d8f4af` the origin column) — the next shift would have rebuilt it from scratch. Grep the claim, never trust the row. **As shipped:** `GET /status/problem-files?limit=&since=` (`RunRoutes.java:99,248`) walks `loadedPipelines()` through the `StatusStore` seam, unions FULL (`QUARANTINED_*` ledger rows + quarantine-tree entries the ledger does not already carry — one row per problem, never two) with PARTIAL (`SUCCESS` + `error_rows > 0`), newest first, bounded with `truncated` reporting the TRUE total and summary counts computed pre-limit so the cards stay honest. **Both honesty rules are in the code:** an unreadable ledger becomes a `WARNING`/`LEDGER_UNREADABLE` row (never silent absence), and untimed quarantine-tree rows are always included rather than hidden by `?since=`. Carries the unpack `origin` column ("From archive"). 7 real-HTTP tests (`ControlApiProblemFilesTest`); UI = the **Problem files** tab on Processing Status (`processing-status.component.html:34`, `stateKey="processing-status-problems"`), `reports.service.problemFiles()`, `ProblemFilesPage` model, mock handler + a strictness spec. **Only genuinely open follow-on:** the row could now also carry `logical_name` (shipped 2026-08-26) beside `origin`, so a re-delivery groups to its earlier compression spelling — additive, build on demand. Original row (provenance): Operators with 100s of pipelines cannot triage ingest failures pipeline-by-pipeline. Wanted: ONE pane listing, across ALL pipelines, files that failed WHOLE (quarantined) and files that ingested PARTIALLY (`error_rows > 0`), with drill-down to the rejected rows. **Grounded design:** the HOME is the existing Processing Status pane (`processing-status.component.ts` — already 'the cross-pipeline rollup Operations lacked', pipeline-grain over `GET /status`) — add a file-grain 'Problem files' tab there, ⛔ never a new sibling pane. **Backend is the real work — no route serves this today**: every `/runs/{name}/*` route resolves ONE PipelineConfig (`RunRoutes.java:36,55-64`), and the ledgers are PER-PIPELINE directory trees (`PipelineConfig.java:1597-1618`); the FILES ledger has NO pipeline column (`OperationalTables.java:36-53` — identity comes from which store you read), only BATCHES does. New route `GET /status/problem-files` (page + `since` filter) with TWO arms behind the existing `StatusStore` seam (`StatusStore.java:21-46`): (a) default file backend — walk `configs.pipelines()` and union each store's FILES (`status != SUCCESS OR error_rows > 0`) + QUARANTINE rows, stamping the pipeline name server-side; `AlertService.java:190` already iterates exactly this way each poll, so the cost is precedented — but bound it (cap rows per pipeline, report `truncated`) and consider sharing AlertService's cadence rather than walking on every request; (b) `-Dstatus.backend=db` armed — `DbStatusStore` (`DbStatusStore.java:146-160,244-287`) already centralizes ALL pipelines' files/quarantine rows with an indexed `pipeline` column; drop the `WHERE pipeline = ?` and it is one query. Same degrade the ops pattern uses everywhere: db arm when armed, file arm otherwise, SAME response shape. **Row shape:** pipeline · filename · verdict (FULL = `QUARANTINED_*`/quarantine-tree row; PARTIAL = SUCCESS with `error_rows > 0`) · parsed_rows · error_rows · reason/error · consignment_id · time · origin archive (once the unpack `origin` column lands — see the unpack open-items row (2)). **Drill-down exists already**: 'View the rejected rows' = `GET /runs/{name}/errors?file=<bare name>` (bounded, `truncated`), row click → that pipeline's Run Detail. **UI:** `<inspecto-data-table>` standard tier, `[stateKey]`, status-badge verdict column, `[rowActions]` = view-rejected-rows + open-run-detail (+ `pinActions`); summary cards: pipelines-with-problems / full / partial counts. Mock handler must be no more lenient than the route. ⚠ Two honesty rules: a pipeline whose ledger cannot be read must surface as a WARNING row, never silently absent; and `cast_failures = -1` (not measured) must render blank, never 0. ⚠ Don't build a THIRD aggregation walk: if request-time walking proves too slow at 100s of pipelines, the answer is arming `DbStatusStore` (whose sync already runs per poll), not a new cache. | `okf/frontend/features/` (pane concept owed) + `okf/backend/control-plane/` |
| **Delimited Grammar redesign (2026-08-19)** | ~~(a) **Canvas node rename**~~ **✅ SHIPPED 2026-08-19 (`e004ae26`)** — the inspector gained an inline pencil-edit for name/description (hidden read-only; a rename deliberately does NOT invalidate the node's test outcome), and the Parse pane's Name/Description fields are deleted, Apply carrying the stored identity verbatim. ~~(b) **E4 full finalization harness**~~ **✅ SHIPPED 2026-08-19** — `FinalizeSourceConcurrencyTest` pins both claims: 8 distinct batches finalizing concurrently through the three SHARED stores (registry reconciliation · ledger PROCESSED per file · the full crash-ordered stage trail · one manifest per batch), and the same-file marker race (exactly ONE loser, on the atomic `Files.createFile` — zero = silent double-commit, two = the winner failed). ⚠ The race fixture deliberately has NO backup dir: a backup dir adds a same-file `Files.move` race whose loser throws platform-dependent exceptions before the marker. The harness pins exactly the stores' own `synchronized`+one-connection claim — a future connection pool must re-run it. ~~(c) **/design gallery**~~ **✅ SHIPPED 2026-08-19** — the 'Schema columns table & column metadata' section mounts both grids live (incl. the Auto icon-menu lock); the tabbed surface is documented as a snippet because the grammar editor is a domain composite (the collector-config precedent). ~~(d) **D1(b) column-metadata grid**~~ **✅ SHIPPED 2026-08-19** — `<inspecto-schema-metadata-grid>` on `[tabFiles]`, a second VIEW over the columns table's rows merged by selector at submit; the carry-through was fixed in THREE places that silently dropped the keys (CSV import, the schema write, the drift-add reseed). Grounded: the keys pass `POST /config/write` verbatim and Catalog's `SchemaProjection.Column` reads them. | `okf/frontend/features/grammar-config.md` · `okf/backend/engine/ingest-wrap-spi.md` |
| **Pipeline definition drawers (P4)** | ~~**B1's mapped-row preview has no consumer**~~ **✅ CLOSED 2026-08-16 — the Load drawer renders it.** *Test mapping* posts the rules being EDITED (not the node's) plus the parser schema's `raw` over the thread's parsed rows, renders `mappedColumns`/`mappedRows` (first 20), and mirrors the result into the thread's **cast hop** — so the parse drawer's strip reads `parsed → cast · N ok` from a mapping test. An edited rule clears the grid (a result may not outlive its config). 🔴 Still only reachable where the parse node is **per-format**: a generic `parser` opens the grammar dialog, which has no thread. ⚠ Two things the preview caught that no unit test could: the offline mock resolved a DIRECT source by EXACT key, so an identity rule seeded from a schema (field names upper-cased) rendered *"3 rows mapped"* over blank cells — fixed to fold case, as DuckDB binds an unquoted identifier; and the drawer header labelled a `transform.map` node **"PARSER"** (`kindLabel` was a two-way acquisition/else ternary in the editor template) — **✅ FIXED 2026-08-17**: the header identity is now a three-way `definitionKind()` on the component (Collector · Parse · Load, the GLOSSARY stages, with the Load drawer taking the TRANSFORM icon), pinned by two specs and verified live. Note the parse arm now reads **"PARSE"** (the stage), not "Parser" (the engine that applies a Grammar). · **P6-e's two deliberate narrowings (A6) — DECIDED 2026-08-17, one kept and one reversed.** *View as graph* **stays dropped** (it needs the stream-vs-reference kind, i.e. an extra settings hop; the Catalog's lineage tab is directly reachable) — end state, not a deferral. ~~*Export configuration* now sits in the editor's `canAuthor()` menu, so it is hidden from the Business lens~~ **REVERSED — both exports are now in an always-visible Export menu.** The narrowing contradicted `exportConfig()`'s own doc comment (inherited from the shell: "Read-only, so every lens gets it — a Business-lens operator handing a config to support is the point"), and grounding found **nothing pinned it** — no spec, mock or backend. *Export document* moved with it rather than diverging, since consistency between the two was the narrowing's whole justification. ⚠ Precedent worth remembering: sibling A4 was ALSO "reviewed and left standing as a recorded decision" and then shipped anyway once grounding showed the blocker was narrower than it read — **a recorded decision is not self-justifying; re-ground it before carrying it forward again.** Verified in the offline app (Business lens reaches both; author menu keeps its five actions, no duplicate). · ~~Per-**segment** schemas (`<id>_<segmentKey>`) are still not swept by a pipeline delete~~ **SHIPPED 2026-08-17** — ⚠ **the stated cause was wrong** ("enumerating them needs the parsed block a delete no longer has"): a segment's schema path is *authored config* on the parse node (`parsing.<asn1|plugin>.segments` → `{key: path}`), so `ownedSegmentSchemas()` reads the names straight off the in-memory graph *before* the delete clears it, and they cascade in the same `forkJoin` as `<id>_schema`/`<id>_enrich` (each `catchError`ed — a missing schema must not fail the delete). ⛔ **Only names inside the pipeline's own `<id>_` namespace are swept**: a hand-authored path pointing elsewhere may be shared, and orphaning another pipeline is worse than leaving an unreferenced file — the same boundary the parse pane already keeps when it refuses to re-derive over a foreign `schema_file`. Both guards falsified separately (disable the sweep → exactly the 3 sweep tests fail; drop the prefix test → exactly the foreign-path test fails). · ~~The Load drawer offers `DIRECT`/`EXPR` only; `CONCAT_DT`/`FILENAME_DATE` cannot be created there~~ **SHIPPED 2026-08-17 — all four types are now authored.** The blocker ("source semantics the grid has no affordance for") was real but narrower than it read: there is no per-rule `format`/`pattern` field to add, because a rule is exactly `{targetColumn, sourceExpression, transformType}` and `MappingCsv` drops every other key — so both types pack their parameters into `sourceExpression` as `|`-delimited positions, and the pane composes/decomposes that string rather than inventing a fourth key. `CONCAT_DT` → `<date>|<time>` (always both — the compiler reads `parts[1]` unconditionally); `FILENAME_DATE` → `<column>|<prefix>|<strptime>` with ⚠ **trailing blanks dropped, not emitted** (an empty third position compiles to `TRY_STRPTIME(…, '')`, whereas a *missing* one defaults to `%Y%m%d`), and 🔴 refused unless it writes **`EVENT_DATE`**, which the engine enforces in three places. The old "preserved, not rewritten" guard still stands but now protects an **unknown** type (e.g. a hand-authored `LOOKUP`). Both new guards falsified separately. ⚠ Offline, Test mapping shows these as **blank cells** — the mock has no SQL engine and only evaluates `DIRECT`; its validator, however, is a faithful mirror. | `okf/frontend/features/pipeline-editor.md` |
...
| **Job framework** | ~~MNT-14 archived-Incident sweep~~ **SHIPPED 2026-07-27** as `incident_purge` (D5's retention tier, enforced; dry-run-first, legal-hold exemption, cascade to notes/links/tag edges). ~~Residuals — operator retention docs carrying the G3 stance, and no scheduled instance~~ **BOTH CLOSED 2026-07-27**: `okf/backend/build-run/operations-reference.md` now carries a *Retention & purging* section — a what-each-task-forgets table, a worked `incident_purge` job TOON, the dry-run-first + legal-hold procedure, and the G3 stance stated for a legal/DPA reader. **"No scheduled instance" was resolved as the decision, not the gap**: nothing schedules it and nothing should — shipping a default that deletes business records is indefensible, so standing up the job is an operator act like `receipt_prune`, and that is now written down · maintenance COULD tier: space-to-space comparison · predictive maintenance (AGT-5/self-healing territory, deliberately deferred) · **D6 reclassified 2026-08-04 — a scheduling feature, NOT the missing UI surface it was filed as:** `meta.domain.timezone` is parsed (`SemanticModel.DomainNotes`) and served (`CatalogRoutes` → `/catalog/kpis`, plus `ExplainEntitySkill`) but **never behaviourally consumed** — the two places that actually need a zone, `PipelineScheduler:117` (cron/trigger firing) and `JobService:205` (feeding `ExpressionContext.zone`, i.e. `$today`/`$yesterday`/`$day(-1)` — the record was `ParameterResolver.Context` until `74989b3c`), both hardcode `ZoneId.systemDefault()` and never read it. ⛔ **Do not close this by adding a timezone picker** — a control implying it governs date math when it does not is worse than the gap. The real buildable is making a configured zone govern cron firing + date-macro substitution, which is a **behaviour change on existing schedules** (every `$today` job shifts) and so needs operator sign-off and a migration story, not a form field. ~~Note the key is also unvalidated — any string is accepted, no `ZoneId.of` check~~ **that half SHIPPED 2026-08-14**: `ConfigSpecs.meta()` carries the ERROR rule `domain-timezone-resolvable`, so a write with an unresolvable zone is a 422 anchored on `domain.timezone`. ⛔ Deliberately a **CrossFieldRule, not an ENUM** of `getAvailableZoneIds()` — an enum field generates a form control, and a picker is exactly what this row bans. ⚠ The value is still **inert** (both consumers hardcode `systemDefault()`), so this is hardening ahead of the behaviour half, and the trade accepted is that a pre-existing meta file with a typo'd zone can no longer be saved until corrected. The finding names the path, **not the offending value** (a `CrossFieldRule`'s description is static) — the row's "422 with the offending value" is only half-met. ~~The **behaviour half is untouched and still needs operator sign-off**~~ **→ ✅ SHIPPED 2026-08-15** as `-Dops.timezone` (`com.gamma.util.OperationsZone`), all 3 questions answered as recommended; as-built in `okf/backend/control-plane/jobs.md` *The operations zone*, plan archived. ⚠ **NEW ROW spun out:** the five other `ZoneId.systemDefault()` sites (`AlertService:374`, `ReferenceCompactor:142`, `EventRoutes:108`, `PackTestHarness:164`, `InspectoTools:385`) are each a display-vs-data-cut judgement and were deliberately NOT swept in — decide them one at a time. **→ 2026-08-15: the five are TWO classes, and one class needed no decision. `EventRoutes:108` and `PackTestHarness:164` SHIPPED onto `OperationsZone`** — the first parses an *operator's* typed `yyyy-MM-dd` bound (⚠ resolve OUTSIDE the parse's `catch (RuntimeException)`, else a bad `-Dops.timezone` returns a 400 blaming the operator's own query), the second had already diverged from its production counterpart `JobService:250` and so could dry-run `$today` against the wrong date. ~~**The remaining THREE are one decision, not three**~~ **→ ✅ DECIDED + CLOSED 2026-08-15 (operator): `AlertService:374`, `ReferenceCompactor:142` and `InspectoTools:385` STAY on `systemDefault()`, to match their writers.** This row is **done — the whole `systemDefault()` sweep is closed**, and "stays" is the **end state, not a deferral**: each site is the read half of a write/read pair over a **zone-naive** stored string (ledger `end_time` ← `BatchProcessor`, `__valid_from` ← `BatchIngestStrategy:215`, audit CSV ← `JobService`), self-consistent precisely *because* both halves agree, and none of the three is an operator-facing clock for `-Dops.timezone` to govern. ⛔ **Do not convert a reader alone** — it offsets every window by the ops-vs-host gap, and in the compactor's case rows fall outside `keep` and are dropped. Recorded as a ⛔ comment **at all three call sites**, since a backlog row cannot reach someone editing the file. ⚠ **One refutation banked:** the compactor is *not* the UTC mismatch it appears to be — DuckDB here bundles+auto-loads **ICU**, so its session `TimeZone` is the **host** zone and `now()::TIMESTAMP` matches `LocalDateTime.now()` (probed against `duckdb_jdbc:1.5.2.1`). An agent asserted the "DuckDB defaults to UTC" folklore as fact and called this a live bug; that "fix" would have **created** a +05:30 skew. DuckDB is also **blind to `-Dops.timezone`**, so any future migration of that pair needs an explicit `SET TimeZone` as a third moving part. The grounding that got here: ⚠ **That grounding REFUTED this row's own framing, twice.** (i) The row conflates the **data** zone (what the timestamps mean — a domain note, whose real consumer is the consignment §5.6/§10.1 event-time cut) with the **operations** zone (what the operator's schedule is expressed in); they routinely differ (data in UTC, a 06:00 `Asia/Kolkata` run), and one value cannot answer both. (ii) `domain.timezone` is **not a single value**: a space may hold any number of `*_meta.toon` files (`ServiceBootstrap:60`, no cardinality limit) and `MetadataGraphService:155` merges them **last-non-blank-wins in iteration order**, so wiring it into the scheduler would make cron firing depend on **directory scan order**. ⛔ So "just read `domain.timezone` in the scheduler" is disqualified. **The recommended split dissolves this row's own gate:** an operations key defaulting to `systemDefault()` shifts no existing schedule, so the "migration story" the sign-off was waiting for does not exist. Also corrected: the row says "the two places that need a zone" — there are **seven** `ZoneId.systemDefault()` sites in main code (`AlertService:374`, `ReferenceCompactor:142`, `EventRoutes:108`, `PackTestHarness:164`, `InspectoTools:385` beyond the two named); the other five are display-vs-data-cut judgements and must be **decided separately**, not swept in. · **EXPR-1 (new 2026-08-07): Expression interpolation inside a longer string is deferred, deliberately.** Step 4 of `superpower/job-parameter-contract-plan.md` settled on **whole-value** evaluation, so `Daily report for $yesterday` in a Subject field stays literal (the plan's own §9 example, now corrected). Blanket interpolation is **refuted, not merely unbuilt**: a `sql.template` body's `$name` tokens are that Job's template parameters, so interpolating them would capture the namespace *and*, under the fail-closed unknown-token rule, REJECT every existing `sql.template` Job — `spaces/demo/config/jobs/orders_summary_sql_job.toon` is a live instance. If interpolation is ever wanted it must be **per-declaration opt-in** (the `expressions` component is the natural carrier), never global | `okf/backend/control-plane/jobs.md` |
| **Collector config** | Unification SHIPPED 2026-08-04 (one `<inspecto-collector-config>` for both hosts, `POST /config/patch`, `transform.dedup.fingerprint` removed). **List-cannot-load refusal FIXED 2026-08-15** — the component now tracks `loading | ok | failed` beside the profile array; only `ok` licenses "not a saved Connection", and under `failed` the id the node was *loaded with* keeps its stored connector while a newly picked one is still refused (distinct "could not be loaded, retry" message). ⚠ the row's stated cause named `connectionOptionLoader` — **wrong**: that loader only feeds the picker's suggestions; `resolveConnector` reads the component's OWN `connections.list()` subscribe, whose `error: () => profiles.set([])` was the ambiguity. Nothing open · `<inspecto-collector-config>` is deliberately **not** in the `/design` gallery — it is a domain composite needing `ConnectionsService`, like `<inspecto-enrichment-editor>`, and the gallery holds primitives | `okf/frontend/features/collector-config.md` |
| **Grammar config** | Unification SHIPPED 2026-08-04 (one `<inspecto-grammar-editor>` for both hosts, `ParserConfigDialog` → `GrammarEditorDialog`, inline `parsing:` by default with explicit extract to a `grammar` component). Plan archived to [`archived-documents/plans-archive/grammar-config-unification.md`](archived-documents/plans-archive/grammar-config-unification.md). ~~Open: **the unknown-`use:`-prefix refusal is still unbuilt**~~ **SHIPPED 2026-08-10** — a `use:` binding whose kind is valid but whose NAMED component does not exist is now refused at save with `UNKNOWN_USE_REF` (422), pinned over real HTTP in `ControlApiPipelineCrudTest`. ⚠ **Two of this row's own claims were wrong, checked against the code before building:** (1) `ComponentRegistry.resolve` **does** have a main caller (`ComponentStore.get`) — only `isKnown` had none; and (2) the seam is **NOT `ConfigSafetyValidator`**, which switches on `pipeline`/`enrichment` `.toon` safety only and never sees an authored graph. The real seam was `PipelineValidator.checkWiring`, which already checked the `use:` KIND prefix via `ComponentRegistry.isComponentType` but never the named target. As built: a `validate(g, ComponentRegistry)` overload (the registry-less `validate(g)` is unchanged, so `PipelineExecutor.validateOrThrow` and `InspectoTools` keep their behaviour), and `PipelineRoutes` passes the registry it already scans for dry-run. ⚠ The check is **skipped when the space has no write root** — `componentRegistry()` yields an EMPTY registry there, against which every binding would look dangling, so a read-only draft-validate must not invent refusals. A bad kind reports once, not twice. **~~Still open, and BIGGER than this row was:** `PipelineJobRunner` (real execution) never calls `ComponentRegistry.effectiveGraph`/`effectiveConfig` at all~~ **→ ✅ SHIPPED 2026-08-11 (`c47e3582`), verified closed 2026-08-15.** `PipelineJobRunner:209` resolves before anything reads the graph (`if (registry != null) g = registry.get().effectiveGraph(g);`), fed by the `Supplier<ComponentRegistry>` wired at `JobService:587`, so ad-hoc **and** scheduled `*_job.toon` runs share it — real runs now resolve exactly as the dry-run route does. Pinned by `ControlApiPipelineRunTest.adhocRunResolvesAUseBinding`, which asserts the **sink row count**, not merely a SUCCESS status — the original failure mode was a `transform.map` projecting *nothing* while still succeeding, so a status-only assertion would have passed against the bug. ⚠ This clause sat stale for four days and reads as the row's biggest open item; it was true for exactly ONE day. **A residual inside a row marked shipped is the easiest thing in this file to re-open by mistake — ground before building.** · **live browser smoke run 2026-08-05 found and FIXED a real backend bug, unrelated to the grammar editor itself**: `PipelineRoutes.saveGraph` (`PUT /pipelines/{name}/graph`) hardcoded its write target to `<name>_pipeline.toon` at the config root instead of resolving the pipeline's actually-registered file via `CollectorService.pathFor`. For any pipeline living at a legacy/non-canonical path (e.g. `subscriber/subscriber_pipeline.toon`), a save silently created a **second, shadow file** — the UI toasted "saved" and the new file had the edit, but the live pipeline (still bound to its original file) kept serving stale content until a restart, at which point the space-loader found two files claiming the same pipeline id and **the whole space failed to boot** (`SpaceManager.bootQuietly` swallows the conflict into a WARN and drops the space entirely, so `GET /spaces` silently lost an entry with no user-facing error). What looked in the browser like "the extracted `grammar:` binding doesn't survive a reload" was this file split-brain, not a dialog hydration bug — the dialog's `use:` read-back logic was already correct. **Fixed**: `saveGraph` now resolves the target via `pathFor(name)` when the pipeline is already registered, falling back to the canonical `<name>_pipeline.toon` only for a genuinely new pipeline. Verified via direct API round-trip (inline edit and extract-to-grammar both write to the original file, no duplicate created, GET after re-registration shows the correct `parsing:`/`use:` shape) — full reactor test run pending. `grammar-editor.dialog.spec.ts`'s "bound-edit round-trip" case is same-session only; a `PipelineRoutes`-level regression test asserting a save to a non-canonically-named pipeline overwrites the ORIGINAL file (not a new one) would have caught this — ~~worth adding~~ **✅ DONE, verified 2026-08-15**: `ControlApiPipelineCrudTest.aSaveOverwritesTheRegisteredFileRatherThanCreatingAShadow` asserts both halves — the edit lands in the registered file, and no shadow exists at the guessed canonical name. ⚠ Its fixture is deliberately nested INSIDE the write root: the sibling round-trip test registers from outside it and so exercises the canonical-name *fallback*, never the branch that broke. ~~**Nothing open in this row**~~ **SUPERSEDED 2026-08-15 — the store contract was REVERSED by operator decision.** A Grammar component is now a **Template** (copy from it; never bind to it): saving one leaves the node's `parsing:` block inline, "Start from a template" copies, and a legacy bound node migrates to an independent inline copy when edited. `use: grammar/<id>` stays read-supported in the engine (unchanged) but is never authored. Shipped in four slices — [`archived-documents/plans-archive/grammar-templates-not-bindings-plan.md`](archived-documents/plans-archive/grammar-templates-not-bindings-plan.md). 🔴 **It also fixed live silent data loss this row never knew about:** a Grammar component has TWO shapes (legacy flat `{delimiter, has_header}` vs nested `parsing:`), and the editor seeds by flattening to `delimited__*` keys — so a flat component matched no spec key and the form showed its **defaults**, displaying (and re-saving) `delimiter: ","` for a component storing `"|"`. Reachable through the dialog's bound-node path since 2026-08-04; fixed once for both surfaces by `grammarContentAsParsingBlock()`, with the regression test falsified against the old code. ~~**Open (small, deferred deliberately):** the Components registry form writes **DSV-only, flat** content~~ **FIXED 2026-08-17.** 🔴 **This row's "Nothing is lost today" was WRONG — it was live, reachable data loss, and the seeded mock proves it.** `buildContent()` returned a fresh six-key literal that never spread the stored content, `submit()` sent it through `PUT /components`, and `ComponentAccess.onUpdate` starts from `new LinkedHashMap<>(incoming)` carrying over only `owner`/`shares` — so opening a non-DSV Grammar and pressing Save **replaced its parser with `{delimiter: ',', has_header: false}`**. Nothing gated the dialog, and it seeded from raw top-level keys, so a nested drawer-written component displayed DSV *defaults* first. Fixed in three parts: (a) seed through `grammarContentAsParsingBlock()`; (b) **write back in the shape it was stored in** — nested stays nested, flat stays flat, and the keys this form does not own (`frontend`, `null_strings`, …) are re-emitted rather than dropped, with a cleared field now *removing* its key; (c) a non-DSV Grammar is **REFUSED** — banner naming the parser, Save disabled — mirroring the Parse drawer's own `pluginBlocked` precedent instead of inventing an editor this form cannot be. ⚠ **The preview found a second instance of the same bug the row never mentioned**: the registry *list* read a top-level `delimiter` too, so **seven of the nine seeded grammars advertised "delimiter ,"** — `cdr_asn1_ber`, `sim_events_ndjson`, `regulatory_xlsx`, `invoice_xml`, `tariff_html`, `mainframe_fixed`, `archive_parquet`, none of which has a delimiter at all. Both surfaces now share one `nonDelimitedGrammar()` in `grammar-block.ts`, the file that already declares itself "the one place that knows a Grammar component can be either of two shapes". Both guards falsified; verified live in the offline preview (refusal renders + Save disabled for `cdr_asn1_ber`; `pipe_delimited` still seeds `|` with Save enabled; list now names each parser). | `okf/frontend/features/grammar-config.md` |
...
> are not.
>
> ### ✅ SEC-INCIDENT-1 — leaked client secrets, CLOSED BY DECOMMISSION 2026-08-29 (opened 2026-07-25)
>
> **Closed by operator call 2026-08-29: the system no longer exists, so rotation at the issuer is moot.**
> Kept as a one-block record because the exposure was real and is independently discoverable — see
> `compliance/controls-matrix.md` CC6.1, which carries the same closure.
>
> **What happened.** Five OAuth client secrets sat in `inspecto-ui/src/environments/*.ts` and were pushed
> to a **PUBLIC** GitHub remote (`jotder/inspecto`, `isPrivate:false`) from **2026-06-12** until removed in
> `94d98593` (2026-07-25) — roughly six weeks, 4 commits: `iamClientSecret` (one value reused across all
> five environment files), `appClientSecret` x3 (dev / gamma / gammadev), and `appClientSecret` for
> `app1.pronto.lebara.sa` (named-customer production). Removing them from HEAD remediated nothing; a
> history rewrite followed on 2026-07-26 (`git-filter-repo --replace-text`, 73 occurrences ->
> `REDACTED-SEC-INCIDENT-1`, 0 secrets in 22 598 objects, all 1008 commits preserved, `HEAD` tree
> byte-identical) — and **`refs/pull/*` on the public remote defeated it**, which is why rotation, not
> deletion, was always the fix. The local checkout was verified clean 2026-07-27.
>
> **Why closure is legitimate now.** Rotation was the fix *for the running system*. With that system
> decommissioned there is no issuer entry to rotate and no live SPA to break. ⚠ The values remain in
> public git history; that is harmless **only** if none of the five was reused against another system.
>
> 🔴 **One action falls due ON this closure, and is NOT done:** the pre-rewrite backup bundle
> `C:/sandbox/cgi_decoder/ucc-prerewrite-backup-20260726-203545.bundle` (16 MiB) **still holds all five
> secrets in cleartext**. It was deliberately retained by operator call 2026-07-27 as the only pre-rewrite
> recovery point *while the incident was open*. That condition has now lapsed — **delete it.** It sits
> outside this repo, so no shift can close this from the checkout.
>
> **What prevents recurrence** (unchanged, and not part of this closure): `tools/check-secrets.mjs` fails
> the build on a reintroduced literal (CI `ci.yml` + `.githooks/pre-push`), and is deliberately *not*
> disableable via `UCC_RELEASE_GUARD_DISABLE` — see `PROJECT_NOTES.md`. `BRANCHING.md` keeps this incident
> as the worked example of why the pre-push guard exists.
>
> **Surviving residual, unrelated to the secrets:** internal hostnames/IPs are published in-repo
> (`68.183.16.242`, `p20.prod.pronto`, `app1.pronto.lebara.sa`). Lower severity, still unaddressed; the
> third is now a decommissioned host, the first two are not necessarily.
...
- **⚠ A deployment that wants IdP sign-out must now set `endSessionUrl`.** RP-initiated logout shipped
  ...
  the follow-on if server-supplied OIDC config is ever wanted. ⛔ **NOT buildable as scoped — reviewed
  2026-08-17 and deliberately left standing.** Grounding confirms the row: `BootstrapRoutes` emits exactly
  six keys (`edition`, `features`, `configSpecs`, `enumerations`, `spaces`, `session`) and no `auth` at all
  (`BootstrapRoutes.java:38-51`); the *only* producer of `bootstrap.auth` is the offline mock
  (`mock/handlers/auth.handler.ts:72-79`). Server-side the properties DO exist — `auth.oidc.issuer`,
  `.jwksUri`, `.audience`, `.rolesClaim`, `.clientId`, `.tokenEndpoint`, plus the gateway trio
  (`OidcAuthenticator.java:57-63`, `OidcTokenRelay.java:59-60,125-129`) — but there is no
  `auth.oidc.authorizeUrl` and no `auth.oidc.endSessionUrl`. So this is a **new capability (publish OIDC
  config to the SPA), not a gap**, and nobody has asked for it: `environment.oidc` configures real
  deployments today and works. 🔴 **Trap for whoever does build it:** the resolution in
  `session.service.ts:97-103` uses `??`, so a server-sent **empty string** would BEAT a populated
  `environment.oidc` value and silently disable sign-out redirect. Emit the key only when set, or switch
  that fallback to truthiness — the mock already emits `authorizeUrl: ''` and would demonstrate the bug the
  moment the backend gained an `auth` block.
...
- **server:** `schema` added to `BundleRoutes.APPLY_ORDER` beside `grammar`/`mapping`, so it is written
  **before** the pipeline that names it. `supported()` was already correct and is untouched.
- **UI:** `schema` moved out of `LEGACY_BUNDLE_KINDS` into `BUNDLE_KINDS`, so export offers it again.
  ⚠ `LEGACY_BUNDLE_KINDS` stays **declared but empty** rather than deleted — it is what gives a genuinely
  retired kind a real index in `KIND_ORDER` (a missing one makes `byKindThenId` compute `NaN`) and a place
  in `KNOWN_KINDS`, so an old bundle still parses instead of failing whole-file.
- **comments:** the stale July retirement notes in `transfer/bundle.ts` and `BundleRoutes` now record the
  reversal, so the next reader does not re-derive the wrong conclusion.

🔴 **Two tests asserted the opposite and were inverted deliberately**, each carrying why:
`ControlApiBundleImportTest.aPipelinesCompanionsAreAppliedBeforeThePipeline` (was `assertFalse(...contains("schema"))`)
and a new UI mirror in `bundle.spec.ts`. ⚠ The original refusal was not careless — it cited unification
W1's retirement of 2026-07-31 accurately. What nobody checked was that
`ComponentStore.WRITABLE_TYPES` records the kind **re-added 2026-08-05** with the objection resolved:
`processing.schema_file: schema/<id>` resolves to `registry/schema/<id>`, the exact mirror of
...
- **(a) FIXED — a satellite config was only ever looked for at the write root.** Opening a Parse step's config issues
  `GET /config/schema/<id>` with no `subdir` param. `orders`' schema really lives at
  `spaces/demo/config/orders/orders_schema.toon`, so the backend's `/config/([^/]+)/([^/]+)` route
  matches but the handler finds nothing. `ConfigService.read()` already *accepts* a `subdir` argument —
  the caller isn't passing the one the pipeline's `schema_file` implies. ⚠ Related to the
  `schema_file`-resolution family that has bitten repeatedly; ground the caller, not the route.
- **(b) FIXED — a Dataset with only a `physicalRef` had no readable source.** `orders_dataset`'s stored
  content is `{"physicalRef":"orders"}` — no `sourceName`. `DatasetRowsService.columns()` → `rows(ds,1)`
  therefore calls `GET /db/table?limit=1` with **no `name`**, and the column pickers silently render
  empty rather than saying why. This is the tail of the documented "go-live registration writes a dataset
  with no `sourceName`" defect: the `?? 'data'` default was correctly removed, but nothing replaced it
  with a `physicalRef` fallback or a refusal. ⛔ Do not re-add a default source name.
- **(c) FIXED — the UI could not read an AUTHORED reconciliation's datasets.** 🔴 My first reading of
  this ("demo-data incompleteness") was **WRONG** and grounding refuted it: `orders_regional_recon.toon`
  *does* name three datasets, in the **v2 anchor-first `datasets[]`** form. `fromContent` read only the
  legacy `leftDataset`/`rightDataset`/`thirdDataset`, so the grid showed blank sides and Run posted
  `datasets: []` — hence the server's "got 0". ⚠ The asymmetry is the tell: `serverConfig()` already
  *sent* v2 while `fromContent` only *read* v1. Now dual-read, explicit fields winning.

**Where each fix landed, and the guard that pins it.**

| Fix | Change | Test |
|---|---|---|
| (a) | `ConfigFileSupport.resolveSatelliteForRead` — a satellite resolves beside its pipeline when the convention path misses. **Unique match only** (ambiguity ⇒ today's 404), **reads only**, and an explicit `subdir` always wins. Wired into `ConfigReadRoutes.readConfig` — ⛔ deliberately NOT `deleteConfig`: a read that finds the wrong file shows wrong data, a delete that does destroys it. | `ControlApiConfigSatelliteReadTest` (6 cases, incl. ambiguity, explicit-subdir, and delete-not-widened). **Falsified**: reverting the one call makes it fail 1/6. |
| (b) | `DatasetsService.fromContent` reads a plain `physicalRef` as the source (⚠ a `shared/<owner>/<item>` ref is excluded — it is resolved server-side, not a local table). Plus a seam-level guard in `DatasetRowsService.rows()`: a blank source returns an explaining error instead of issuing a nameless request. | `datasets.service.spec` (3), `dataset-rows.service.spec` (3). ⚠ An existing test asserted the OPPOSITE and was inverted deliberately — its intent, "never a FABRICATED source", is unchanged and still pinned. |
| (c) | `ReconciliationsService.fromContent` dual-reads the v2 `datasets[]` list. | `reconciliations.service.spec` (3, incl. 2-way and both-shapes-present). |

**Re-driven against a live backend after the fixes (2026-08-31):** (a) `GET /config/schema/orders_schema`
**404 → 200**; (b) the column probe is now `GET /db/table?name=orders&limit=1` **400 → 200** and the Link
Analysis column pickers populate (ORDER_ID, ORDER_DATE, REGION, …); (c) the grid shows its Left/Right
datasets and `/recon/run` returns a real board (4 regions matched, totals 1390.6).

⚠ **One residue, and it IS sample data this time:** the 3-way arm of `orders_regional_recon` names
`orders_rollup_dataset`, whose store has no parquet yet
(`spaces/demo/data/rollup/**/*.parquet` — "No files found"). The 2-way run is correct; the third side
needs the rollup pipeline to have run. ⛔ Not fixed by committing generated parquet — data files do not
go in commits.

✅ What the same test PROVED healthy: boot against a real ControlApi with no login (15/15 calls 200 under
`/api/v1`), the pipeline list and editor over `/pipelines/*/graph/raw`, the relocated TS lift/lower, the
parser palette now served by the **real** `ParserRoutes` (`GET /parsers` 200 — it had been answered by
the one UNGATED mock handler even in shipping builds), real backend datasets in the pickers, and zero JS
exceptions. Every console error was one of (a)–(c).

**BUNDLE-SCHEMA-1 — `schema` is a referenced kind that never travels in a bundle (2026-08-31).**
🔴 **This row's original framing was REFUTED the same week, in the opposite direction.** It said the server
wrongly writes a `schema` bundle item that was "retired 2026-07-31 (unification W1)". The retirement was
real but **reversed five days later**: `ComponentStore.WRITABLE_TYPES` records `schema` **re-added
2026-08-05** (ELT amendment Phase 1 slice 3) *with the original objection resolved* — a registry schema is
now genuinely executable. `schema/` sits beside `grammar/` and `mapping/` in
`ConfigSafetyValidator.REGISTRY_REF_PREFIXES`, `RecipeCompiler` emits `processing.schema_file: schema/<id>`,
and the parser rewrites it to `registry/schema/<id>`. So the server writing it was **correct**; the stale
...
schema it executes. That is the identical defect gap 6c fixed for `mapping`. On the UI side `'schema'` is
still in `LEGACY_BUNDLE_KINDS`, so export never offers it: export a pipeline whose
`processing.schema_file: schema/<id>`, import it elsewhere, and the schema does not travel — the pipeline
lands broken.

**Fix (three surfaces, not one line):** (1) add `schema` to `APPLY_ORDER` beside `grammar`/`mapping`;
(2) move `'schema'` from `LEGACY_BUNDLE_KINDS` into `BUNDLE_KINDS` so export offers it again; (3) correct
the stale retirement comments in `transfer/bundle.ts` and `BundleRoutes`. ⛔ Do **not** act on the original
row's "drop `schema` from `supported()`" — that would make the breakage permanent.
...
store with a warning rather than failing boot on a default. ✅ **The FRESHNESS blocker is FIXED
2026-08-31** — `CollectorService.runPipeline` refreshes the projection after every triggered run, pinned
by `StatusProjectionFreshnessTest` (3 cases; **falsified** — commenting out the one call fails 2 of 3,
and the file-store case correctly keeps passing).

🔴 **Grounding refuted this row's own diagnosis in two places.** (1) *"projects the audit once, at boot"*
was **false**: `PipelineScheduler.runOne` already refreshed once a poll cycle's last run finished, and its
comment said so. The real gap was the **trigger paths** — manual API run, `notify`, `on_commit` chain,
dataset-write — which all funnel through `runPipeline` and refreshed nothing, so a triggered commit stayed
invisible until the next cycle. (2) The *"rides along"* worry does **not** occur: `DbStatusStore`
implements `BrowsableStore`, so `DbBrowserRoutes` already lists it as `ops:<id>` / `kind: "operational"`,
and its file lives in the space's `duckdb/` dir, not `data/`. ⚠ The refresh is hooked on `runPipeline`,
**not** on `BatchEventBus` as this row proposed: the bus delivers synchronously on the **ingest thread**
and fires **per batch**, so projecting the whole audit from a listener would put repeated DB work on the
ingest path.

✅ **DEFAULT FLIPPED to `db` 2026-08-31** by operator decision, once the gate was met. Every deployment
now serves the three ledgers from the projection — DuckDB on Personal, PostgreSQL where the bundle carries
the driver **and** a URL. ⚠ Read surface only: the CSVs remain the durable write-ahead, `openStatusStore`
degrades to the file store with a warning rather than failing boot, and `-Dstatus.backend=file` opts out.

🔴 **The flip had one consequence worth recording, and it is the half of the "rides along" note that was
RIGHT.** I refuted the note's framing — the status store is not listed as a *business* store, since
`DbStatusStore` implements `BrowsableStore` and `DbBrowserRoutes` classifies it `ops:<id>` /
`kind: "operational"` — but I wrongly concluded there was nothing to do. An operational group now appears
in `/db/catalog` **by default**, and `ControlApiDbBrowserTest.catalogListsBusinessStores` caught it by
asserting `groups.size() == 1`. That appearance is BY DESIGN (the Data Browser exists to show operational
stores, and `DatasetRowsService.stores()` already filters `ops:*` out of the business list), so the test
was rewritten to assert the invariant that actually matters — **exactly one business group, and every other
...
- ✅ **PKG-2 — CLOSED 2026-08-18: `run.bat ADAPTER` resolves a pipeline for the first time.**
  ...
  finds nothing and every invocation printed `ERROR: no pipeline file found` and exited 1. `for /d` DOES
  glob directories, so the fix enumerates the spaces first and leaves only the filename wildcard to an
  inner `FOR`. Verified on a packaged bundle: **7/7 parity with `run.sh`** across all three bundled spaces
  (`cdr`, `events` in `default`; `payments`, `orders` in `demo`; `voucher`, `sites` in `ucc`; plus an
  unknown adapter ⇒ error + exit 1), including the multi-match `orders` case where both pick the same
  first match. This also made `run.bat`'s `INSPECTO_JAVA_OPTS` pass-through verifiable **end-to-end**,
  which `1768978f` could only probe with the script's own extracted lines.
  ⚠ **The sibling `run.sh` had the mirror defect and is fixed in the same change:** under
  `set -euo pipefail` a non-matching glob made `ls` fail, the pipe fail, and the assignment fail, so the
  script died at that line with **exit 2 and no message** — its friendly error branch was unreachable
  dead code. `|| true` on the assignment restores it (now exit 1 + the message, matching `run.bat`).
  ⚠ Worth knowing when probing `.bat` files here: this sandbox sets `NoDefaultCurrentDirectoryInExePath=1`,
  so a bare `cmd /c run.bat` reports *"not recognized as an internal or external command"* — use
  `.\run.bat`. It looks exactly like a broken script and yields empty "no output" probe results.
...
- ✅ **Branch-aware executor — CLOSED 2026-08-26** (`b3a8bd40` → `23b9265d`, full enterprise reactor
  ...
  **Residuals — all three GROUNDED 2026-08-26, and two of this row's own reasons were WRONG:**
  · **`mode: clone` arming** — ⚠ "needs B9/D8 partial-commit UX" is **STALE**. D8 (the `(batch, branch)`
    commit) SHIPPED, `BranchCommitCoordinator` keys `expectedBranches` by sink NODE id so it is already
    generic over case vs clone, `RowShaper.route` already implements clone fan-out, and **B9 is a
    deliberate ACCEPTED constraint** ("a clone may briefly have some branches committed, others
    retrying") — not missing work. The real reason it stays refused: **nothing surfaces partial-commit
    state to an operator**. Arming it is a product call about that visibility; the engine is ready.
  · **multi-schema + route** — ⛔ **NOT a branch-count fix, and do NOT lift the refusal.** Grounded
    mechanism: `writeAndTrace` is called **once per segment** (`UnionModeIngester`'s loop,
    `destTable = "transformed_" + segKey`) while the divert inside it lifts `PipelineLift.lift(cfg)` —
    the WHOLE multi-schema graph — so arming would run **every schema's route tree against every
    segment's table**. A real fix needs a **segment-scoped lift**: a design change. ⚠ An investigation
    named `BatchGraphRunner.dataFedSinkCount`'s collapsing of identical route keys across schema
    branches as the blocker; it is real but **harmless** — it feeds `engages()` (a `> 1` test, and any
    multi-schema+route count already exceeds 1) plus one diagnostic log line.
...
- **`AiDraft.prerequisites`: the REVIEW half is CLOSED 2026-08-26; the sequencing half stays deferred.**
  The review defect is fixed — prerequisites render with their label and config, so the operator approving
  a non-atomic N-widgets-then-dashboard write can see what each widget will be (`prerequisiteRows` +
  `ai-assist.component.spec.ts`, which had ZERO prerequisite coverage before). ⚠ Two terms in the original
  row were stale by the time it was read: **`configDiff` no longer exists**, and "nothing applies them" is
  wrong — `dashboard-editor.component.ts:294` does. What was actually broken was narrower and worse: the
  surface whose stated job is *"review the STRUCTURE before applying"* showed a bare COUNT.
  ⚠ A prerequisite deliberately carries **no badge of its own** — the tool POOLS every widget's findings
  with the dashboard's into ONE verdict, so an unclean widget cannot hide under a Validated parent, and a
  per-item badge would invent a verdict nobody computes. **Still deferred (unchanged):** generic
  *sequencing* — `kpi_report_builder` remains the only tool with prerequisites, so a shared applier would
  have exactly one caller. **Extract it when a second tool gains prerequisites.** ⚠ **RE-VERIFIED
  2026-08-26 — the trigger has NOT fired.** `prerequisites` is produced in exactly ONE place,
  `ai-draft.ts:121` (the widgets branch); `kpi_report_builder` is still the only tool with them. Building
  the shared applier now would be a single-caller abstraction, which CLAUDE.md §2 bans outright. Do not
  re-open this without a second producer. Original row (provenance):
  `ai-draft.ts:107-129` populates it and the template rendered *"N dependent components will be applied
  first"*, but nothing applies them — each adopting pane must sequence the writes itself.
...
- ~~**D6 — the spec vocabulary is defined by a frontend file.**~~ **CLOSED 2026-08-15 — pinned by a
  ...
  review: `inspecto-ui/src/app/inspecto/contracts/attribute-spec.contract.json` is the one committed artifact,
...
- ~~**M4 Fuse remainder**~~ — **DONE 2026-08-26.** The operator explicitly authorized touching vendored
  `@gamma` shell code, so `src/@gamma/lib/mock-api/` (8 files) is **deleted** and `gamma.provider.ts` no
  longer imports `GAMMA_MOCK_API_DEFAULT_DELAY`/`mockApiInterceptor`, nor carries the `mockApi` provider
  branch or that field on `GammaProviderConfig`. Grounded first: the **only** referrer was
  `gamma.provider.ts`, and the sole `provideGamma(...)` call (`app.config.ts:82`) passes only `gamma:` —
...
- **D11 — ✅ SHIPPED 2026-08-26 (measured 2026-07-27).** The pair is live and on by default, owned by the
  ...
  deferred:** (a) ✅ **SHIPPED 2026-08-26** (*pre-existing, not D11's*): any Save on the Scheduler pane wrote
  `max_concurrent_consignments` into the file, seizing a `-D`-sourced cap's provenance — fixed exactly as
  ...
  **1..100 inside the pattern, not as a separate range check**: the bound has to travel with the served
  ...
  universal constant. The blocker was
...
- ~~🔴 **AUTHOR-1 — an authored `transform.map` config is SILENTLY DROPPED on save.**~~ **SHIPPED
  ...
  - **Follow-on, NOT done (a real second gap, flagged not absorbed):** a map node's *inline* config is
    ...
    **→ ✅ SHIPPED 2026-08-15** (decision: option 1 / refuse the conflict / hand-rolled validation).
    ...
    `okf/backend/pipeline-graph/editable-round-trip.md` §16. Plan archived. The design record below
  ...
  - ~~**Follow-on (b) — the UI picker on hard-refused kinds, "entangled with `needsRef`, design-first".**~~
    **CLOSED 2026-08-15, and the entanglement it was gated on no longer existed.** `bindKindFor` already
    answered `grammar`/`null` and `needsRef` had already been split into `NEEDS_CONFIG`, so nothing was
    owed but the guard. What remains real is that the UI keys on **category** while `USE_HOME` keys on
    **type**, agreeing only because `PARSE` holds one type — now a tripwire: `bind-kinds.contract.json`
    written by `BindKindHomeContractTest` (Java) and read by `pipeline-graph.contract.spec.ts` (TS),
    pinning **a picker requires a home** (⚠ one-way: a `connection/` home needs no picker, since a
    Connection is not a `ComponentType`). ⚠ **The artifact diff alone is not the guard** — a probe giving
    `transform.map` a home left `bindableCategories` unchanged (a category flips only when *every* type in
    it is homed) and was caught solely by the explicit `neitherTransformNorSinkIsBindable` assertion.
    As-built: `okf/backend/pipeline-graph/pipeline-graph-design.md`.
...
- ~~🟠 **DRYRUN-1 — a `transform.join` pipeline cannot be dry-run at all.**~~ **SHIPPED 2026-08-13** — as
  ...
  As-built in `okf/backend/pipeline-graph/editable-round-trip.md` §18. Verified over real HTTP
...
- ✅ **ROUTE-PRED-1 — an armed `route:` branch with no `where` registered and then threw mid-run. FIXED
  2026-08-28.** Found while grounding the map-list `AttributeSpec` row above (the two meet at what a branch
  entry actually is). `RowShaper.route` requires `where` on EVERY branch of BOTH modes (`reqStr`), but
  `RouteArming.refusals` checked only `key`/`database`/`default`/mode/multi-schema — so the shape passed
  every arming rule, armed, and failed on the first row of the first run. ⚠ **Reachable from the product
  itself**: the editor's `addRouteBranch` writes `{key}` and wires the sink, with the predicate typed in
  afterwards via `setRouteBranchWhere`, so "wired but blank" is the normal intermediate state. The gate uses
  the existing severity split, so mid-authoring saves still work — inactive draft ⇒ WARNING, active ⇒
  ERROR/422. ⛔ The `default:` branch is NOT exempt (it is one of `branches[]` and gets its own WHEN;
  "everything else" is the CASE's ELSE arm). 🔴 **Three fixtures encoded the same hole and two called
  themselves "well-formed"** — `RouteArmingTest.branch(key, db)` and
  `ControlApiRouteArmingTest.wellFormedRouteSavesClean` both asserted success for a route that could never
  execute; the control-plane one surfaced ONLY in the full reactor, not in the targeted module run that made
  the change green. As-built: `okf/backend/pipeline-graph/editable-round-trip.md` §19.

- ~~🔴 **AUDIT-CSV-1 — `GET /events/export?format=csv` DROPS every audit attribute.**~~ **✅ FIXED
  2026-08-28, same day it was filed** — option (a), the audit-shaped projection: a CSV export
  filtered to `type=AUDIT` or `type=ACCESS_DENIED` appends one column per `AuditAttrs` key after the
  seven base columns. The column list is **derived from the new `AuditAttrs.ALL`** (declared beside
  the constants; `AuditAttrsTest` pins it by reflection so a new key cannot silently miss the
  export). Unfiltered exports keep the seven-column operational-triage shape unchanged. Covered in
...
| **Step Processor catalog** (2026-09-02) | `ProcessorCatalog` (121 processors, 8 families; 34 delivered / 18 partial / 69 planned) served on `GET /pipelines/processor-catalog`, contract `processor-catalog.contract.json`, rendered by the pipeline palette with undelivered entries inactive. The EDITIONS §Step Processors table is generated from it (`tools/render-processor-board.mjs` regenerates the rows from the contract; `--check` reports staleness). Delivering a processor = give it a node type + attributes + lift/lower, flip its status, regenerate contract + table |
| **Feature × edition board** (`EDITIONS.md` §Feature × edition matrix, opened 2026-09-02) | Working board, cell-addressed (`<row>/<P\|S\|E>`); 2026-09-02 operator decisions: SEC-08 + CMP-08 Enterprise-only; OPS-05/OPS-06/SEC-10/CP-09/CP-11/CP-13/CP-15 (delivery channels) NOT for Personal — all core code today, so **EDG-01** (edition gating: modules or `-D` capability switches, 503-panel UI) is the open build/design slice. Remaining ❓: SEC-07 secrets providers per edition. Deliver cell-wise; a scheduled 🔲 cell cites its row |
