...
- **P2** · **`GRAPH-RAW-COMPANION-ENRICHMENT-ILLEGAL-EMIT-1` — `GET /pipelines/{name}/graph/raw` synthesizes an edge the validator refuses.** `PipelineGraphRoutes.attachCompanionEnrichments` adds `sink.persistent --data--> enrichment` for every `*_enrich.toon` companion; `PipelineValidator` refuses it `ILLEGAL_EMIT`, so an untouched open → save (`PUT …/graph`) and the candidate dry run of any Pipeline with a companion (e.g. the shipped `orders_daily_enrich`) answer 422. No valid rel exists (a same-graph `on_commit` is refused too). **OPERATOR DECISION:** drop the edge / a derived display-only edge kind / a validator exemption for derived companions. `ControlApiPipelineCrudTest` AUTHOR-1(b) misses it (hand-built shape). → `okf/backend/engine/pipeline-test-run.md`.
...
- **P2** · **`POST-ACTION-MOVE-RECOLLECTS-ARCHIVE-1` — a MOVE `archive_path` inside the Collector's root is collected again every cycle.** With the default unbounded `recursive_depth` it is rediscovered next cycle under a new relative path, re-ingested (duplicate rows) and moved again to `archive/archive/…`. Nothing excludes `archive_path` from discovery or refuses the config at save. Repro: `CollectorProcessorRemoteCycleTest#anArchivedFileIsNotCollectedAgain` (disabled). → `okf/backend/pipeline-graph/step-catalog.md`.
- **P3** · **`COLLECTOR-ON-CHANGE-SKIP-IS-REPROCESS-1` — the acquisition form offers `duplicate__on_change: skip`, which `DuplicatePolicy.OnChange.from` does not recognise → silently REPROCESS.** The contract test compares strings only. Fix the options to `ignore` / `alert` / `archive_old_version`. → `okf/backend/pipeline-graph/step-catalog.md`.
...
- **P3** · **`PARKED-BRANCH-LEAK-ON-FAILED-BATCH-1` — a `ParkedBranches` entry for a batch is never drained after an unreadable chunk**; the same leak exists on any non-SUCCESS batch that parked a branch. Residual of `CHUNKED-UNREADABLE-FAILS-BATCH-1` (`3f5183d4`). → `okf/backend/engine/pipeline-test-run.md`.
