...
> someone asks by name"** — so those 33<!--count:backlog-p3--> are mostly a list of things deliberately *not* being built, not a
> backlog to burn down. Reading all 65<!--count:backlog-rows--> as pending work overstates what is owed by roughly half.
...
- **P3** · **`LA-CASE-CREATE-IN-PLACE-1` — the Save-analysis dialog can attach to an existing Case but cannot create one.** Deferred by the operator 2026-09-22 when the merged **Save this analysis** dialog shipped with an *optional* Case: select-existing only, for now. ⛔ **Blocked twice over, and neither half is an oversight.** (1) `POST /objects` refuses a body with no entries in `links` — measured 2026-09-22 for **every** object type, not just `CASE`, so in a space holding no objects nothing can be created through that API at all — and the UI states it as a product decision (2026-07-22, `object-create.dialog.ts:224`): *a case CONTAINS its members*. Link Analysis holds graph **nodes**, which are value-projected entity ids and **not** operational objects, so this screen has no legal link target to offer. ~~(2) `LA-03`'s backend half shipped 2026-09-22 but the SPA is not wired to it…~~ ✅ **HALF DISSOLVED 2026-09-23 — LA-03 is COMPLETE.** The SPA now calls `POST /inv/snapshots` and `/attach`; `add`/`attach` are **removed**, `save`/`attachTo` return Observables, and the signal updates only after the server confirms. So an attachment made here IS durable, and the “durable Case carrying a session-only attachment” objection no longer applies. ⇒ **only blocker (1) remains**: `POST /objects` refuses a body with no `links` entries, and a Link Analysis node is a value-projected entity id, not an operational object — so this screen still has no legal link target to offer. Whoever picks this up decides ONE question first: may a Case be born with no members (relaxing the 2026-07-22 decision), or does Link Analysis gain a way to mint an operational object from a graph node? → `okf/frontend/features/link-analysis.md`.
...
- **P3** · ⚠ **HALF FIXED 2026-09-22 (`WB-17`)** — `lookup_step` now ships a sample (all three `mappings` plus an unmapped status), so every file-fed Pipeline can be test-run as shipped; `tools/seed-samples.mjs` reports a genuinely empty inbox instead of guessing from directory names. **Still open: the five domain-shaped demos** (ASN.1, Excel, fixed-width, XML, `route`), which is the Could-tier bulk of this row. Original row: **`DEMO-CORPUS-FORMAT-COVERAGE-1` — five authoring surfaces exist only as synthetic `spaces/default` examples, and one ships no sample at all.** ASN.1, Excel, fixed-width, XML and `route` have no domain-shaped demo; `lookup_step` has **no sample file**, so the sweep — and any builder — cannot test it as shipped. Every finding above for those surfaces rests on one file each. A realistic demo per surface is what would have found `TESTRUN-SEGMENT-ROUTE-NO-FLOW-1` before a sweep did. → `superpower/pipelines-workbench-moscow.md`.
