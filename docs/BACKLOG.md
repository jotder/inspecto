...
|---|---|---|
| **Delimited Grammar redesign (2026-08-19)** | ~~(a) **Canvas node rename**~~ **✅ SHIPPED 2026-08-19 (`e004ae26`)** — the inspector gained an inline pencil-edit for name/description (hidden read-only; a rename deliberately does NOT invalidate the node's test outcome), and the Parse pane's Name/Description fields are deleted, Apply carrying the stored identity verbatim. (b) **E4 full finalization harness** — the ledger contiguity is stress-pinned, but `DbConsignmentOutputStore`/`DbAcquisitionLedger`/`DbFileStageStore` + marker/manifest fail-fast under concurrent `BatchProcessor.finalizeSource` remain structurally argued only. (c) **/design gallery** — the tabbed grammar editor + icon-type-menu patterns are not yet shown in the gallery. (d) **D1(b) column-metadata grid** — description/unit/classification per column (model shipped long ago; `synonym` shipped 2026-08-19) still has no UI on the Files & metadata tab; project it via the editor's `[tabFiles]` slot. | `okf/frontend/features/grammar-config.md` · `okf/backend/engine/ingest-wrap-spi.md` |
