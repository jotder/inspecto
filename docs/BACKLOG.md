...
| **DOCS-LINKS-1 — 8 broken `superpower/` links in current docs (measured 2026-08-27; all PRE-EXISTING, from earlier shifts' archive moves)** | Open, **small and concrete — a good first pickup.** Six files hold them: `BACKLOG.md`, `GLOSSARY.md`, `okf/backend/architecture-layers.md`, `okf/backend/control-plane/job-vs-step.md`, `okf/backend/engine/consignment-addressing.md`, `okf/frontend/features/reconciliation.md`. **7 of 8 are a mechanical repoint** — the targets all exist under `archived-documents/plans-archive/`: `component-model`, `consignment-addressing-plan`, `geo-map-analysis-plan`, `job-parameter-contract-plan`, `link-analysis-and-graphsource`, `link-analysis-studio-plan`, `modularization-optimization-plan`. ⚠ Mind the relative depth per file (`okf/backend/...` needs `../../`, `okf/frontend/features/...` needs `../../../`). 🔴 **The 8th needs a decision, not a repoint: `superpower/reviews/reconciliation.md` exists NOWHERE** — not in `superpower/`, not in the archive. Decide whether to drop the link or point it at the OKF concept. ⛔ Do NOT 'fix' struck-through `~~superpower/x.md~~` mentions — those cite the old path deliberately for provenance and are not links. A name-only grep over-reports this ~5x (44 hits vs 8 real); match `](...)`. |
