...
|---|---|---|
| **PARK-1 — park/drain residuals (ELT Phase 4 S4, D-13; filed 2026-08-29 when S4 shipped whole)** | Three deliberate deferrals, all decision-gated, none blocking: (a) **no manifest-level park detail is served** — a UI can list parked Consignments off the ledger's `status=PARKED`, but which BRANCH parked and where its park table sits live only in the on-disk manifest (`parkedAt`/`parkedTables`); a finer inspection surface needs a new read route, not a client-side derivation. (b) **A batch with unpack-EXPANSION members is REFUSED, not drained** — its archive original stayed in the inbox (moving a shared original would strand sibling batches), so it re-expands and re-parks as a fresh batch each cycle; draining the stale one would commit rows the new one is about to write. Making those drainable needs the shared-original lifecycle solved first. (c) **The offline mock seed has no `route:` pipeline**, so the canvas Step switch cannot be exercised in the preview (it is unit-tested at both levels); adding one to the seed would close that. | [Per-Step enabled — park and drain](okf/backend/pipeline-graph/step-park-drain.md) |
