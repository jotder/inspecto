...
- **P2** · **WORKBENCH-S4** — the one-surface Step workbench (input-relation picker, column filter, grouping beside the field list). Grounded 2026-09-06: none of the three controls exists; the S1/S2/S5 plumbing they would host does (`upstreamColumns`, `/components/transform/describe`, the preview components); no "list a node's input relations" endpoint — the relation is implicit. `RowShaper.fuse` was DELETED the same day (caller-less, lossy). Needs a UI design before build. → `okf/backend/engine/catalog-vs-executors.md` §distilled
...
- **P2** · **D4-ENRICH-1** — the D-4 migration (`enrichment` → `transform: {join}` + read-only kind) is BLOCKED by a design gap, grounded twice on 2026-09-06. `orders_daily_enrich.toon` is a join PLUS an aggregate over the LANDED `orders` store; the lift (`PipelineLift.stageTwo`) reads ONE `steps[]` + ONE `output_store:` per pipeline file and seeds Stage 2 from that pipeline's own store — a second `type: pipeline` job over the same file runs but yields the identical chain, and sourcing another pipeline's store is not wired (not forbidden — there is no check to remove). Options: (a) N named chains per pipeline (reshapes `steps[]`/`output_store:`, the lift/lower round trip, `PipelineEditable`, the editor); (b) a job-level `source_pipeline:` + a `stageTwo` source override + a cross-pipeline store-access policy; (c) keep `enrichment` first-class — it has capabilities the Stage-2 chain lacks (multi-reference with SCD2 `as_of`, dual `on_pipeline` + `schedule_seconds` triggers, event-time partition bounds). ⚠ Grounding RECOMMENDS (c) narrowed: migrate only single-join/aggregate enrichments expressible as one chain and carve join+aggregate-over-landed-store out — which reverses half of the D-4 GO decision, so it is the OPERATOR's call. → `superpower/elt-final-amendment-plan.md` §D-4, `okf/backend/engine/post-sync-step-chains.md`
