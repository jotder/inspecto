...
The residuals of the 2026-09-23 drain that closed the domain-demo rows above (`e7f8e2aa` … `d8ce162e`). ⚠ Each is the lane's finding; `VALIDATE-CONFIGPATH-UNJAILED-1` was explicitly **unverified** when filed (since reproduced and fixed, `8d96929b`).
...
- **P2** · **`PREVIEW-REFERENCE-PATH-UNJAILED-1` — two preview routes read a caller-supplied reference data file with no path jail** (security). `POST /pipelines/authored/{id}/dry-run` and `POST /enrichment/preview` read a join step's `path:` data file unjailed and return its rows (or the resolved path in a 422). Deliberate for now per the `PipelineGraphRoutes.dryRunReferences` javadoc (data files live outside the write root), but both routes need ONE decision — e.g. jail against `PathJail.allowedRoots()` like `/validate {configPath}` (`VALIDATE-CONFIGPATH-UNJAILED-1`). Needs an operator decision. → `okf/backend/config/config-safety.md`.
- **P2** · **`PROCESSOR-RELEASE-READINESS-1` — releasability audit (2026-09-23) of the 37 DELIVERED processors against a draft 7-point bar** (documented · real-path test · workbench preview/lift/lower · UI palette · edition gating · fails clearly at save · demo). ~22 pass nearly all. Gap groups: **G1** catalog↔docs drift (`parser.asn1.ber` maps to no node type → inactive in the canvas; `schema.drift` Partial in SC vs Delivered in the contract; three different delivered counts; a stale "Standard+" name) · **G3** graph save and config write run DIFFERENT gates (graph save skips `unknownConnectionFindings` + `AcceptedConfigKeys`; config write lacks `routeColumnFindings` + `summarizeMeasureFindings`; `webhook.connection` not checked at save) · **G4** step config errors only at run (lookup, profiler, dedup keys, filter; `circuit_breaker` / `retry` / `fetch.rate_limit` not in `ConfigSpecs`) · **G5** Collector keys undocumented / not in the form · **G6** Collector features unit-tested only (breaker, throttle, `post_action` MOVE, SFTP/JDBC pipeline runs) · **G7** at-rest real-path tests (profile, lookup, webhook branch) · **G8** dry-run blind spots (⚠ an enrichment node is SILENTLY skipped — `PipelineExecutor.isShapeable:406`; the graph dry run misses the webhook Personal refusal) · **G9** edition board vs code (`alert.dispatch`, archive, ducklake not gated as `EDITIONS.md` says) · **G10** missing demos · **G11** docs for non-Step processors. Questionably DELIVERED: `schema.drift` (detection only), circuit breaker / throttle (thin), `constraint.check` / `alert.dispatch` / `sink.quarantine` (capabilities, not Steps), `sink.archive` (move-on-success), `db.jdbc` (no pipeline test). **AWAITING operator sign-off** on the bar and on what to do with the questionable ones. → `okf/backend/pipeline-graph/step-catalog.md`.
- **P3** · **`VALIDATE-PREPARE-WRITES-STATUS-DIR-1` — `POST /validate {configPath}` is not strictly read-only.** It runs `PipelineConfig.prepare()`, which creates the Pipeline's status directory, so `CapabilityManifest`'s "read-shaped, writes nothing" exemption for the route is not strictly true. → `okf/backend/config/config-safety.md`.
...
- **P3** · **`CHUNKED-UNREADABLE-FAILS-BATCH-1` — the chunked native CSV lane never quarantines an unreadable file.** `NativeCsvStreamingEngine.chunkedIngest` has no quarantine catch: an unreadable chunk FAILS the batch, the file stays in the inbox and retries every cycle instead of `QUARANTINED_UNREADABLE`. Fix: the same `COUNT(*)` read check per chunk that `SINGLE-MEMBER-TRANSFORM-FAILURE-QUARANTINES-1` (`73a4e758`) added. → `okf/backend/engine/pipeline-test-run.md`.
- **P3** · **`CONFIGCODEC-CALLERS-NO-FILE-NAME-1` — ~20 readers decode TOON themselves and their errors do not name the file.** They call `ConfigCodec.toMap(Files.readString(p))` (`DataSourceRoutes`, `AlertRule`, ops Tag / Workflow, the `ConfigMigrator` family): a decode refusal names the line but not the file. Switch them to `ToonHelper.load`. Residual of `CONFIGCODEC-LENIENT-IS-STRICT-1` (`5c525689`). → `okf/backend/config/toon-config.md`.
