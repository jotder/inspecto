...
**Accurate: 12 of the 16 gates were RUN here — all 12 still gated. 0 discharged.** Plus 2 gates outside
this section (§3 Platform Services Stage 2, §5 D8-SUPPRESS-1), also run, also holding: **14 run, 14 hold.**
The 4 not run, and why: **Compliance program (NFR-7)** — external parties only, no repo-side check exists
by design (its repo-side half moved to §5 on the first pass); **AGT-5** and **AGT-6b** — cross-repo
`jotder/inspect-agent`, not checked out here; **Completeness KPI hold** — needs a query over a month of
landed filenames, and that corpus is off-repo (see DATA-GOV-1, two rows up). ⚠ A gate you cannot run is
not a gate that holds — it is a gate with no evidence either way, and the two must not be summed.
...
| **AGT-5 `DryRunProvider` + `incident_explain`** | Per-tool preview seam on `PlatformBuilder` (lets inspecto drop its parallel `AgentApprovals` previewer); `incident_explain` waits on the eoiagent host seam. Local-models-only scope cut stands. | ⬜ **NOT RUN 2026-09-07** — the repo is not checked out here. Cross-repo `jotder/inspect-agent`: **reopen when it ships a per-tool `DryRunProvider` seam.** ("Demand" dropped — it named no watcher and no definition.) → `superpower/agt-6-plan.md` §4.2 G2 |
| **AGT-6b multi-step agent graphs** | First cut = generalize `RunbookActions`, never free-form ReAct over mutating tools. | ⬜ **NOT RUN 2026-09-07** — same reason as AGT-5. Same upstream repo, two concrete preconditions: the eoiagent approval gate is synchronous per-call (nested gates deadlock) and there is no per-tool `DryRunProvider` seam. Reopen when both are gone. → `superpower/agt-6-plan.md` §4 |
...
| **Completeness KPI hold** | K2 wiring / K4 / K5 held: whether `{seq}` restarts per hour is a carrier fact. | ⬜ **NOT RUN 2026-09-07** — the query needs the off-repo corpus (DATA-GOV-1). 🔴 **Not actually external.** The plan records the filename shape `CDR_{yyyyMMddHH}_{seq}_*` and `GapDetector` already parses those tokens, so **a query over one month of landed filenames answers it** without the carrier. Reopen by running that query — it is roughly half an hour of work, not a blocked dependency. → `superpower/completeness-kpi-plan.md` §2b |
