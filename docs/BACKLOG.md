...
**Open:**
- **D7 phase 2 — the tag split-brain (the one that will confuse users).** Phase 1 shipped the central
  assignment store without migrating the legacy path, so **two tag stores are live at once**: a Tag Rule
  writes `attributes.tags` (CSV inside the object, `ObjectService.ATTR_TAGS`, five write sites, all reads
  through `csvTags()`), while `POST /tags/assignments/…` writes the assignment store. Neither is visible
  to the other — `GET /tags/{name}/targets` will not list a rule-tagged Incident. Fix = the one-time
  backfill Q2 called for, then reimplement `csvTags()` over the assignment store (it is the single read
  helper, so this is a one-function switch). **Do this before any tag UI**, or the UI will show one half
  of the truth. → `okf/backend/control-plane/tags.md`
- **D7 — `rename`/`removeTag` have no route.** Both are implemented and tested on `TagAssignmentStore`
  (rename is the operation the whole central-store architecture was chosen for), but nothing calls them:
  there is no rename endpoint, and `POST /tags` has no delete counterpart, so a tag removed from the
  registry would leave its assignments orphaned. Wire them together with a registry delete.
...
| Generic tag system | D7 (rescoped) · **phase 1 shipped 2026-07-26**, phase 2 open (§6) · `okf/backend/control-plane/tags.md` · `superpower/generic-tags-plan.md` · ⚠ supersedes the old §4 "`category`/`tags` params on `GET /objects`" row — the *existing* `Tag`/`TagRule`/`/tags` system is what gets generalized, not a separate feature. Keep aligned with **D10** (generalized notes): same `(kind, id)` addressing problem, so one adopts the other's scheme |
