...
| Metrics (Prometheus) | `GET /metrics` — throughput, error rate, lag |
...
- **Config changes** — validated drafts only (`POST /api/v1/validate` → `/api/v1/config/write` under the fail-closed
