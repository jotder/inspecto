...
| `batch.max_files` / `batch.max_bytes` / `batch.order` (`name`\|`mtime`) | `processing:` / `  batch:` / `    max_files: 500` | `ConsignmentPlanner` |
...
Extra JVM flags go in **`INSPECTO_JAVA_OPTS`** (fallback `EXTRA_JAVA_OPTS`) — honored by all six
launchers (`serve`/`run`/`ura` × `.sh`/`.bat`), whitespace-separated, appended after the script's own
flags so the mandatory ones can't be clobbered, and echoed at launch so an inert flag is visible.
`JAVA_OPTS` is *assigned* by the scripts and is silently discarded — do not use it.
...
**E. Steps** `[done 2026-09-06]` — `examples/07-steps/`: one example per Step kind of the (since retired, 2026-09-24) recipe-verb catalogue — collect · route · sink one-shot; filter · dedup · join · summarize · sql at rest through an on-commit `type: pipeline` Job (`serve-example … --demo`). The README's Step catalog maps all 15 kinds to an example.
