...
| Dir | artifactId | Role |
|---|---|---|
| `inspecto/` | `inspecto-processor` | Lean engine + control plane. No network/AI deps. The fat JAR. |
| `inspecto-agent/` | `inspecto-agent` | Assist skills (failure diagnosis etc.); vendored kernel layer + eoiagent model transport. Optional. |
| `inspecto-agent-hosted/` | `inspecto-agent-hosted` | Hosted model providers (langchain4j). Optional. |
| `inspecto-connectors/` | `inspecto-connectors` | SFTP/FTP/FTPS/DB-export connectors. ServiceLoader-discovered; optional. |
