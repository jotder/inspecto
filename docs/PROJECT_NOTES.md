...
| **Pipelines workbench as a GENERIC ingest-authoring surface (2026-09-22)** — where it stands per format × surface, and the plan to make every trust surface give one answer | [`pipelines-workbench-moscow.md`](superpower/pipelines-workbench-moscow.md) (v2, all 26 shipped Pipelines swept; §3.1 coverage matrix) · [`workbench-trust-plan.md`](archived-documents/plans-archive/workbench-trust-plan.md) (`WB-01`..`WB-20`, decisions `D2`–`D9` awaiting signature, §8 exit contract with a 30-day stall clock) · ⚠ the *Recipe view* named on this page's editor row **no longer exists** (`6d3c68fa`, 2026-09-18) — `pipeline-editor.md:550` records the removal |
