...
| Authoring redesign for business users — Parse pane (Delimited first) + `transform.sql` Step with a Simple fields grid that generates the SQL (**two active plans**, decided 2026-09-03; clickable mockup source in `superpower/assets/authoring-redesign-mockup/`) | [`parse-pane-redesign-plan.md`](superpower/parse-pane-redesign-plan.md) · [`sql-transform-v1-plan.md`](superpower/sql-transform-v1-plan.md) · current-state truth: [`schema-mapping-authoring.md`](okf/frontend/features/schema-mapping-authoring.md), [`catalog-vs-executors.md`](okf/backend/engine/catalog-vs-executors.md) |
