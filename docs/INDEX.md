...
| [`recon-cardinality-plan.md`](superpower/recon-cardinality-plan.md) | ✅ **TIER 1 SHIPPED end to end** (2026-09-12; tier 2 demand-gated, one decision open in §7) | `RECON-CARDINALITY-1`: one-to-many / many-to-many recon. ⚠ **NOT a release gate** — the whitepaper's cardinality sentence was deleted by the v1.2 rewrite (`db11a412`) a day BEFORE the build-over-drop decision was recorded. 🔴 The row's premise holds but understates it: duplicates are **summed away before the join** (`sideSql:290-302` → `joinChain:342-351`), so cardinality is structurally unreachable, not merely unimplemented. ✅ The finding that shrinks the work: `COUNT(*) AS mr` **already carries per-key multiplicity through the join** and is discarded as a boolean at `:495-497`. ⇒ Recommends **tier 1 only** — cardinality as an *assertion* (a fourth break type, no join change, sum-vs-value semantics unchanged), because for one-invoice-vs-three-payments today's arithmetic is already right; the real hole is that a **duplicate row reconciles clean**. Row-level pairing (tier 2) is demand-gated — its Break shape cannot be designed without a named workflow. |
