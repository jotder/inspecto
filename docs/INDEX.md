...
| [`duckle-concepts-candidates.md`](superpower/duckle-concepts-candidates.md) | **TRIAGE LIST — nothing decided, nothing built** (2026-09-14) | Ten main + fifteen smaller operational rules distilled from slothflowlabs/duckle (Dataset freshness SLA, run diff, dead-property refusal, narrowing-only policy, baseline QA Expectation, admission-only pools …), each grounded against `inspecto*/src/main` and BACKLOG. ⛔ Not BACKLOG rows yet — operator adds/removes/prioritises here first; survivors are filed, then the file is archived. Out of scope by operator decision: git integration, component count, UI languages. |
