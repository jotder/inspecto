...
| [`run-model-plan.md`](superpower/run-model-plan.md) | **PLAN — nothing built** (2026-09-12; one decision open in §6) | §13's **Run model**, the named blocker left by `CONSIGNMENT-ID-DETERMINISTIC-1`. ⚠ "§13's Run model" had been a **forward pointer with no content** — cited from five places, specified nowhere; this file is that missing spec. `GLOSSARY.md` §6-A already binds the semantics (`Run ⊇ Consignment ⊇ File`, identity `(consignment_id, run_id)`, a reprocess is a **new Run over the same Consignment**), so the operator's key shape was right — what was refuted is that it is implementable today: `run_id` is unconditionally NULL at all four `record()` sites. 🔴 Carries the correction that a run id **cannot** discharge D15 — `newRunId` is clock+in-memory-counter derived, so two pods mint different ids; split-brain is stopped by the **fenced `RunLease`**, and this constraint only covers retry idempotency and reprocess correctness. Four slices, the third (the two framework-less paths) being most of the cost. |
