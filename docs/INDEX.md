...
| [`connection-pool-plan.md`](superpower/connection-pool-plan.md) | **IN FLIGHT** (started 2026-09-14) | The last unbuilt item of scale-out **phase A** (`enterprise-scale-out-plan.md` §5.1): HikariCP behind `JdbcDrivers`, stores borrowing per operation instead of holding one connection for life. 🔴 Grounding corrected the parent plan three ways — three of §5.1's five bullets were already done, the store count is **15** not the archived plan's eight/ten, and phase A's boot-failure verify gate is **already met** (only the pool-saturation test is still owed). ⛔ DuckDB must stay an effective pool of **1** (single-writer file lock); only Postgres pools. |
