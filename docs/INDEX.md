...
| [`route-gating-audit.md`](superpower/route-gating-audit.md) | **AUDIT COMPLETE 2026-09-15 — nothing changed in code** | Every ungated MUTATING control-plane route, classified. Commissioned by the operator's "audit all first" decision on `ROUTE-UNGATED-DEFAULT-1` (P1). **332 registrations · 91 gated · 241 ungated · 83 mutating**, enumerated with the shape `CapabilityManifestTest` already trusts (an earlier ad-hoc regex matched only 304 and missed `PATCH`). 🔴 **The finding reframes the row: a VOCABULARY gap, not 83 oversights** — only **12** are gateable with an existing capability, **22** cannot be expressed at all (Space lifecycle · Incident/Case triage · agent governance), and **`Roles` has no admin capability**, so "unlisted ⇒ admin" is not expressible today. ⛔ Do not ship the ratchet first — it would freeze 83 unreviewed exemptions into a baseline. |
