...
| [`dataset-column-derivation-plan.md`](superpower/dataset-column-derivation-plan.md) | **DESIGN ONLY for the headline derivation — two of its parts SHIPPED** (2026-09-11); **three decisions open in §6** (Q2, Q3, Q4 — ⚠ this row said "four" until 2026-09-14; Q1 was answered by building it) | `TYPEFLOW-DATASET-COLUMNS-1`: should a Dataset's `columns{name,type,role}` be derived rather than hand-authored. 🔴 Grounding refuted the row's framing: the role heuristic ALREADY existed in **three** unpinned copies (one Java, two TS). ⚠ **The "refresh REPLACES the document, destroying authored roles" defect this row used to state in the present tense was FIXED 2026-09-11** — `MaterializeTask` now merges keys-by-owner, restating only its four job-owned provenance keys. ⇒ the contract pin and the merge-not-replace fix both landed; only the headline derivation is unbuilt. §7 closed 2026-09-14: exactly one committed dataset carries authored roles and no committed job materializes, so the sample tree cannot regression-test that fix. ⛔ Option A needs no `TypeFlow` at all, despite the parent row naming it. |
| [`route-gating-compliance-plan.md`](superpower/route-gating-compliance-plan.md) | **DRAFTED 2026-09-15 — ✅ step 1 SHIPPED; steps 2–4 sequenced** | The audit’s second half, reframed for *"goal is to pass compliance"*: 🔴 **the enforcement model is fail-OPEN** — `withCapability` is opt-in and an undeclared route is simply open, a mechanism finding that outranks any single route. Four steps: gate the five (incl. a live request-forgery pair in `/assist/settings*`); decide the remainder so every mutating route is a capability OR a categorized exemption; **flip the default** — a marked `Gated` handler makes the capability visible to the router with zero call-site changes, and an undeclared mutating route then refuses at boot AND fails CI; audit events carry the capability, one inventory event per boot, and a **derived, CI-enforced** route-gating evidence report under `compliance/evidence/` (step 4e — not yet written; the seven files there today are all hand-authored). ⛔ Nine things to avoid, each a way to look compliant without being. |
