...
- 🟡 **Pipeline authoring polish — PART-SHIPPED.** The dedicated run endpoint for authored Pipelines ✅ shipped (`POST /pipelines/authored/{id}/run`); rounding out the visual editor continues as open P2 letters; the adapter stream-consumer runtime for streaming Collectors ✅ shipped 2026-09-24 as a hardened per-cycle Collector drain with at-least-once delivery (`STREAM-CONSUMER-1`, closed).
