...
export class ExpectationsComponent implements OnInit {
  ...
  readonly columnDefs: ColDef<Expectation>[] = [
    { field: "name", headerName: "Expectation", flex: 1, minWidth: 180 },
    {
      ...
      width: 120,
      valueGetter: (p) =>
        p.data ? (KIND_LABELS[p.data.kind] ?? p.data.kind) : "",
    },
    {
      headerName: "Target",
      flex: 1,
      ...
      cellRenderer: (p: ICellRendererParams<Expectation>) =>
        p.data?.lastResult
          ? statusBadgeHtml(p.data.lastResult.status) +
            (p.data.lastResult.violations
              ? ` <span class="text-secondary text-xs">${p.data.lastResult.violations}</span>`
              : "")
          : "—",
    },
    {
  ...
  history(e: Expectation): void {
    this.dialog
      .open(ComponentHistoryDialog, {
        data: { type: "expectation", id: e.name, label: e.name },
      })
      .afterClosed()
      .subscribe((restored) => {
  ...
  evaluate(e: Expectation): void {
    this.api.evaluate(e.name).subscribe({
      ...
      error: (err) =>
        this.toastr.error(
          apiErrorMessage(err, `Could not evaluate "${e.name}".`),
        ),
    });
  }
    ...
    this.api.evaluateAll().subscribe({
      next: (rows) => {
        ...
        const failed = rows.filter(
          (r) => r.lastResult?.status === "FAILED",
        ).length;
        if (failed)
          this.toastr.warning(
            `${failed} expectation(s) FAILED — Incidents were raised.`,
          );
  ...
  async remove(e: Expectation): Promise<void> {
    ...
    this.api.remove(e.name).subscribe({
      ...
      error: (err) =>
        this.toastr.error(
          apiErrorMessage(err, `Could not delete "${e.name}".`),
        ),
    });
  }
