...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>
      {{
        isImport
          ? 'Import bundle into "' + data.spaceId + '"'
          : "Create space from bundle"
      }}
    </h2>
      ...
      @if (!isImport) {
        <mat-form-field class="w-full" subscriptSizing="dynamic">
          ...
          @if (newId.hasError("required") && newId.touched) {
            ...
            <mat-error
              >Use a–z, 0–9, hyphen; start with a letter or digit; max 63
              chars.</mat-error
            >
        ...
        <div class="space-y-3 rounded-lg border p-3">
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1">
                @for (d of p.dataSources; track d) {
                  <span
                    class="rounded bg-gray-100 px-2 py-0.5 text-xs dark:bg-gray-700"
                    >{{ d }}</span
                  >
                }
              </div>
          ...
          <div class="text-secondary text-sm">
            {{ p.files.length }} file(s){{
              p.hasSpaceToon ? " · includes space.toon" : ""
            }}
          </div>
          ...
          @if (p.conflicts.length) {
            <div class="space-y-1">
              ...
              <div class="text-sm">
                These data sources already exist in “{{ data.spaceId }}”:
              </div>
              <div class="flex flex-wrap gap-1">
              ...
              <mat-checkbox [formControl]="overwrite"
                >Overwrite existing data sources</mat-checkbox
              >
            </div>
          }
    ...
    <mat-dialog-actions align="end">
      <button type="button" mat-button mat-dialog-close [disabled]="busy">
        Cancel
      </button>
      @if (isImport) {
        <button
...
export class ImportBundleDialog {
  ...
  private runPreview(): void {
    ...
    this.api.importPreview(this.data.spaceId!, this.file!).subscribe({
      next: (p) => {
        ...
        this.preview = p;
        if (!p.valid)
          this.toastr.warning("Bundle has validation errors — see findings.");
      },
      error: (err) => {
        this.previewing = false;
        this.toastr.error(
          apiErrorMessage(err, "Could not analyse the bundle."),
        );
      },
    });
  }
  ...
  doImport(): void {
    ...
    this.busy = true;
    this.api
      .importBundle(this.data.spaceId!, this.file!, this.overwrite.value)
      .subscribe({
        next: (r) => {
          this.busy = false;
          this.toastr.success(
  ...
  findingEntries(
    p: ImportPreview,
  ): { file: string; findings: ImportPreview["findings"][string] }[] {
    return Object.entries(p.findings).map(([file, findings]) => ({
      file,
      findings,
    }));
  }
