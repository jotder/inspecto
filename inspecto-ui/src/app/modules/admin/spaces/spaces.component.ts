...
import {
  SpaceTemplateGalleryData,
  SpaceTemplateGalleryDialog,
} from "./space-template-gallery.dialog";
...
export class SpacesComponent implements OnInit {
  ...
  newFromTemplate(): void {
    ...
    this.dialog
      .open(SpaceTemplateGalleryDialog, {
        data,
        width: "640px",
        maxHeight: "85vh",
      })
  ...
  private openImport(data: ImportBundleData): void {
    this.dialog
      ...
      .subscribe((changed?: boolean) => {
        if (!changed) return;
        this.reload();
        if (data.spaceId && this.expanded[data.spaceId])
          this.loadDataSources(data.spaceId, true);
      });
  }

  toggleDataSources(s: Space): void {
    this.expanded[s.id] = !this.expanded[s.id];
    if (this.expanded[s.id] && this.dataSources[s.id] === undefined)
      this.loadDataSources(s.id);
  }
  ...
  private loadDataSources(id: string, force = false): void {
    ...
    this.spaces.dataSources(id).subscribe({
      ...
      error: (e) => {
        this.dsLoading[id] = false;
        this.toastr.warning(
          apiErrorMessage(e, `Could not load data sources for "${id}".`),
        );
      },
    });
  }
  ...
  exportSpace(s: Space): void {
    this.spaces.exportSpace(s.id).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Could not export "${s.id}".`)),
    });
  }
  ...
  exportDataSource(id: string, ds: string): void {
    this.spaces.exportDataSource(id, ds).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Could not export "${ds}".`)),
    });
  }
  ...
  async remove(s: Space, purge: boolean): Promise<void> {
    ...
    if (
      !(await this.confirm.confirmDestructive(msg, {
        title: purge ? "Delete & purge" : "Unload space",
        requireText: s.id,
      }))
    )
      return;
    this.spaces.remove(s.id, purge).subscribe({
      next: () => {
        this.toastr.success(
          `Space "${s.id}" ${purge ? "deleted (purged)" : "unloaded"}`,
        );
