      ...
      <mat-dialog-content class="space-y-3">
        ...
        @if (editMode) {
          <div class="text-secondary text-xs">
            Id: <span class="font-mono">{{ data?.space?.id }}</span> — cannot be
            changed.
          </div>
        } @else if (!editId()) {
          <div class="text-secondary flex items-center gap-2 text-xs">
            ...
            >
            <button type="button" mat-button (click)="editId.set(true)">
              Edit id
            </button>
          </div>
        } @else {
          <mat-form-field class="w-full" subscriptSizing="dynamic">
            ...
            />
            <mat-hint
              >lowercase letters, digits and hyphens — becomes the space
              folder</mat-hint
            >
            @if (form.get("id"); as c) {
        ...
        <div class="flex flex-col gap-2 border-t pt-3">
          ...
          <div class="flex items-center gap-4">
            <div
              class="bg-hover flex h-14 w-28 items-center justify-center rounded border p-2"
            >
              <img
            ...
            <div class="flex flex-col gap-1">
              <input
                ...
                accept="image/*"
                class="sr-only"
                aria-labelledby="space-logo-label"
        ...
        <mat-form-field class="w-full" subscriptSizing="dynamic">
          ...
          <input
            ...
            autocomplete="off"
            placeholder="Unveil stories from your data"
          />
...
export class SpaceFormDialog {
  ...
  constructor() {
    ...
    if (s) {
      ...
      this.form.controls["display_name"].valueChanges
        .pipe(takeUntilDestroyed())
        .subscribe((v) => {
          if (!this.idManual)
            this.form.controls["id"].setValue(slugify(String(v ?? "")), {
              emitEvent: false,
            });
        });
    }
    ...
    save$
      ...
      .subscribe({
        next: ({ space }) => {
          this.saving = false;
          this.toastr.success(
            `Space "${space.id}" ${existing ? "updated" : "created"}`,
          );
        ...
        error: (e) => {
          ...
          const msg =
            e?.status === 409
              ...
              : apiErrorMessage(
                  e,
                  `Could not ${existing ? "update" : "create"} "${id}".`,
                );
