...
export class JobsComponent implements OnInit, OnDestroy {
  ...
  readonly columnDefs: ColDef<JobView>[] = [
    { field: "name", headerName: "Job", flex: 1, minWidth: 160 },
    {
      ...
      flex: 1,
      minWidth: 150,
      valueGetter: (p) => (p.data ? whatScheduled(p.data) : ""),
    },
    {
      headerName: "Schedule",
      flex: 1,
      minWidth: 150,
      valueGetter: (p) => (p.data ? scheduleSummary(p.data) : ""),
    },
    {
      field: "nextFire",
    ...
    },
    {
      ...
      headerName: "Enabled",
      width: 110,
      cellRenderer: (p: ICellRendererParams<JobView>) =>
  ...
  readonly reportColumnDefs: ColDef<JobRunRow>[] = [
    ...
    { field: "trigger", headerName: "Trigger", width: 130 },
    {
      ...
      headerName: "Status",
      width: 110,
      cellRenderer: (p: ICellRendererParams<JobRunRow>) =>
        statusBadgeHtml(p.value as string),
    },
    {
      field: "durationMs",
      headerName: "Duration",
  ...
  readonly runActions: InspectoRowAction<JobRunRow>[] = [
    {
      ...
      hint: "Details",
      onClick: (r) => this.openRunDetail(r),
    },
  ];
  ...
  setMode(m: JobsViewMode): void {
    ...
    this.mode = m;
    if (m === "reporting" && !this.reportLoaded && !this.reportLoading)
      this.loadReport();
  }
  ...
  async trigger(job: JobView): Promise<void> {
    if (!(await this.confirm.confirm(`Run job "${job.name}" now?`, "Run job")))
      return;
    this.api.trigger(job.name).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Run failed for ${job.name}`)),
    });
  }
  ...
  toggleEnabled(job: JobView): void {
    this.api.setEnabled(job.name, !job.enabled).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Could not update ${job.name}`)),
    });
  }
  ...
  edit(job: JobView, focusSchedule: boolean): void {
    this.api.get(job.name).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Could not load ${job.name}`)),
    });
  }
  ...
  async remove(job: JobView): Promise<void> {
    ...
    this.api.remove(job.name).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Could not delete ${job.name}`)),
    });
  }
  ...
  private toChartData(days: JobFailureDay[]): ChartData | null {
    ...
    return {
      ...
      datasets: [
        ...
        {
          label: "Failed",
          data: ordered.map((d) => d.failed),
          backgroundColor: CHART_SERIES.error,
        },
      ],
    };
  }
  ...
  toggleLive(on: boolean): void {
    ...
    if (on)
      this.liveSub = visibleInterval(LIVE_TAIL_MS).subscribe(() =>
        this.loadReport(true),
      );
  }

  openRunDetail(row: JobRunRow): void {
    this.dialog.open(JobRunDetailDialog, {
      data: row,
      width: "600px",
      maxHeight: "85vh",
    });
