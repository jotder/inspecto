...
function agentMock(
  script: (handlers: AgentStreamHandlers) => void,
  openError?: unknown,
) {
  return {
    openSession: vi.fn(() =>
      openError
        ? throwError(() => openError)
        : of({ sessionId: "s-1", openedAt: "2026-07-19T00:00:00Z" }),
    ),
    askStream: vi.fn(
      async (_id: string, _q: string, handlers: AgentStreamHandlers) =>
        script(handlers),
    ),
  };
}
...
describe("AgentChatComponent", () => {
  it("opens one session on first send, streams tokens into the agent bubble, and mounts the artifact", async () => {
    ...
    expect(agent.askStream).toHaveBeenCalledWith(
      "s-1",
  ...
  it("offers a validated navigate button for a NAVIGATION answer and drops a bogus target", async () => {
    ...
    agent.askStream.mockImplementationOnce(
      async (_id, _q, h: AgentStreamHandlers) =>
        h.onComplete({
          kind: "NAVIGATION",
          text: "Go.",
          citations: [],
          navigationTarget: "no-such-route",
          artifact: null,
        }),
    );
    c.send();
