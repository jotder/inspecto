...
describe('query ComponentKind (R3)', () => {
    ...
    it('derives its one binds edge to the source dataset', () => {
        expect(
            QUERY_KIND.deriveRefs!({ type: 'sql', datasetId: 'cdr_sample', text: 'SELECT 1', parameters: [] }),
        ).toEqual([{ kind: 'dataset', id: 'cdr_sample', rel: 'binds', via: 'dataset' }]);
    });

    it('flags a missing source dataset', () => {
        expect(
            validateQueryConfig({ type: 'sql', text: 'SELECT 1' }).some(
                (f) => f.severity === 'error' && f.path === 'datasetId',
            ),
        ).toBe(true);
    });

    it('flags a SQL query with no text, and a structured query with no model', () => {
        expect(validateQueryConfig({ type: 'sql', datasetId: 'd', text: '  ' }).some((f) => f.path === 'text')).toBe(
            true,
        );
        expect(
            validateQueryConfig({ type: 'structured', datasetId: 'd', model: null }).some((f) => f.path === 'model'),
        ).toBe(true);
    });
