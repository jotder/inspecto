 ...
 * effective log (ops not undone): seed → parameter `seedN`, exclude/hide/keep → dropped (count only), an expand
 * that named ids → generalised. The server's answer (with `exact`) is shown after the save and is authoritative.
 */
export function templatePreview(entries: InvestigationLogEntry[]): TemplatePreview {
    ...
    for (const e of entries) {
        ...
        if (e.op === 'seed') {
            out.parameters.push({
                name: `seed${out.parameters.length + 1}`,
