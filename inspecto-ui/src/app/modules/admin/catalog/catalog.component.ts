...
} from "./onboarding/onboarding-create.dialog";
...
export class CatalogComponent implements OnInit {
  ...
  readonly originRowActions: InspectoRowAction<MetadataNode>[] = [
    {
      ...
      hint: (row) =>
        row.attrs?.["active"] === false
          ? "Resume onboarding"
          : "Open onboarding",
      visible: (row) =>
        this.lens.canAuthorWorkbench() && !!row.attrs?.["pipeline"],
      onClick: (row) =>
        this.router.navigate([
          "/catalog",
          "onboard",
          String(row.attrs?.["pipeline"]),
        ]),
    },
  ];
  ...
  ngOnInit(): void {
    ...
    if (onboard === "stream" || onboard === "reference") {
      const idx = this.tabs.findIndex(
        (t) => t.id === (onboard === "reference" ? "references" : "streams"),
      );
      if (idx >= 0) this.tabIndex = idx;
  ...
  runGraph(): void {
    ...
    this.api
      .graph({
        ...
        kinds: this.graphKinds.trim()
          ? this.graphKinds.split(",").map((s) => s.trim())
          : undefined,
        edgeKinds: this.graphEdgeKinds.trim()
          ? this.graphEdgeKinds.split(",").map((s) => s.trim())
          : undefined,
  ...
  onboard(kind: "stream" | "reference"): void {
    ...
    const existingNames = [...this.streams, ...this.references]
      ...
      .filter(Boolean);
    this.dialog
      .open<OnboardingCreateDialog, unknown, OnboardingCreateResult>(
        OnboardingCreateDialog,
        {
