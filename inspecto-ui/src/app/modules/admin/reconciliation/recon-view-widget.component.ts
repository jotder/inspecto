...
@Component({
  ...
  template: `
    <div class="flex h-full min-h-0 flex-col">
      @if (error(); as message) {
        <inspecto-alert class="block" variant="warning">{{
          message
        }}</inspecto-alert>
      } @else if (result(); as res) {
        <div
          class="text-secondary mb-1 flex flex-wrap items-center gap-x-3 text-xs"
        >
          <a
            [routerLink]="[
              '/reconciliation',
              $safeNavigationMigration(recon()?.id),
            ]"
          ...
          <span
            >{{
              res.summary.byType.missing_right + res.summary.byType.missing_left
            }}
            missing</span
          >
        ...
        <div
          class="text-secondary flex h-full items-center justify-center text-sm"
        >
          Loading…
        </div>
      }
