...
export class ModelSettingsComponent implements OnInit {
  ...
  get hostedProviders(): string[] {
    return (this.settings?.knownProviders ?? []).filter(
      (p) => !this.isLocal(p),
    );
  }
  ...
  isLocal(p: string): boolean {
    return (
      this.settings?.defaults?.[p]?.local ??
      (p === "ollama" || p === "llamacpp")
    );
  }
  ...
  get hostedJarMissing(): boolean {
    return (
      !!this.settings && this.hostedProviders.some((p) => !this.isSelectable(p))
    );
  }
  ...
  load(): void {
    ...
    this.api.settings().subscribe({
      next: (s) => {
        ...
        this.baseUrl =
          s.baseUrl ?? this.settings?.defaults?.[this.provider]?.baseUrl ?? "";
      ...
      error: () => {
        ...
        this.toastr.warning(
          "Could not load assist settings — is ControlApi running?",
        );
      },
    });
  }
  ...
  save(): void {
    ...
    this.api.saveSettings(update).subscribe({
      next: (s) => {
        ...
        this.testResult = null;
        this.toastr.success(
          `Model provider set to ${this.label(this.provider)}`,
        );
      },
      error: (e) => {
        this.saving = false;
        this.toastr.error(
          apiErrorMessage(
            e,
            "Save failed — check the control token scope (assist.write)",
          ),
        );
      },
    });
  ...
  test(): void {
    ...
    this.api.testSettings().subscribe({
      next: (r) => {
        ...
        if (all.length && all.every((x) => x!.ok))
          this.toastr.success("All tiers responded");
      ...
      error: (e) => {
        ...
        this.toastr.error(
          apiErrorMessage(
            e,
            "Test failed — check the control token scope (assist.write)",
          ),
        );
      },
    });
