...
export class TagsComponent implements OnInit {
  ...
  loadRegistry(): void {
    ...
    this.objects.tags().subscribe({
      ...
      error: (err) => {
        this.loadingTags.set(false);
        this.toastr.error(
          apiErrorMessage(err, "Could not load the tag vocabulary"),
        );
      },
    });
  }
  ...
  select(name: string | null): void {
    ...
    this.api.targets(name).subscribe({
      ...
      error: (err) => {
        ...
        this.loadingTargets.set(false);
        this.toastr.error(
          apiErrorMessage(err, `Could not load what carries “${name}”`),
        );
      },
    });
  }
  ...
  commitRename(): void {
    ...
    this.api.rename(from, to).subscribe({
      ...
      error: (err) =>
        this.toastr.error(apiErrorMessage(err, `Could not rename “${from}”`)),
    });
  }
  ...
  async remove(): Promise<void> {
    ...
    this.api.remove(name).subscribe({
      next: (r) => {
        this.toastr.success(
          `Deleted “${name}” — ${r.assignments} assignment(s) removed`,
        );
      ...
      error: (err) =>
        this.toastr.error(apiErrorMessage(err, `Could not delete “${name}”`)),
    });
  }

  async untag(a: TagAssignment): Promise<void> {
    const ok = await this.confirm.confirm(
      `Remove “${a.tag}” from ${a.targetKind} “${a.targetId}”?`,
      "Remove tag",
    );
    if (!ok) return;
    this.api.unassign(a.targetKind, a.targetId, a.tag).subscribe({
      next: () => {
        this.targets.update((rows) =>
          rows.filter(
            (r) =>
              !(r.targetKind === a.targetKind && r.targetId === a.targetId),
          ),
        );
      },
      error: (err) =>
        this.toastr.error(apiErrorMessage(err, "Could not remove the tag")),
    });
  }
}
