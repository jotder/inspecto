    ...
    template: `
        ...
        <mat-dialog-content class="flex flex-col gap-2 pt-2">
            <p class="text-secondary text-sm">
                Pick the <b>surviving</b> case — the others are absorbed into it (their member incidents, tags and
                watchers move over; the absorbed cases close with a merge trace).
            </p>
            <mat-radio-group
                class="flex flex-col gap-1"
                [value]="survivorId()"
                (change)="survivorId.set($event.value)"
...
export class MergeCasesDialog {
    ...
    readonly survivorId = signal([...this.data.cases].sort((a, b) => b.updatedAt - a.updatedAt)[0]?.id ?? '');
