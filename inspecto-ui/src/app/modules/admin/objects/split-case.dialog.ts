    ...
    <mat-dialog-content class="flex flex-col gap-3 pt-2">
      <mat-form-field subscriptSizing="dynamic">
        ...
        <input
          ...
          placeholder="what the carved-out part is about"
          required
          cdkFocusInitial
        />
        @if (
      ...
      <div>
        <div class="mb-1 text-xs font-bold uppercase tracking-wider opacity-60">
          Members to move
        </div>
        @for (n of data.members; track n.id) {
          <mat-checkbox
            ...
            (change)="toggle(n.id, $event.checked)"
          >
            {{ n.title }}
            <span class="text-secondary text-xs">({{ n.id }})</span>
          </mat-checkbox>
        }
        @if (nonePicked()) {
          <p class="text-secondary text-sm" role="alert">
            Tick at least one member to move.
          </p>
        }
      </div>
    </mat-dialog-content>
...
export class SplitCaseDialog {
  ...
  split(): void {
    ...
    this.api
      .splitCase(this.data.caseObj.id, {
        title: v.title!.trim(),
        members: [...this.picked()],
      ...
      .subscribe({
        next: (res) => {
          this.toastr.success(
            `Split ${res.membersMoved} member(s) into ${res.case.id} ("${res.case.title}")`,
          );
          this.ref.close(true);
        },
        error: (e) => {
