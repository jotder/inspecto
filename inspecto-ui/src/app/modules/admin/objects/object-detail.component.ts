...
export class ObjectDetailComponent implements OnInit {
  ...
  get actions(): string[] {
    if (!this.obj) return [];
    return (
      ObjectDetailComponent.TRANSITIONS[this.obj.objectType]?.[
        (this.obj.status ?? "").toUpperCase()
      ] ?? []
    );
  }
        ...
        const contained = new Set(
          g.edges
            .filter(
              (e) =>
                e.from === this.id &&
                e.relationship?.toUpperCase() === "CONTAINS",
            )
            .map((e) => e.to),
        );
        this.members = g.nodes.filter(
          (n) =>
            n.id !== this.id && (contained.size === 0 || contained.has(n.id)),
        );
  ...
  addAttachment(): void {
    ...
    this.api.addAttachment(this.id, { name, uri }).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, "Could not add attachment")),
    });
  }
  ...
  applyRca(): void {
    ...
    this.api.applyRca(this.id, { sections }).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, "Could not apply RCA")),
    });
  }
