      ...
      <form [formGroup]="form" class="flex flex-col gap-3">
        ...
        @if (isIncident) {
          <div class="flex gap-3">
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              @if (
                form.controls.l1.hasError("required") &&
                form.controls.l1.touched
              ) {
            ...
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              <mat-select
                formControlName="l2"
                required
                (selectionChange)="form.patchValue({ l3: '' })"
              >
              ...
              @if (
                form.controls.l2.hasError("required") &&
                form.controls.l2.touched
              ) {
                <mat-error>Required.</mat-error>
              }
            </mat-form-field>
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              @if (
                form.controls.l3.hasError("required") &&
                form.controls.l3.touched
              ) {
                <mat-error>Required.</mat-error>
              }
            </mat-form-field>
        ...
        <div class="flex gap-3">
          ...
          <mat-form-field class="flex-1" subscriptSizing="dynamic">
            ...
            <mat-autocomplete
              #tagAc="matAutocomplete"
              (optionSelected)="pickTag($event)"
            >
        ...
        <div class="flex gap-3">
          ...
          <mat-form-field class="flex-[2]" subscriptSizing="dynamic">
            ...
            @if (
              form.controls.links.hasError("required") &&
            ...
            @if (!candidates().length) {
              <mat-hint
                >No existing objects to link to yet — one must exist before a
                new {{ data.label }} can be created.</mat-hint
              >
            }
          </mat-form-field>
        </div>
...
export class ObjectCreateDialog {
  ...
  readonly data = inject<{ type: string; label: string; assignees?: string[] }>(
    MAT_DIALOG_DATA,
  );
  ...
  readonly requestClose = guardDirtyClose(
    this.ref,
    () => this.form.dirty || this.tags().length > 0,
    this.confirm,
  );
  ...
  readonly assigneeOptions = (): string[] => {
    const q = String(this.form.controls.assignee.value ?? "")
      ...
      .toLowerCase();
    const all = [
      ...new Set([currentOperator(), ...(this.data.assignees ?? [])]),
    ].sort();
  ...
  l2Options(): string[] {
    ...
    return l1 && INCIDENT_TAXONOMY[l1]
      ? Object.keys(INCIDENT_TAXONOMY[l1])
      : [];
  }
  ...
  l3Options(): string[] {
    ...
    return l1 && l2 && INCIDENT_TAXONOMY[l1]?.[l2]
      ? INCIDENT_TAXONOMY[l1][l2]
      : [];
  }
  ...
  save(): void {
    ...
    if (this.isIncident)
      attributes["category"] = joinCategory(v.l1!, v.l2!, v.l3!);
    const tags = this.tags().join(",");
    if (tags) attributes["tags"] = tags;
    if (Object.keys(attributes).length) body.attributes = attributes;
    body.links = (v.links ?? []).map((to) => ({
      to,
      relationship: v.linkRelationship || "RELATED_TO",
    }));
