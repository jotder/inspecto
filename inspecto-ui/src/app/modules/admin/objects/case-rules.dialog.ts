                    ...
                    >
                        <div class="min-w-0 flex-auto">
                            ...
                            <div class="text-secondary truncate text-xs">
                                ≥{{ r.threshold }} incidents in {{ r.windowMinutes }}m where {{ describe(r) }} → "{{
                                    r.title
                                }}"
                            </div>
                        </div>
                ...
                <form [formGroup]="form" class="flex flex-col gap-3">
                    ...
                    @if (criteriaMissing()) {
                        <p class="text-secondary text-sm" role="alert">
                            Set at least one incident criterion (priority / status / text / category).
                        </p>
                    }
                </form>
            </section>
        ...
        this.api.saveCaseRule(rule).subscribe({
            next: (saved) => {
                this.rules.update((all) =>
                    [...all.filter((r) => r.name !== saved.name), saved].sort((a, b) => a.name.localeCompare(b.name)),
                );
                this.form.reset({
                    name: '',
                    title: '',
