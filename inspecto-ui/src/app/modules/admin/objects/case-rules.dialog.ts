...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="flex flex-col gap-4 pt-2">
      <section aria-label="Saved case rules">
        @for (r of rules(); track r.name) {
          <div
            class="mb-1 flex items-center gap-2 rounded border px-3 py-2"
          ...
          >
            <div class="min-w-0 flex-auto">
              ...
              <div class="text-secondary truncate text-xs">
                ≥{{ r.threshold }} incidents in {{ r.windowMinutes }}m where
                {{ describe(r) }} → "{{ r.title }}"
              </div>
            </div>
            <button
              ...
              [disabled]="busy()"
              (click)="evaluate(r)"
              matTooltip="Group matching incidents now"
            >
              Evaluate now
            </button>
            <button
          ...
          <p class="text-secondary text-sm">
            No case rules yet — save the first one below.
          </p>
        }
      </section>

      <section aria-label="New case rule">
        <h3 class="mb-2 text-xs font-bold uppercase tracking-wider opacity-60">
          New rule
        </h3>
          ...
          <div class="flex gap-3">
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              <input
                ...
                placeholder="e.g. critical-cluster"
                required
                cdkFocusInitial
              />
              @if (
            ...
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              <input
                ...
                placeholder="e.g. Critical incident cluster"
                required
              />
              @if (
          ...
          <div class="flex gap-3">
            ...
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              <input
                ...
                placeholder="e.g. Pipeline / Ingest"
              />
            </mat-form-field>
          </div>
          @if (criteriaMissing()) {
            <p class="text-secondary text-sm" role="alert">
              Set at least one incident criterion (priority / status / text /
              category).
            </p>
          }
        </form>
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-stroked-button [disabled]="busy()" (click)="save(false)">
        Save
      </button>
      <button
        mat-flat-button
    ...
    this.api.saveCaseRule(rule).subscribe({
      next: (saved) => {
        this.rules.update((all) =>
          [...all.filter((r) => r.name !== saved.name), saved].sort((a, b) =>
  ...
  evaluate(rule: CaseRule): void {
    ...
    this.api.evaluateCaseRule(rule.name).subscribe({
      next: (r) => {
        ...
        if (r.grouped > 0) {
          this.toastr.success(
            `"${rule.name}": ${r.grouped} incident(s) grouped ${r.opened ? "into a new case" : "into the open case"} (${r.caseId})`,
          );
        } else {
          this.toastr.info(
            `"${rule.name}": ${r.matched} matched, none newly grouped`,
          );
        }
      },
      error: (e) => {
