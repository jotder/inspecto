    ...
    <mat-dialog-content class="pt-2" style="min-width: 34rem">
      @if (loading()) {
        ...
        @if (categoryChart(); as chart) {
          <div class="mt-5">
            <div
              class="mb-1 text-xs font-bold uppercase tracking-wider opacity-60"
            >
              By category
            </div>
            <inspecto-chart
        ...
        <div class="mt-5">
          ...
          <dl class="grid grid-cols-[1fr_auto] gap-x-4 gap-y-0.5 text-sm">
            @for (row of statusRows(); track row.label) {
              ...
              <dd class="text-secondary text-right font-semibold">
                {{ row.count }}
              </dd>
            }
          </dl>
...
export class CaseAnalyticsDialog {
  ...
  readonly tiles = computed<Tile[]>(() => {
    ...
    return [
      ...
      {
        label: "Impact total",
        value: a.impact.impactAmount
          ? a.impact.impactAmount.toLocaleString()
          : "—",
      },
      {
        label: "Records affected",
        value: a.impact.recordsAffected
          ? a.impact.recordsAffected.toLocaleString()
          : "—",
      },
    ];
  });
  ...
  readonly statusRows = computed<{ label: string; count: number }[]>(() =>
    Object.entries(this.analytics()?.byStatus ?? {}).map(([label, count]) => ({
      label,
      count,
    })),
  );
