    ...
    <mat-dialog-content class="pt-2">
        ...
        <mat-select [formControl]="pick" required cdkFocusInitial>
          @for (i of candidates(); track i.id) {
            <mat-option [value]="i.id"
              >[{{ displayStatus(i) }}] {{ i.title }} ({{ i.id }})</mat-option
            >
          }
        </mat-select>
      ...
      @if (!candidates().length) {
        <p class="text-secondary text-sm">
          Every incident is already contained by this case (or none exist).
        </p>
      }
    </mat-dialog-content>
...
export class AddMemberDialog {
  ...
  readonly data = inject<{ caseId: string; exclude: string[] }>(
    MAT_DIALOG_DATA,
  );
  ...
  constructor() {
    ...
    this.api.list({ type: "INCIDENT", limit: 500 }).subscribe({
      next: (all) =>
        this.candidates.set(all.filter((o) => !excluded.has(o.id))),
  ...
  add(): void {
    ...
    this.api
      .link(this.data.caseId, this.pick.value!, "CONTAINS", currentOperator())
      .subscribe({
        next: () => {
