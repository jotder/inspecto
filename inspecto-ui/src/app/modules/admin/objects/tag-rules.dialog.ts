      ...
      <section aria-label="Saved tag rules">
        @for (r of rules(); track r.name) {
          ...
          <p class="text-secondary text-sm">
            No tag rules yet — save the first one below.
          </p>
        }
      </section>
        ...
        <form [formGroup]="form" class="flex flex-col gap-3">
          <div class="flex gap-3">
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              <input
                ...
                placeholder="e.g. critical-is-urgent"
                required
                cdkFocusInitial
              />
              @if (
            ...
            <mat-form-field class="flex-1" subscriptSizing="dynamic">
              ...
              />
              @if (
                form.controls.tag.hasError("required") &&
                form.controls.tag.touched
              ) {
                <mat-error>Required.</mat-error>
              }
            </mat-form-field>
          ...
          @if (criteriaMissing()) {
            <p class="text-secondary text-sm" role="alert">
              Set at least one criterion — an unconstrained rule would tag
              everything.
            </p>
          }
        </form>
...
export class TagRulesDialog {
  ...
  readonly statuses =
    this.data.type === "INCIDENT" ? INCIDENT_STATUSES : CASE_STATUSES;
  ...
  applyNow(rule: TagRule): void {
    ...
    this.api.applyTagRule(rule.name).subscribe({
      next: (res) => {
        ...
        this.changed.set(true);
        this.toastr.success(
          `"${rule.name}": ${res.matched} matched, ${res.updated} newly tagged "${rule.tag}"`,
        );
      },
      error: (e) => {
        this.busy.set(false);
