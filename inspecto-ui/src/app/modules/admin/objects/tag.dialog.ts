...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>
      Tag {{ data.targets.length }} item{{
        data.targets.length === 1 ? "" : "s"
      }}
    </h2>
...
export class TagDialog {
  ...
  readonly data = inject<{ targets: OperationalObject[]; registry: Tag[] }>(
    MAT_DIALOG_DATA,
  );
  ...
  constructor() {
    ...
    this.tags.set(
      [...names]
        ...
        .map(
          (name) =>
            this.data.registry.find((t) => t.name === name) ?? {
              name,
              createdAt: 0,
            },
        ),
    ...
    for (const name of names) {
      const on = this.data.targets.filter((o) =>
        objectTags(o).includes(name),
      ).length;
      this.initial.set(
        name,
        on === 0 ? "none" : on === this.data.targets.length ? "all" : "some",
      );
    }
  }
    ...
    this.api.createTag(name).subscribe({
      next: (t) => {
        ...
        this.tags.update((all) =>
          [...all.filter((x) => x.name !== t.name), t].sort((a, b) =>
  ...
  apply(): void {
    ...
    for (const [name, checked] of this.touched) {
      ...
      if (!checked && this.initial.get(name) !== "none")
        change.remove.push(name);
    }
