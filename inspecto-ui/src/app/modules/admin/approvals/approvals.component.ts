...
export class ApprovalsComponent implements OnInit {
  ...
  readonly columnDefs: ColDef<AgentApproval>[] = [
    ...
    { field: "agentActor", headerName: "Actor", width: 160 },
    {
      ...
      headerName: "Status",
      width: 120,
      cellRenderer: (p: ICellRendererParams<AgentApproval>) =>
  ...
  async approve(a: AgentApproval): Promise<void> {
    if (!(await this.confirm.confirm(this.review(a), `Approve ${a.tool}?`)))
      return;
    ...
    this.api.decide(a.id, decision).subscribe({
      ...
      error: (err) =>
        this.toastr.error(
          apiErrorMessage(
            err,
            `Could not ${decision} the request (it may have lapsed).`,
          ),
        ),
    });
  }
