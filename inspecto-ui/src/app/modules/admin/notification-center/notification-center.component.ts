...
export class NotificationCenterComponent implements OnInit {
  ...
  readonly deliveryCols: ColDef<ChannelDelivery>[] = [
    ...
    { field: "subject", headerName: "Subject", flex: 2, minWidth: 220 },
    {
      ...
      headerName: "Status",
      width: 110,
      cellRenderer: (p: ICellRendererParams<ChannelDelivery>) =>
  ...
  async removeRule(rule: NotificationRule): Promise<void> {
    if (
      !(await this.confirm.confirmDestructive(`Delete rule "${rule.id}"?`, {
        title: "Delete rule",
      }))
    ) {
      return;
