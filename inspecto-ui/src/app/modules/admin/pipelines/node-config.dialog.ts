...
import {
  ComponentFormDialog,
  ComponentFormResult,
} from "app/modules/admin/components/component-form.dialog";
  ...
  template: `
    ...
    >
      ...
      <button
        ...
        type="button"
        [attr.aria-label]="
          chrome.maximized() ? 'Exit full screen' : 'Full screen'
        "
    ...
    <form [formGroup]="form" (ngSubmit)="save()">
      <mat-dialog-content class="space-y-1">
        <p class="mb-2 text-xs opacity-70">
          {{ data.typeLabel }} · {{ data.categoryLabel }}
        </p>
        ...
        @if (data.bindKind && !isEnrichment) {
          <div class="mb-1">
            ...
            <p class="text-xs opacity-60">
              Choose a reusable {{ data.bindKind }} or create one inline · bound
              as
          ...
          <mat-form-field class="w-full" subscriptSizing="dynamic">
            ...
            <input
              matInput
              formControlName="use"
              placeholder="transform/my_component"
            />
          </mat-form-field>
        }
          ...
          @if (enrichSource() === "loading") {
            <div class="flex items-center gap-2 py-3">
              ...
              <span class="text-sm opacity-70"
                >Loading the bound enrichment…</span
              >
            </div>
          } @else if (enrichUneditable()) {
            <inspecto-alert
              variant="warning"
              title="Hand-authored transform file"
            >
              This enrichment uses <code>transform_file</code> — edit its TOON
              directly; saving here would overwrite the file reference with
              inline SQL.
            </inspecto-alert>
          } @else {
            <mat-form-field class="w-full" subscriptSizing="dynamic">
          ...
          <div formArrayName="config" class="space-y-2">
            ...
            @if (!configRows.length) {
              <p class="text-sm opacity-60">
                No extra config — add a key/value entry above.
              </p>
            }
          </div>
        }
...
export class NodeConfigDialog {
  ...
  readonly enrichSource = signal<"loading" | Record<string, unknown> | null>(
    null,
  );
  ...
  constructor() {
    ...
    for (const row of split.extraRows)
      this.configRows.push(this.configRow(row.key, row.value));
    ...
    this.freeFormOpen.set(
      this.isEnrichment
        ? extraKeys > 0
        : this.specs().length === 0 || extraKeys > 0,
    );
  ...
  private initEnrichment(): void {
    ...
    const bound = use.startsWith("enrichment/")
      ? use.slice("enrichment/".length)
      : null;
    ...
    if (bound) {
      ...
      this.configApi.read("enrichment", bound).subscribe({
        ...
        error: () => {
          ...
          this.toastr.warning(
            `Could not read "${bound}" — authoring a fresh enrichment.`,
          );
        },
      });
    }
    ...
    effect(() => {
      ...
      if (editor && src && src !== "loading" && !editor.isDirty())
        editor.hydrate(src);
    });
  }
  ...
  createComponent(): void {
    ...
    ref.afterClosed().subscribe((res?: ComponentFormResult) => {
      ...
      this.componentOptions.update((opts) =>
        opts.some((o) => o.name === res.saved!.name)
          ? opts
          : [...opts, res.saved!],
      );
   ...
   */
  private configForm():
    | { validate(): boolean; value(): Record<string, unknown> }
    | undefined {
  ...
  save(): void {
    ...
    const connector = this.isAcquisition
      ? (this.collector?.resolveConnector() ?? null)
      : null;
  ...
  private saveEnrichment(): void {
    ...
    const wiring = nestKeys(compact(this.schemaForm?.value() ?? {})) as Record<
      string,
    ...
    if (Object.keys(parts.references).length > 0)
      draft["references"] = parts.references;
    ...
    this.configApi.write("enrichment", draft, { overwrite: true }).subscribe({
      next: (written) => {
        ...
        this.configApi.registerEnrichment(written.path).subscribe({
          next: () => {
            this.toastr.success(
              "Enrichment saved — runs after every committed batch",
            );
...
function compact(m: Record<string, unknown>): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(m).filter(([, v]) => v !== "" && v != null),
  );
}
