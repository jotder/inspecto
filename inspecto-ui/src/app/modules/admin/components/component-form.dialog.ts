...
const jsonValidator: ValidatorFn = (
...
export class ComponentFormDialog {
  ...
  readonly title = computed(
    () => `${this.isEdit ? "Edit" : "New"} ${this.kind}`,
  );
  ...
  private buildContent(): Record<string, unknown> | null {
    ...
    switch (this.kind) {
      case "grammar": {
        ...
        if (Number(v.skipHeaderLines) > 0)
          out["skip_header_lines"] = Number(v.skipHeaderLines);
      ...
      case "transform": {
        ...
        const config: Record<string, unknown> = v.config?.trim()
          ? JSON.parse(v.config)
          : {};
  ...
  submit(): void {
    ...
    req$.subscribe({
      next: (saved) => {
        this.saving = false;
        this.toastr.success(
          `${this.kind} "${id}" ${this.isEdit ? "updated" : "created"}`,
        );
  ...
  runTest(): void {
    ...
    const obs: Observable<RelationsPreview | SinkPreview> =
      this.kind === "transform"
        ? this.api.testTransform(id, rows)
...
function partitionLabel(p: unknown): string {
  return (
    p && typeof p === "object"
      ? String((p as { column?: unknown }).column ?? "")
      : String(p ?? "")
  ).trim();
}
