  ...
  template: `
    ...
    <mat-dialog-content class="space-y-4">
      ...
      <div class="grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
        <div>
          <span class="text-secondary font-medium">time:</span>
          {{ fmt(data.ts) }}
        </div>
        <div>
          <span class="text-secondary font-medium">event id:</span>
          <span class="font-mono text-xs">{{ data.eventId }}</span>
        </div>
        <div>
          <span class="text-secondary font-medium">source:</span>
          <span class="font-mono text-xs">{{ data.source || "—" }}</span>
        </div>
        <div>
          <span class="text-secondary font-medium">pipeline:</span>
          {{ data.pipeline ?? "— (service-wide)" }}
        </div>
            ...
            <tbody>
              @for (k of attrKeys; track k) {
                <tr class="border-b border-gray-100 dark:border-gray-800">
                  <td
                    class="text-secondary py-1 pr-4 align-top font-mono text-xs"
                  >
                    {{ k }}
                  </td>
                  <td class="py-1 font-mono text-xs break-all">
                    {{ data.attributes[k] }}
                  </td>
                </tr>
              }
    ...
    <mat-dialog-actions align="end">
      @if (data.correlationId) {
        <button
          mat-stroked-button
          (click)="drillCorrelation()"
          matTooltip="Show all events with this correlation id"
        >
          <mat-icon
            class="icon-size-5"
      ...
      <button
        mat-stroked-button
        (click)="drillType()"
        matTooltip="Filter the list to this event type"
      >
        <mat-icon
          class="icon-size-5"
...
export class EventDetailDialog {
  ...
  drillCorrelation(): void {
    if (this.data.correlationId)
      this.ref.close({ correlationId: this.data.correlationId });
  }
