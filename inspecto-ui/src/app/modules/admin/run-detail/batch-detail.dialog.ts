      ...
      @if (loading) {
        ...
        @if (!batchRow) {
          <p class="text-secondary text-sm">
            No batch-summary row found for this id.
          </p>
        } @else {
        ...
        <div class="mt-4 font-semibold">
          Member files ({{ batchFiles.length }})
        </div>
        <inspecto-data-table
          tier="mini"
          sourceName="files"
...
export class BatchDetailDialog implements OnInit {
  readonly data = inject<{ pipeline: string; batchId: string }>(
    MAT_DIALOG_DATA,
  );
  ...
  get batchSummary(): { key: string; value: string }[] {
    return this.batchRow
      ? Object.entries(this.batchRow).map(([key, value]) => ({ key, value }))
      : [];
  }
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      next: ({ batches, files, lineage }) => {
        this.batchRow =
          batches.find((b) => b["consignment_id"] === batchId) || null;
