...
export class RunDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.name =
      this.nameInput() ?? this.route.snapshot.paramMap.get("name") ?? "";
    this.loadTab();
  }
  ...
  get auditRowActions(): InspectoRowAction<AuditRow>[] {
    if (this.activeTab !== "batches" && this.activeTab !== "quarantine")
      return [];
    ...
    if (!this.lens.canOperateRuns())
      return this.activeTab === "quarantine" ? [details, rejects] : [details];
  ...
  async reprocessRow(r: AuditRow): Promise<void> {
    ...
    if (
      !(await this.confirm.confirm(
        `Reprocess batch "${id}" of "${this.name}"?`,
        "Reprocess batch",
      ))
    )
      return;
    this.api.reprocess(this.name, id).subscribe({
      ...
      error: (e) =>
        this.toastr.error(apiErrorMessage(e, `Reprocess failed for ${id}`)),
    });
  }
