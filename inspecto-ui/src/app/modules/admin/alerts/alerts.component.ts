...
export class AlertsComponent implements OnInit {
  ...
  readonly columnDefs: ColDef<FiredAlert>[] = [
    ...
    },
    {
      ...
      headerName: "Severity",
      width: 120,
      cellRenderer: (p: ICellRendererParams<FiredAlert>) =>
  ...
  readonly ruleColumnDefs: ColDef<AlertRule>[] = [
    ...
    { field: "metric", headerName: "Metric", flex: 1, minWidth: 140 },
    {
      ...
      valueGetter: (p) =>
        p.data
          ? `${p.data.comparator} ${p.data.threshold} / ${p.data.window}`
          : "",
    },
    {
      field: "severity",
      headerName: "Severity",
      width: 120,
      cellRenderer: (p: ICellRendererParams<AlertRule>) =>
        statusBadgeHtml(p.value as string),
    },
    {
      headerName: "Scope",
    ...
    this.api.evaluate().subscribe({
      ...
      error: (e) => {
        ...
        this.toastr.warning(
          apiErrorMessage(
            e,
            "No alert rules armed — create one under Alert Rules below.",
          ),
        );
      },
    });
