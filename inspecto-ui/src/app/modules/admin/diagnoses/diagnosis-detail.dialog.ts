  ...
  template: `
    ...
    <mat-dialog-content>
      <table class="mb-4 text-sm">
        <tbody>
          <tr>
            <th scope="row" class="pr-4 text-left">Pipeline</th>
            <td>{{ d.pipeline }}</td>
          </tr>
          <tr>
            <th scope="row" class="pr-4 text-left">Batch</th>
            <td>{{ d.batchId }}</td>
          </tr>
          <tr>
            <th scope="row" class="pr-4 text-left">Severity</th>
            <td>{{ d.severity }}</td>
          </tr>
          <tr>
            <th scope="row" class="pr-4 text-left">Heuristic only</th>
            <td>{{ d.heuristicOnly }}</td>
          </tr>
        </tbody>
      </table>
      ...
      @if (d.citations?.length) {
        <div class="mt-3 flex flex-wrap gap-2">
          @for (c of d.citations; track c) {
            <span
              class="rounded-full bg-gray-200 px-3 py-0.5 text-xs dark:bg-gray-700"
            >
              {{ c.source }}: {{ c.ref }}
            </span>
          }
      ...
      @if (d.suggestedAlertRuleToon) {
        <div class="mt-4">
          <div class="flex items-center justify-between font-semibold">
            ...
            <button
              mat-icon-button
              aria-label="Copy suggested alert rule"
              (click)="copy(d.suggestedAlertRuleToon)"
            >
              <mat-icon
          ...
          <pre
            class="mt-1 overflow-auto rounded bg-gray-100 p-3 text-xs dark:bg-gray-800"
            >{{ d.suggestedAlertRuleToon }}</pre
          >
        </div>
      }
      ...
      <p class="text-secondary mb-2 text-sm">
        Hand this diagnosis to the diagnose-and-alert skill to produce a
