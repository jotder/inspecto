...
export class AutonomyComponent implements OnInit {
  ...
  readonly columnDefs: ColDef<AutonomyAction>[] = [
    ...
    { field: "actionClass", headerName: "Action", width: 150 },
    {
      ...
      headerName: "Outcome",
      width: 130,
      cellRenderer: (p: ICellRendererParams<AutonomyAction>) =>
  ...
  async toggleKillSwitch(): Promise<void> {
    ...
    this.api.setKillSwitch(engaging).subscribe({
      next: (p) => {
        ...
        this.rows = this.buildRows(p);
        this.toastr.success(
          engaging ? "Kill switch engaged" : "Kill switch disengaged",
        );
      },
      error: (err) =>
        this.toastr.error(
          apiErrorMessage(err, "Could not change the kill switch"),
        ),
    });
  }
  ...
  savePolicy(): void {
    ...
    this.saving = true;
    this.api
      .updatePolicy({ killSwitch: this.policy.killSwitch, classes })
      .subscribe({
        next: (p) => {
          this.policy = p;
          this.rows = this.buildRows(p);
