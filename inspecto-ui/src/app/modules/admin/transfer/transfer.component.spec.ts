...
const DATASET_DEF = {
  ...
  content: { name: "cdr_sample" },
};
const WIDGET_DEF = {
  type: "widget",
  name: "cost_by_tariff",
  ref: "widget/cost_by_tariff",
  content: { vizType: "bar", datasetId: "cdr_sample", controls: {} },
};
const PIPELINE = {
  name: "cdr_ingest",
  active: true,
  nodes: [{ id: "c", type: "acquisition", use: "connection/cdr_sftp_prod" }],
  edges: [],
};
...
function create(opts: { canAuthor?: boolean } = {}) {
  const create = vi.fn((_type: string, content: Record<string, unknown>) =>
    of({ content }),
  );
  const update = vi.fn(() => of({}));
  const componentLists: Record<string, unknown[]> = {
    dataset: [DATASET_DEF],
    widget: [WIDGET_DEF],
  };
  TestBed.configureTestingModule({
    ...
    providers: [
      ...
      provideHttpClientTesting(),
      {
        ...
        useValue: {
          list: (t: string) => of(componentLists[t] ?? []),
          create,
          update,
        },
      },
      {
        ...
        useValue: {
          list: () => of([CONNECTION]),
          create: vi.fn(() => of(CONNECTION)),
          update: vi.fn(() => of(CONNECTION)),
        },
      },
      {
      ...
      {
        provide: LensService,
        useValue: { canAuthorWorkbench: () => opts.canAuthor !== false },
      },
      {
...
const importRequest = (http: HttpTestingController) =>
  http.expectOne(
    (r) => r.method === "POST" && r.url.endsWith("/bundle/import"),
  );
...
describe("TransferComponent", () => {
  ...
  it("exports the selection expanded to its dependency closure, with the server authoring the content", async () => {
    ...
    c.toggleItem({
      kind: "widget",
      id: "cost_by_tariff",
      content: WIDGET_DEF.content,
    });
    expect(c.selectedCount()).toBe(1);
    c.exportBundle();

    const req = http.expectOne(
      (r) => r.method === "POST" && r.url.endsWith("/bundle/export"),
    );
    ...
    expect(req.request.body.items.map((i: { id: string }) => i.id)).toEqual([
      "cdr_sample",
      "cost_by_tariff",
    ]);
    expect(Object.keys(req.request.body.items[0])).toEqual([
      "kind",
      "id",
      "refs",
    ]);
  ...
  it("applies an import as ONE /bundle/import call carrying the actionable rows and the overwrite actions", () => {
    ...
    expect(
      req.request.body.bundle.items.map((i: { id: string }) => i.id),
    ).toEqual(["cdr_sample", "new_ds"]);
    expect(req.request.body.actions).toEqual({
      "dataset/cdr_sample": "overwrite",
    });
  ...
  it("skip is the default for existing items — nothing is sent unless chosen", () => {
    ...
    fixture.detectChanges();
    const bundle = buildBundle(
      [{ kind: "dataset", id: "cdr_sample", content: {} }],
      null,
    );
    c.rows.set(
      planImport(
        bundle,
        targetIndex([{ kind: "dataset", id: "cdr_sample", content: {} }]),
      ),
    );
    c.apply();
  ...
  it("read-only lens cannot apply", () => {
    ...
    c.rows.set(
      planImport(
        buildBundle([{ kind: "dataset", id: "x", content: {} }], null),
        new Map(),
      ),
    );
    c.apply();
    ...
    importRequest(http).flush(
      {
        error: {
          message:
            "bundle fails referential integrity — import would introduce: widget/w → dataset/absent",
        },
      },
