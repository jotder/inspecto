...
} from "app/inspecto/connections/connection-form.dialog";
...
export class ConnectionsComponent implements OnInit {
  ...
  load(): void {
    ...
    this.api.list().subscribe({
      ...
      error: () => {
        ...
        this.loading = false;
        this.toastr.warning(
          "Could not load connections — is ControlApi running?",
        );
      },
    });
  }
  ...
  test(id: string): void {
    ...
    this.api.test(id).subscribe({
      next: (r) => {
        ...
        if (r.reachable) {
          this.toastr.success(
            `${id}: reachable${r.latencyMs != null ? ` (${r.latencyMs} ms)` : ""}`,
          );
        } else {
  ...
  optionEntries(c: ConnectionProfile): { key: string; value: string }[] {
    return Object.entries(c.options ?? {}).map(([key, value]) => ({
      key,
      value,
    }));
  }
