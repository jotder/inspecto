...
export function toAttrSpecs(fields: FieldSpec[]): AttributeSpec[] {
    return fields.map((f, i) => {
        ...
        if (f.defaultValue !== undefined && f.defaultValue !== null) {
            spec.default =
                f.type === 'LIST' && Array.isArray(f.defaultValue) ? f.defaultValue.join(', ') : f.defaultValue;
        }
        return spec;
    });
}
...
export class ConfigComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.loadSpec();
        this.typeCtrl.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.loadSpec());
    }
