...
function attrType(f: FieldSpec): AttributeType {
  switch (f.type) {
    ...
    case "BOOL":
      return "boolean";
    default:
      return "string"; // STRING, FILEPATH, CRON, SQL, MAP, LIST (comma text)
  }
}
...
export class ConfigComponent implements OnInit {
  ...
  @ViewChild(InspectoSchemaFormComponent) set schemaForm(
    c: InspectoSchemaFormComponent | undefined,
  ) {
    this.form = c;
    this.previewSub?.unsubscribe();
    if (!c) return;
    const refresh = (): void => {
      this.assembledPreview.set(this.previewJson(c.value()));
    };
    refresh();
    ...
    this.api.spec(this.typeCtrl.value).subscribe({
      ...
      error: (e) => {
        ...
        this.toastr.warning(
          apiErrorMessage(
            e,
            `Could not load the "${this.typeCtrl.value}" spec.`,
          ),
        );
      },
    });
  ...
  private previewJson(values: Record<string, unknown>): string {
    try {
      return JSON.stringify(
        assembleConfig(this.spec?.fields ?? [], values),
        null,
        2,
      );
    } catch {
  ...
  validateDraft(): void {
    ...
    this.result = null;
    this.api
      .validateDraft(this.typeCtrl.value, config, this.safetyCtrl.value)
      .subscribe({
        next: (r) => {
          this.result = r;
