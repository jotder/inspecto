...
function setup(id: string | null): void {
  TestBed.configureTestingModule({
    providers: [
      provideHttpClient(withXhr(), withInterceptors([spaceInterceptor])),
      provideHttpClientTesting(),
...
describe("spaceInterceptor", () => {
  describe("with an active space", () => {
    ...
    it("leaves /health server-global", () =>
      expectUrl(`${base}/health`, `${base}/health`));

    it("leaves /metrics server-global", () =>
      expectUrl(`${base}/metrics`, `${base}/metrics`));

    it("does not prefix the /spaces list", () =>
      expectUrl(`${base}/spaces`, `${base}/spaces`));
