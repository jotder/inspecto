...
export class JobsService {
    ...
    get(name: string): Observable<JobDetail> {
        return this.http
            .get<Record<string, unknown>>(apiUrl(`/jobs/${encodeURIComponent(name)}`))
    ...
    setEnabled(name: string, enabled: boolean): Observable<JobDetail> {
        return this.http
            ...
            >(apiUrl(`/jobs/${encodeURIComponent(name)}/${enabled ? 'enable' : 'disable'}`), {})
            .pipe(map(jobFromWire));
    }
    ...
    runLogs(name: string, runId: string): Observable<JobRunLogs> {
        return this.http.get<JobRunLogs>(
            apiUrl(`/jobs/${encodeURIComponent(name)}/runs/${encodeURIComponent(runId)}/logs`),
        );
    }
    ...
    runArtifact(name: string, runId: string): Observable<ReportArtifact> {
        return this.http.get<ReportArtifact>(
            apiUrl(`/jobs/${encodeURIComponent(name)}/runs/${encodeURIComponent(runId)}/artifact`),
        );
    }
