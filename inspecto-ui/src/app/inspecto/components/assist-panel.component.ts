...
export class AssistPanelComponent implements OnInit {
  ...
  run(): void {
    ...
    if (this.isSql) {
      if (text && partialInput["kpiDescription"] === undefined)
        partialInput["kpiDescription"] = text;
  ...
  get draftToon(): string | undefined {
    return this.d<string>("draftToon");
  }
  get humanReadable(): string | undefined {
    return this.d<string>("humanReadable");
  }
  get narrative(): string | undefined {
    return this.d<string>("narrative");
  }
  get nextRuns(): string[] {
    return this.d<string[]>("nextRuns") || [];
  }
  get sampleRows(): Record<string, unknown>[] {
    return this.d<Record<string, unknown>[]>("sampleRows") || [];
  }
  get findings(): {
    severity?: string;
    fieldPath?: string;
    ...
    return (
      this.d<{ severity?: string; fieldPath?: string; message?: string }[]>(
        "findings",
      ) || []
    );
  }
  ...
  get artifact(): A2uiArtifact | null {
    ...
    if (isRecord(served) && typeof served["kind"] === "string")
      return served as unknown as A2uiArtifact;
    const parts: A2uiArtifact[] = [];
    if (this.humanReadable)
      parts.push({ kind: "text", config: { text: this.humanReadable } });
    if (this.narrative)
      parts.push({ kind: "text", config: { text: this.narrative } });
    ...
    if (this.findings.length) {
      parts.push({
        ...
        config: {
          rows: this.findings,
          columns: ["severity", "fieldPath", "message"],
        },
      });
    }
