...
class SizableDialog {
  @ViewChild(InspectoDialogResizeDirective)
  chrome!: InspectoDialogResizeDirective;
}

function openDialog(): {
  ref: MatDialogRef<SizableDialog>;
  pane: HTMLElement;
  ...
  const ref = TestBed.inject(MatDialog).open(SizableDialog, {
    ariaLabel: "Sizable dialog",
  });
  TestBed.tick();
  const pane = document.querySelector(".cdk-overlay-pane") as HTMLElement;
  const grip = pane.querySelector(
    ".inspecto-dialog-resize-grip",
