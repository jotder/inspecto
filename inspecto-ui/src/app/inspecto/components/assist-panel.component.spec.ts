...
function run(
  fixture: ReturnType<typeof TestBed.createComponent<AssistPanelComponent>>,
  userText = "explain",
): HTMLElement {
...
describe("AssistPanelComponent", () => {
  it("runs the intent and renders the answer + SQL section", () => {
    ...
    expect(api.run).toHaveBeenCalledWith(
      "explain-entity",
      expect.objectContaining({ userText: "explain this pipeline" }),
    );
  ...
  it("uses a server-shaped result.artifact verbatim instead of composing one", () => {
    ...
    expect(host!.textContent).not.toContain(
      "ignored — the served artifact wins",
    );
  });
  ...
  it("has no a11y violations with a rendered result", async () => {
    const { fixture } = create({
      ...
      data: {
        ...RESULT.data,
        humanReadable: "Prose.",
        findings: [{ severity: "warn", fieldPath: "x", message: "m" }],
      },
    });
    run(fixture);
