...
export class MapViewComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes["tileServerUrl"] &&
      !changes["tileServerUrl"].firstChange &&
      this.map
    )
      this.applyScheme();
  }
  ...
  kindColors(): Map<string, string> {
    const kinds = [
      ...new Set((this.data?.points ?? []).map((p) => p.kind)),
    ].sort();
    return new Map(
      kinds.map((k, i) => [
        k,
        ICON_COLOR_SWATCHES[i % ICON_COLOR_SWATCHES.length],
      ]),
    );
  }
  ...
  private mount(): void {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      map.on("error", (e) =>
        console.error("[inspecto-map-view]", e.error ?? e),
      );
      map.addControl(
      ...
      map.on("load", () => {
        ...
        if (this.camera)
          map.jumpTo({ center: this.camera.center, zoom: this.camera.zoom });
        else this.fitToData();
      });
  ...
  private routesGeoJson(): FeatureCollection {
    ...
    const kinds = [
      ...new Set((this.data?.routes ?? []).map((r) => r.kind)),
    ].sort();
    const kindColor = new Map(
      kinds.map((k, i) => [
        k,
        ICON_COLOR_SWATCHES[(colors.size + i) % ICON_COLOR_SWATCHES.length],
      ]),
    );
      ...
      features: (this.data?.routes ?? []).flatMap((r) => {
        ...
        if (dup > 0) {
          ...
          coords = coords.map(([lon, lat], i) => {
            ...
            return [lon + (nx / len) * f, lat + (ny / len) * f] as [
              number,
              number,
            ];
          });
  ...
  private addDataLayers(map: MlMap): void {
    ...
    map.addSource(POINTS_RAW_SOURCE, {
      type: "geojson",
      data: this.pointsGeoJson(),
    });
  ...
  private applyData(): void {
    ...
    (this.map?.getSource(ROUTES_SOURCE) as GeoJSONSource | undefined)?.setData(
      this.routesGeoJson(),
    );
    (this.map?.getSource(OVERLAY_SOURCE) as GeoJSONSource | undefined)?.setData(
      this.overlay ?? EMPTY_FC,
    );
    this.applyDisplayMode();
    this.applyLayerToggles();
  }
  ...
  private applyScheme(): void {
    ...
    this.map.once("styledata", () => {
      if (this.map && !this.map.getSource(POINTS_SOURCE))
        this.addDataLayers(this.map);
    });
  }
}
