...
export const CASE_STUDY_GRAMMARS: Record<string, Record<string, unknown>> = {
    cdr_asn1_ber: {
        parser_type: 'asn1',
        encoding_rules: 'BER',
        schema_spec: 'cdr_3gpp_ts32297',
        extension: 'asn1',
        decode_implicit: true,
    },
    ...
    nodes: [
        {
            id: 'c_sftp',
            type: 'acquisition',
            name: 'Switch ASN.1 drops',
            use: 'connection/cdr_sftp_prod',
            config: { include: 'glob:**/*.asn1', stability: { window: '60s' } },
        },
        { id: 'c_stream', type: 'adapter', name: 'CDR event bus', config: { guarantee: 'AT_LEAST_ONCE' } },
        {
            ...
            type: 'acquisition',
            name: 'Roaming TAP-in',
            use: 'connection/pg_warehouse',
            config: { incremental: { watermark: 'loaded_at' } },
        },
        ...
        { id: 'p_json', type: 'parser', name: 'Parse event NDJSON', use: 'grammar/sim_events_ndjson' },
        {
            id: 'n_switch',
            type: 'transform.map',
            name: 'Normalize switch leg',
            config: { rename: 'calling_number=msisdn', cast: 'duration_s:int, cost_usd:decimal(18,6)' },
        },
        {
            id: 'n_stream',
            type: 'transform.map',
            name: 'Normalize stream leg',
            config: { rename: 'imsi=msisdn', drop: 'raw_payload' },
        },
        {
            id: 'n_roam',
            type: 'transform.map',
            name: 'Normalize roaming leg',
            config: { derive: 'cost_usd = sdr_amount * fx_rate' },
        },
        {
            id: 'dedup',
            type: 'transform.filter',
            name: 'Drop duplicates',
            config: { predicate: 'NOT is_duplicate(record_id)' },
        },
        { id: 'route', type: 'transform.route', name: 'Split by service', config: { mode: 'case' } },
        {
            ...
            type: 'transform.map',
            name: 'Rate voice',
            config: { derive: 'rated_usd = duration_s * tariff_rate / 60' },
        },
        { id: 'rate_sms', type: 'transform.map', name: 'Rate SMS', config: { derive: 'rated_usd = tariff_rate' } },
        {
            ...
            type: 'transform.map',
            name: 'Rate data',
            config: { derive: 'rated_usd = bytes_used / 1048576 * tariff_rate' },
        },
        {
            id: 'rate_roam',
            type: 'transform.map',
            name: 'Rate roaming',
            config: { derive: 'rated_usd = cost_usd * roaming_uplift' },
        },
        {
            id: 'agg',
            type: 'transform.derive',
            name: 'Revenue rollup',
            config: { group_by: 'tariff, event_date', aggregations: 'SUM(rated_usd) AS revenue, COUNT(*) AS events' },
        },
        {
            id: 'a_rev',
            type: 'alert',
            name: 'Revenue drop alert',
            config: { condition: 'revenue_delta_pct < -5', severity: 'CRITICAL' },
        },
        {
            id: 's_lake',
            type: 'sink.persistent',
            name: 'Rated CDR lake',
            use: 'sink/cdr_parquet',
        ...
        },
        {
            ...
            config: { table: 'rated_cdr_daily', mode: 'upsert', key_columns: 'tariff, event_date' },
        },
...
const FRAUD_VELOCITY: AuthoredPipeline = {
    ...
    nodes: [
        ...
        { id: 'p_ndjson', type: 'parser', name: 'Parse NDJSON', use: 'grammar/sim_events_ndjson' },
        {
            id: 'derive',
            type: 'transform.map',
            name: 'Derive velocity',
            config: { derive: 'velocity_kmh = haversine(prev_loc, loc) / hours_between(prev_time, event_time)' },
        },
        { id: 'clone', type: 'transform.route', name: 'Tee real-time / archive', config: { mode: 'clone' } },
        {
            ...
            type: 'transform.filter',
            name: 'High-risk only',
            config: { predicate: 'velocity_kmh > 900 OR swap_count_24h >= 3' },
        },
        {
            id: 'a_fraud',
            type: 'alert',
            name: 'Impossible-travel alert',
            config: { condition: 'true', severity: 'CRITICAL' },
        },
        {
            id: 's_cases',
            type: 'sink.materialized',
            name: 'Fraud candidates',
            config: { table: 'fraud_candidates', mode: 'upsert', key_columns: 'msisdn' },
        },
        {
            id: 'agg5',
            type: 'transform.derive',
            name: '5-minute windows',
            config: {
                group_by: "msisdn, window('5m')",
                aggregations: 'COUNT(*) AS events, MAX(velocity_kmh) AS top_speed',
            },
        },
        {
            id: 's_archive',
    ...
    nodes: [
        {
            id: 'a_collect',
            type: 'acquisition',
            name: 'Switch dumps (S3)',
            use: 'connection/s3_archive',
            config: { include: 'glob:switch/**/*.asn1' },
        },
        { id: 'a_parse', type: 'parser', name: 'Decode switch CDR', use: 'grammar/cdr_asn1_ber' },
        {
            id: 'a_norm',
            type: 'transform.map',
            name: 'Normalize switch side',
            config: { rename: 'a_number=msisdn', cast: 'cost_usd:decimal(18,6)' },
        },
        {
            id: 'a_sink',
            type: 'sink.materialized',
            name: 'Load switch_cdr',
            config: { table: 'switch_cdr', mode: 'replace' },
        },
        {
            id: 'b_collect',
            type: 'acquisition',
            ...
            use: 'connection/pg_warehouse',
            config: { incremental: { watermark: 'event_id' } },
        },
        {
            id: 'b_norm',
            type: 'transform.map',
            name: 'Normalize billing side',
            config: { cast: 'cost_usd:decimal(18,6)' },
        },
        {
            id: 'b_sink',
            type: 'sink.materialized',
            name: 'Load billing_cdr',
            config: { table: 'billing_cdr', mode: 'replace' },
        },
        {
            id: 'a_gap',
            type: 'alert',
            name: 'Watermark gap alert',
            config: { condition: 'watermark_gap_minutes > 60', severity: 'WARNING' },
        },
        {
            id: 'd_collect',
            type: 'acquisition',
            name: 'DRAFT — legacy leg',
            description: 'Unwired draft: legacy mainframe side, pending sign-off.',
            use: 'connection/legacy_ftp_down',
...
const FORMAT_GAUNTLET: AuthoredPipeline = {
    ...
    nodes: [
        ...
        // it is the deletable fixture in connections.handler.spec.ts).
        {
            ...
            type: 'acquisition',
            name: 'Mixed dropzone',
            config: { include: 'glob:drops/**/*', recursive_depth: 5 },
        },
        ...
        { id: 'p_fixed', type: 'parser', name: 'Mainframe fixed-width', use: 'grammar/mainframe_fixed' },
        {
            id: 'u_norm',
            type: 'transform.map',
            name: 'Union normalize',
            config: { cast: 'amount:decimal(18,2)', derive: 'source_format = file_ext' },
        },
        {
            id: 'u_filter',
            type: 'transform.filter',
            name: 'Drop empty amounts',
            config: { predicate: 'amount IS NOT NULL' },
        },
    ...
    nodes: [
        {
            id: 't_collect',
            type: 'acquisition',
            name: 'Legacy FTP gold feed',
            use: 'connection/legacy_ftp_down',
            config: { include: 'glob:*.csv', stability: { window: '300s' } },
        },
        { id: 't_parse', type: 'parser', name: 'Parse pipe-delimited', use: 'grammar/pipe_delimited' },
        {
            id: 't_rename',
            type: 'transform.map',
            name: 'Rename legacy fields',
            config: { rename: 'MSISDN_NR=msisdn, CALL_SECS=duration_s' },
        },
        {
            id: 't_screen',
            type: 'transform.filter',
            name: 'Screen test traffic',
        ...
        },
        {
            ...
            type: 'transform.map',
            name: 'Cast types',
            config: { cast: 'duration_s:int, cost_usd:decimal(18,6)' },
        },
        {
            id: 't_daily',
            type: 'transform.derive',
            name: 'Daily totals',
            config: {
                group_by: 'msisdn, event_date',
                aggregations: 'SUM(duration_s) AS total_secs, SUM(cost_usd) AS total_usd',
            },
        },
        {
            id: 't_derive',
            type: 'transform.map',
            name: 'Derive usage band',
            config: { derive: "usage_band = CASE WHEN total_secs > 3600 THEN 'heavy' ELSE 'normal' END" },
        },
        {
            id: 't_final',
            type: 'transform.filter',
            name: 'Positive totals only',
            config: { predicate: 'total_usd >= 0' },
        },
        {
            id: 's_gold',
            type: 'sink.materialized',
            name: 'Gold table',
            config: { table: 'gold_cdr_daily', mode: 'append' },
        },
        ...
        { id: 's_samples', type: 'sink.persistent', name: 'Dropped-row samples', config: { format: 'CSV' } },
        {
            id: 's_dlq',
            type: 'sink.materialized',
            name: 'Dead-letter table',
            config: { table: 'dead_letter', mode: 'append' },
        },
    ],
    edges: [
