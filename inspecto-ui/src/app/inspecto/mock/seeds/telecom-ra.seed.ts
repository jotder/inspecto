...
export function seedTelecomRa(store: MockStore, space: string): void {
    ...
    const connections: ConnectionProfile[] = [
        {
            id: 'switch_sftp',
            connector: 'sftp',
            host: 'msc1.telco.example',
            port: 22,
            basePath: '/cdr/out',
            username: 'ra',
            password: '${ENV:SWITCH_SFTP_PW}',
            description: 'MSC switch CDR drop zone',
        },
        {
            id: 'billing_db',
            connector: 'db',
            host: 'billing.telco.example',
            port: 5432,
            database: 'rated',
            username: 'ra_ro',
            password: '${ENV:BILLING_DB_PW}',
            options: { sslmode: 'require' },
            description: 'Billing rated-events replica (read-only)',
        },
        {
            id: 'ra_archive',
            connector: 's3',
            ...
            description: 'Reconciled-batch archive',
        },
    ];
    ...
    putComponent(store, space, 'transform', 'drop_zero_duration', {
        ...
        nodes: [
            {
                id: 'collect',
                type: 'acquisition',
                name: 'Collect switch CDRs',
                use: 'connection/switch_sftp',
            ...
            { id: 'parse', type: 'parser', name: 'Parse CDR CSV', config: { delimiter: ',', header: true } },
            {
                id: 'filter',
                type: 'transform.filter',
                name: 'Drop zero-duration',
                config: { predicate: 'duration_s > 0' },
            },
    ...
    putComponent(store, space, 'widget', 'total_revenue', {
        name: 'total_revenue',
        datasetId: 'cdr_traffic',
        vizType: 'kpi',
        controls: { value: [{ field: 'cost_usd', agg: 'sum' }] },
        description: 'Rated revenue across the loaded CDR sample',
        tags: ['ra'],
    });
    putComponent(store, space, 'widget', 'revenue_by_tariff', {
        name: 'revenue_by_tariff',
        datasetId: 'cdr_traffic',
        vizType: 'bar',
        controls: { x: [{ field: 'tariff' }], y: [{ field: 'cost_usd', agg: 'sum' }] },
        description: 'Where the money is: rated revenue per tariff plan',
        tags: ['ra', 'billing'],
    });
    putComponent(store, space, 'widget', 'usage_over_time', {
        ...
        description: 'Daily data usage trend',
        tags: ['ra', 'usage'],
    });
    ...
    putComponent(store, space, 'requirement', 'daily_switch_vs_billing_recon', {
        ...
        kind: 'reconciliation',
        description:
            'Compare switch CDRs against billing rated events daily; every unrated or mis-rated call is potential revenue leakage.',
        status: 'delivered',
    ...
    store.put<JobDetail>(space, JOBS_COLL, 'switch_ingest_daily', {
        name: 'switch_ingest_daily',
        type: 'ingest',
        cron: '0 0 5 * * *',
        onPipeline: null,
        enabled: true,
        lastStatus: 'SUCCESS',
        lastRunTime: iso(-60 * 7),
        nextFire: iso(60 * 17),
        catchUp: true,
    ...
    recordRun(
        ...
        now - 6 * 3_600_000,
        12_800,
        '3 open breaks (1 value, 1 unbilled, 1 unmatched billing row).',
    );
    ...
    raEvents.forEach(([type, level, pipeline, message], i) => {
        ...
        store.put(
            ...
            eventToSignal({
                ...
                source: 'engine',
                pipeline,
                correlationId: i < 3 ? 'corr-ra-1' : null,
                message,
                attributes: {},
            }),
        );
    ...
    store.put<AlertRule>(space, ALERT_RULES_COLL, 'billing_delta_pct', {
        name: 'billing_delta_pct',
        metric: 'billing_delta_pct',
        comparator: 'gt',
        threshold: 0.5,
        window: '1d',
        severity: 'CRITICAL',
    ...
    store.put(
        ...
        alertToSignal(
            {
                rule: 'billing_delta_pct',
                severity: 'CRITICAL',
                pipeline: 'billing_rated_load',
                metric: 'billing_delta_pct',
                value: 0.62,
                comparator: 'gt',
                threshold: 0.5,
                window: '1d',
                epochMillis: now - 5_400_000,
    ...
    incidents.forEach(([objectType, title, status, severity, priority], i) => {
        ...
        store.put<OperationalObject>(space, OPS_OBJECTS_COLL, `${objectType.toLowerCase()}-ra-${i}`, {
            id: `${objectType.toLowerCase()}-ra-${i}`,
            objectType,
            title,
            description: 'Seeded by the Telecom RA space template.',
            status,
            severity,
            priority,
            owner: 'ra-ops',
            ...
            attributes: { pipeline: 'billing_rated_load' },
            createdAt: ts,
