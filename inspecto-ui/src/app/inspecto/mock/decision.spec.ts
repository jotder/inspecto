...
describe('executeConsequences', () => {
    it('emit-signal writes a signal to the ledger, sourced from the rule', () => {
        ...
        const out = executeConsequences(
            store,
            'default',
            rule([{ action: 'emit-signal', params: { type: 'REVIEW', severity: 'warn', message: 'hi' } }]),
        );
    ...
    it('create-alert emits an ALERT_FIRED record signal (carries a rule payload)', () => {
        ...
        executeConsequences(
            store,
            'default',
            rule([{ action: 'create-alert', params: { rule: 'ai_x', metric: 'cost' } }]),
        );
    ...
    it('start-job with a target emits JOB_STARTED; without a target it is skipped', () => {
        ...
        const out = executeConsequences(
            store,
            'default',
            rule([{ action: 'start-job', target: { kind: 'job', id: 'j1' } }, { action: 'start-job' }]),
        );
    ...
    it('create-incident opens a managed Incident (any severity), deduped to one open per rule', () => {
        ...
        const incidents = () =>
            store.list<OperationalObject>('default', OPS_OBJECTS_COLL).filter((o) => o.objectType === 'INCIDENT');
    ...
    it('routing actions are record-level — skipped here, no signal written', () => {
        ...
        const out = executeConsequences(
            store,
            'default',
            rule([{ action: 'route', destination: 'emea' }, { action: 'drop' }]),
        );
