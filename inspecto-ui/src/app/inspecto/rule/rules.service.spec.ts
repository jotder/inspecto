...
function setup() {
    const create = vi.fn((_t: string, c: Record<string, unknown>) => of({ type: 'rule-template', name: String(c['id']), ref: `rule-template/${c['id']}`, content: c }));
    const list = vi.fn(() =>
        of([{ type: 'rule-template', name: 'r1', ref: 'rule-template/r1', content: { name: 'r1', source: 'alerts', projection: '*', where: emptyGroup(), sqlOverride: null } }]),
    );
