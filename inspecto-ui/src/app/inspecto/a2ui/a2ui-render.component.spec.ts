...
describe("A2uiRenderComponent", () => {
  ...
  it("renders a kpi artifact through <inspecto-kpi> with mapped value/label", () => {
    const fixture = create({
      kind: "kpi",
      config: { value: 99, label: "Rows loaded" },
    });
    fixture.detectChanges();
    const el = fixture.nativeElement as HTMLElement;
  ...
  it("maps a chart artifact to Chart.js inputs (computed only — Chart.js cannot paint in jsdom)", () => {
    ...
    const c = create({
      kind: "chart",
      config: { type: "line", data, options: { plugins: {} } },
    }).componentInstance;
  ...
  it("degrades wrong-typed config to defaults instead of throwing", () => {
    const fixture = create({
      kind: "kpi",
      config: { value: "not-a-number", label: 42 } as never,
    });
    fixture.detectChanges();
    const c = fixture.componentInstance;
  ...
  it("renders only navigate-intent actions in the navigate group; a bogus target is disabled and never navigates", () => {
    ...
    const c = fixture.componentInstance;
    expect(c.navigateActions().map((a) => a.label)).toEqual([
      "Open catalog",
      "Bogus",
      "External",
    ]);
    ...
    const navButtons = buttons.filter((b) =>
      ["Open catalog", "Bogus", "External"].includes(
        b.textContent?.trim() ?? "",
      ),
    );
  ...
  describe("invoke actions (S6 — gated agentic write)", () => {
    it("starts idle, dry-runs via simulate (no mutation) on click, then requires a SEPARATE confirm to apply", () => {
      const fixture = create(
        {
          ...
          actions: [
            {
              label: "Quarantine high-cost rows",
              intent: "invoke",
              target: "high_cost",
            },
          ],
      ...
      let btn = (fixture.nativeElement as HTMLElement).querySelector(
        "button",
      ...
      const simulateReq = http.expectOne(
        (r) =>
          r.url.includes("/decision-rules/high_cost/simulate") &&
          r.method === "POST",
      );
      simulateReq.flush({
        lastSimulation: { matched: 3, total: 10, checkedAt: 1 },
      });
      fixture.detectChanges();
      ...
      const applyReq = http.expectOne(
        (r) =>
          r.url.includes("/decision-rules/high_cost/apply") &&
          r.method === "POST",
      );
      expect(applyReq.request.headers.get("X-Agent-Session")).toBe("sess-1");
      applyReq.flush({
        rule: "high_cost",
        executed: [{ action: "quarantine", status: "executed", detail: "ok" }],
      });
      fixture.detectChanges();
    ...
    it("declining after a dry-run (Cancel) never calls apply", () => {
      const fixture = create({
        ...
        config: { text: "go" },
        actions: [
          { label: "Quarantine", intent: "invoke", target: "high_cost" },
        ],
      });
      fixture.detectChanges();
      const http = TestBed.inject(HttpTestingController);
      ...
      fixture.detectChanges();
      http
        .expectOne((r) => r.url.includes("/simulate"))
        .flush({ lastSimulation: { matched: 1, total: 1, checkedAt: 1 } });
      fixture.detectChanges();
      expect(c.invokeState("high_cost").phase).toBe("ready");
  ...
  it("renders nested parts recursively, capped at depth 3", () => {
    ...
    for (let i = 0; i < 5; i++)
      leaf = { kind: "text", config: { text: `level ${i}` }, parts: [leaf] };
    const fixture = create(leaf);
    fixture.detectChanges();
    const nested = (fixture.nativeElement as HTMLElement).querySelectorAll(
      "inspecto-a2ui-render",
    );
