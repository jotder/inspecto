...
describe("LensSwitcherComponent", () => {
  ...
  it("shows the current lens label (default Builder)", () => {
    ...
    expect(
      fixture.componentInstance.labelFor(
        fixture.componentInstance.lens.currentLens(),
      ),
    ).toBe("Builder");
  });
  ...
  it("offers only the lenses the subject grants project onto (R2)", () => {
    ...
    fixture.detectChanges();
    expect(
      fixture.componentInstance.lens.allowedLenses().map((l) => l.id),
    ).toEqual(["business", "ops"]);
