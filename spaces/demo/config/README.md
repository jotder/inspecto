...
|---|---|---|---|
| `jobs/sample_hello_job.toon` | `sample.hello` | hourly | **Does nothing on purpose.** Echoes its resolved parameters and emits one notification — through its declared `requires: [notifications]` Platform Service grant, so it is also the reference for *using* a service. Its declaration is a tour of the whole authoring contract — grouped fields, a select (`severity`), list chips (`tags`, `notify`), bounds (`retries` 0–5), an ADVANCED tier, a masked `api_token`, and `run_date` deduced from `$today`. Safe to schedule, trigger and delete. |
| `jobs/stream_failure_watch_job.toon` + `orders/orders_stream_failures_alert.toon` | `alert.evaluate` | hourly (`5 * * * *`) | **File-processing failures for one stream, within the hour → Incident.** The *rule* does the detecting (`rejected_files > 3`, `window: 1h`, `onPipeline: orders`); the Job only supplies the clock. `severity: CRITICAL` is what promotes the Alert to a managed **Incident** — a lower severity stays an Alert. Both are deduped, so an unresolved breach is not re-raised every hour. |
...
| `jobs/orders_by_region_materialize_job.toon` | `maintenance` (`task: materialize`) | daily (05:00) | **A Dataset rolled up into a Dataset.** Writes `orders_dataset` grouped to one row per region (`region` x `sum(gross)` — the same pair the `orders_revenue_by_region` widget charts) as a Parquet snapshot, and registers the result as the Dataset `orders_by_region`, so a chart can point at the summary instead of re-aggregating the detail on every open. `target` must differ from `dataset` and is both the output directory and the new Dataset's id. Scheduled rather than chained to the pipeline on purpose: it rewrites the whole snapshot, so firing it per arriving batch would be wasteful. The same task runs ad hoc from the Dataset page — `POST /datasets/orders_dataset/materialize` with `{"target":"orders_by_region"}` — which registers no job. |
