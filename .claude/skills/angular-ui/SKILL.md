...
2b. **Typecheck all THREE tsconfigs — `npm run build` covers only the app one.**
   ...
   the root config to lean on even by accident. A spec that runs green is not proof it typechecks.
   **Check the EXIT CODE, not just the pass count**: an unhandled error (e.g. a G6/AntV or MapLibre
   canvas mounting in jsdom) makes vitest exit non-zero even with 0 test failures → CI red.
   `GraphViewComponent` only mounts a canvas when
