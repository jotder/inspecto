...
/**
 * In-process publish/subscribe bus for {@link ConsignmentEvent}s — the trigger backbone
 ...
 */
public final class ConsignmentEventBus {

    private static final Logger log = LoggerFactory.getLogger(ConsignmentEventBus.class);

    private final List<Consumer<ConsignmentEvent>> listeners = new CopyOnWriteArrayList<>();

    /** Register a listener. Thread-safe; may be called while the bus is in use. */
    public void subscribe(Consumer<ConsignmentEvent> listener) {
        listeners.add(listener);
    }

    /** A sink to hand to the ingest layer (e.g. {@code CollectorProcessor.run(cfg, bus.sink())}). */
    public Consumer<ConsignmentEvent> sink() {
        return this::publish;
    }

    /** Deliver {@code event} to every subscriber; isolate per-listener failures. */
    public void publish(ConsignmentEvent event) {
        for (Consumer<ConsignmentEvent> l : listeners) {
            try {
                l.accept(event);
            } catch (Exception e) {
                log.error("Consignment-event listener failed for batch {} (pipeline {})",
