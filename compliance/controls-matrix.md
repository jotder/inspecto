...
|---|---|---|---|
| G1 | SBOM is not generated | C3 | Plan §6 Q2 (CycloneDX vs SPDX) — recommendation stands at CycloneDX. ⚠ **Scope correction 2026-08-28: generate it PER PACKAGED BUNDLE, not from the reactor** (see CC9) — the reactor set includes optional modules a given edition does not ship. `tools/dependencies.lock` is the review baseline, **not** an SBOM: it lists coordinates, no licences or hashes. |
...
| G8 | Access-review (effective-grants) view | C4 | ⚠ **UNBLOCKED 2026-08-28 — this row's blocker was stale.** `rbac-abac-plan` R5 ✅ SHIPPED 2026-07-23 (Settings ▸ Access, tabbed admin-visibility view — plan now in `plans-archive/`). Remaining work: verify that view answers an auditor's effective-grants question and record it as CC6 evidence; build only what's missing. |
