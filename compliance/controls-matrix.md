...
| A1 | Availability: capacity, recovery | Single-node by design (NFR-8); crash-isolated idempotent Runs (`OVERWRITE_OR_IGNORE` output idempotence) | Engine suites | product + customer | **Backup/restore runbook + documented RTO/RPO + a restore drill** → C4 |
...
| G2 | Customer **verification runbook** for `.sha256`/`.asc` | C3 | Nothing — the artifacts already ship |
| G3 | Signing is *possible* but not *routine* | C3 | Plan §6 Q3 — where the org's GPG key lives |
| G4 | Auditor **audit-log extraction runbook** | C3 | Nothing |
...
| G6 | Backup/restore runbook + RTO/RPO + a restore drill | C4 | Nothing |
